...
<a name="install-destdir"></a>Both `PREFIX` and `DESTDIR` can be relative or absolute directories. An installation can be repeated for different locations of `PREFIX` without triggering a rebuild. The prefix directory inside of each of the [package configuration files](#pkg-config) is set to where LIBXSMM is built (staging folder). If `make PREFIX=/path/to/prefix install` is performed, the pkg-config files written to the destination will however contain that destination. If `DESTDIR` is given however, it takes precedence and is used as prefix inside of the generated pkg-config no matter if the install-target is invoked or not.
