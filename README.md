...
![This performance plot for a single Intel Xeon Phi 7120 Coprocessor card is based on code generated by running "./[make-cp2k.sh](https://github.com/hfp/libxsmm/blob/master/make-cp2k.sh) test". The actual plot was generated by "cd samples/cp2k ; ./smm-perf.sh specialized cp2k-libxsmm.png -1". The plot shows almost all performance samples (few samples fall outside of the shown 3d box to make the plot visually more appealing) of a series of code specializations as useful for CP2K Open Source Molecular Dynamics [1]. The shown performance is well above 100 GFLOPS/s for almost all cases, it reaches up to ~275 GFLOPS/s for the samples shown, and actually peaks at 375 GFLOPS/s for some of the samples not shown as part of the 3d box (towards larger M, N, and K).](samples/cp2k/results/Xeon_Phi-7120/1-cp2k-libxsmm.png)
> This performance graph for a single Intel Xeon Phi 7120 Coprocessor card is based on code generated by running "./[make-cp2k.sh](https://github.com/hfp/libxsmm/blob/master/make-cp2k.sh) test". The actual plot was generated by "cd samples/cp2k ; ./smm-perf.sh specialized cp2k-libxsmm.png -1". The plot shows almost all performance samples (few samples fall outside of the shown 3d box to make the plot visually more appealing) of a series of code specializations as useful for CP2K Open Source Molecular Dynamics [1]. The shown performance is well above 100 GFLOPS/s for almost all cases, it reaches up to ~275 GFLOPS/s for the samples shown, and actually peaks at 375 GFLOPS/s for some of the samples not shown as part of the 3d box (towards larger M, N, and K).

![This performance plot summarizes the performance as presented in figure 1 by averaging the results over K (therefore the bar on the right hand side may not reach the same maximum). As one can see, the performance is well-tuned at least across the M-N parameter space (no "cold islands", and even the lower left corner represents as reasonable achievement). Please refer to figure 1 on how to reproduce the results.](samples/cp2k/results/Xeon_Phi-7120/2-cp2k-libxsmm.png)
> This performance plot summarizes the performance as presented in figure 1 by averaging the results over K (therefore the bar on the right hand side may not reach the same maximum). As one can see, the performance is well-tuned at least across the M-N parameter space (no "cold islands", and even the lower left corner represents as reasonable achievement). Please refer to figure 1 on how to reproduce the results.

![In order to further summarize the case presented in figure 1 and 2, this graph shows the cumulative distribution function (CDF) of the performance across all cases. For example, the median value is where 50% of the cases produce less or equal 203 GFLOPS/s. Of course 100% of the cases produce less or equal 375 GFLOPS/s (maximum). The value highlighted by the arrows is usually the median value, however the plot as produced by [smm-test.plt](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/smm-test.plt) attempts to highlight a single "fair performance value" representing all cases by linearly fitting the CDF, projecting onto the x-axis, and taking the midpoint (usually at 50%). Please refer to figure 1 on how to reproduce the results.](samples/cp2k/results/Xeon_Phi-7120/3-cp2k-libxsmm.png)
> In order to further summarize the case presented in figure 1 and 2, this graph shows the cummulative distribution function (CDF) of the performance across all cases. For example, the median value is where 50% of the cases produce less or equal 203 GFLOPS/s. Of course 100% of the cases produce less or equal 375 GFLOPS/s (maximum). The value highlighted by the arrows is usually the median value, however the plot as produced by [smm-test.plt](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/smm-test.plt) attempts to highlight a single "fair performance value" representing all cases by linearly fitting the CDF, projecting onto the x-axis, and taking the midpoint (usually at 50%). Please refer to figure 1 on how to reproduce the results.
