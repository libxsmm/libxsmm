...
![This performance plot for a single Intel Xeon Phi 7120 Coprocessor card shows a "compact selection" out of 328 specializations as useful for CP2K Open Source Molecular Dynamics [1]. In addition to the samples that are falling outside of the shown 3d box (to make the plot visually more appealing), some larger problem samples (MNK > 60^3) have not been specialized and were subject to the "BLAS fallback" routine. The code has been generated by running "./[make.sh](https://github.com/hfp/libxsmm/blob/master/make.sh) -cp2k test", and this and below plots were generated by running "cd samples/cp2k ; ./[cp2k-plot.sh](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/cp2k-plot.sh) specialized cp2k-specialized.png -1".](samples/cp2k/results/Xeon_Phi-7120/1-cp2k-specialized.png)
> This performance plot for a single Intel Xeon Phi 7120 Coprocessor card shows a "compact selection" out of 328 specializations as useful for CP2K Open Source Molecular Dynamics [1]. In addition to the samples that are falling outside of the shown 3d box (to make the plot visually more appealing), some larger problem samples (MNK > 60^3) have not been specialized and were subject to the "BLAS fallback" routine. The code has been generated by running "./[make.sh](https://github.com/hfp/libxsmm/blob/master/make.sh) -cp2k test", and this and below plots were generated by running "cd samples/cp2k ; ./[cp2k-plot.sh](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/cp2k-plot.sh) specialized cp2k-specialized.png -1".

![This plot summarizes the performance of the generated kernels by averaging the results over K (therefore the bar on the right hand side may not show the same maximum when compared to other plots). The performance is well-tuned across the parameter space with no "cold islands", and a lower left "cold" corner which is fairly limited. Please refer to the first figure on how to reproduce the results.](samples/cp2k/results/Xeon_Phi-7120/2-cp2k-specialized.png)
> This plot summarizes the performance of the generated kernels by averaging the results over K (therefore the bar on the right hand side may not show the same maximum when compared to other plots). The performance is well-tuned across the parameter space with no "cold islands", and a lower left "cold" corner which is fairly limited. Please refer to the first figure on how to reproduce the results.

![This plot shows the arithmetic average of the performance with respect to groups of problem sizes (MNK). The problem sizes are binned into three groups: "small", "medium", and "larger" (according to the shown intervals). Please refer to the first figure on how to reproduce the results.](samples/cp2k/results/Xeon_Phi-7120/3-cp2k-specialized.png)
> This plot shows the arithmetic average of the performance with respect to groups of problem sizes (MNK). The problem sizes are binned into three groups: "small", "medium", and "larger" (according to the shown intervals). Please refer to the first figure on how to reproduce the results.

![In order to further summarize the previous plots, this graph shows the cumulative distribution function (CDF) of the performance across all cases. Simlar to the median value at 50%, one can read for example that 100% of the cases are yielding less or equal the largest discovered value. The value highlighted by the arrows is usually the median value, the [plot script](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/cp2k-perf.plt) however attempts to highlight a single "fair performance value" representing all cases by linearly fitting the CDF, projecting onto the x-axis, and taking the midpoint of the projection (usually at 50%). Please refer to the first figure on how to reproduce the results.](samples/cp2k/results/Xeon_Phi-7120/4-cp2k-specialized.png)
> In order to further summarize the previous plots, this graph shows the cumulative distribution function (CDF) of the performance across all cases. Simlar to the median value at 50%, one can read for example that 100% of the cases are yielding less or equal the largest discovered value. The value highlighted by the arrows is usually the median value, the [plot script](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/cp2k-perf.plt) however attempts to highlight a single "fair performance value" representing all cases by linearly fitting the CDF, projecting onto the x-axis, and taking the midpoint of the projection (usually at 50%). Please refer to the first figure on how to reproduce the results.
