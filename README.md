...
![This performance plot for a dual-socket Intel Xeon E5-2697v3 shows a "compact selection" (to make the plot visually more appealing) out of 328 specializations as useful for CP2K Open Source Molecular Dynamics [1]. The code has been generated and built by running "./[make.sh](https://github.com/hfp/libxsmm/blob/master/make.sh) -cp2k AVX=2 -j". This and below plots were generated by running "cd samples/cp2k ; ./[cp2k-plot.sh](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/cp2k-plot.sh) specialized cp2k-specialized.png -1".](samples/cp2k/results/Xeon_E5-2697v3/1-cp2k-specialized.png)
> This performance plot for a dual-socket Intel Xeon E5-2697v3 shows a "compact selection" (to make the plot visually more appealing) out of 328 specializations as useful for CP2K Open Source Molecular Dynamics [1]. The code has been generated and built by running "./[make.sh](https://github.com/hfp/libxsmm/blob/master/make.sh) -cp2k AVX=2 -j". This and below plots were generated by running "cd samples/cp2k ; ./[cp2k-plot.sh](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/cp2k-plot.sh) specialized cp2k-specialized.png -1".

![This plot summarizes the performance of the generated kernels by averaging the results over K (therefore the bar on the right hand side may not show the same maximum when compared to other plots). The performance is well-tuned across the parameter space with no "cold islands", and a lower left "cold" corner which is fairly limited. Please refer to the first figure on how to reproduce the results.](samples/cp2k/results/Xeon_E5-2697v3/2-cp2k-specialized.png)
