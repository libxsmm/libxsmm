...
![This performance plot for a single Intel Xeon Phi 7120 Coprocessor card is based on code generated by running "./[make-cp2k.sh](https://github.com/hfp/libxsmm/blob/master/make-cp2k.sh) test". The actual plot was generated by "cd samples/cp2k ; ./[cp2k-plot.sh](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/cp2k-plot.sh) specialized cp2k-specialized.png -1". The plot shows most performance samples (some samples fall outside of the shown 3d box to make the plot visually more appealing) of a series of code specializations as useful for CP2K Open Source Molecular Dynamics [1]. The shown performance is well above 100 GFLOPS/s for almost all cases, reaches up to ~250 GFLOPS/s for the samples shown, and actually peaks at ~375 GFLOPS/s for some of the samples not shown as part of the 3d box (towards larger M, N, and K).](samples/cp2k/results/Xeon_Phi-7120/1-cp2k-specialized.png)
> This performance plot for a single Intel Xeon Phi 7120 Coprocessor card is based on code generated by running "./[make-cp2k.sh](https://github.com/hfp/libxsmm/blob/master/make-cp2k.sh) test". The actual plot was generated by "cd samples/cp2k ; ./[cp2k-perf.sh](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/cp2k-perf.sh) specialized cp2k-specialized.png -1". The plot shows most performance samples (some samples fall outside of the shown 3d box to make the plot visually more appealing) of a series of code specializations as useful for CP2K Open Source Molecular Dynamics [1]. The shown performance is well above 100 GFLOPS/s for almost all cases, reaches up to ~250 GFLOPS/s for the samples shown, and actually peaks at ~375 GFLOPS/s for some of the samples not shown as part of the 3d box (towards larger M, N, and K).

![This plot summarizes the performance as presented in the previous figure by averaging the results over K (therefore the bar on the right hand side may not reach the same maximum). As one can see, the performance is well-tuned at least across the M-N parameter space (no "cold islands", and even the lower left corner represents as reasonable achievement). Please refer to the previous figure on how to reproduce the results.](samples/cp2k/results/Xeon_Phi-7120/2-cp2k-specialized.png)
> This plot summarizes the performance as presented in the previous figure by averaging the results over K (therefore the bar on the right hand side may not reach the same maximum). As one can see, the performance is well-tuned at least across the M-N parameter space (no "cold islands", and even the lower left corner represents as reasonable achievement). Please refer to the previous figure on how to reproduce the results.

![In order to further summarize further, this graph shows the cumulative distribution function (CDF) of the performance across all cases. For example, the median value is where 50% of the cases yield less or equal 203 GFLOPS/s. Of course 100% of the cases yielding less or equal 375 GFLOPS/s (maximum). The value highlighted by the arrows is usually the median value, however the plot (as produced by [cp2k-perf.plt](https://github.com/hfp/libxsmm/blob/master/samples/cp2k/cp2k-perf.plt)) attempts to highlight a single "fair performance value" representing all cases by linearly fitting the CDF, projecting onto the x-axis, and taking the midpoint (usually at 50%). Please refer to the first figure on how to reproduce the results.](samples/cp2k/results/Xeon_Phi-7120/4-cp2k-specialized.png)
