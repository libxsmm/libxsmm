...
The above case is obviously requiring to build the shared library of LIBXSMM:
...
### Call Trace
During the initial steps of employing the API, one may rely on a debug version of the library (`make DBG=1`). The latter implies also standard error output in case of an unexpected condition inside of the library (otherwise the library is not performing any visible or non-private I/O). Towards an application which is successfully using the library, being able to trace library calls might be useful:

```
make TRACE=1
```

In order to actually tracing calls without running in a debugger can be accomplished using an environment variable called LIBXSMM_TRACE.
...
Please note, that debug symbols without the trace facility and without any eventual console output can be enabled separately (`make SYM=1`) which might be useful when profiling an application.
