...
void libxsmm_generator_packed_spgemm_csr_bsparse_aarch64( libxsmm_generated_code*         io_generated_code,
 ...
 /* packed blocking  */
  ...
  while ( l_packed_done != l_simd_packed_iters ) {
    ...
    while ( l_n_done < l_max_cols ) {
      ...
      for ( l_n_processed = l_n_done; l_n_processed < l_n_done + l_col_reg_range[l_packed_count][l_n_count]; l_n_processed += l_n_blocking ) {
        libxsmm_generator_packed_spgemm_csr_bsparse_aarch64_kloop( io_generated_code,
  ...
  /* advance A pointer */
  libxsmm_aarch64_instruction_alu_set_i64( io_generated_code, l_gp_reg_mapping.gp_reg_help_0,
...
LIBXSMM_API_INTERN
void libxsmm_generator_packed_spgemm_csr_bsparse_aarch64_kloop( libxsmm_generated_code*            io_generated_code,
        ...
        for ( l_n = 0; l_n < l_row_elements; l_n++ ) {
          ...
          if ( (i_column_idx[i_row_idx[l_k] + l_n] < (unsigned int)i_xgemm_desc->n) &&
            ...
            libxsmm_aarch64_instruction_alu_set_i64( io_generated_code, i_gp_reg_mapping->gp_reg_help_1,
                                                     (i_row_idx[l_k] + l_n) * i_micro_kernel_config->datatype_size );
            libxsmm_aarch64_instruction_asimd_move( io_generated_code, LIBXSMM_AARCH64_INSTR_ASIMD_LDR_R,
                                                    i_gp_reg_mapping->gp_reg_b, i_gp_reg_mapping->gp_reg_help_1, 0,
                                                    l_max_reg_block+1, (i_micro_kernel_config->datatype_size == 4) ?  LIBXSMM_AARCH64_ASIMD_WIDTH_S : LIBXSMM_AARCH64_ASIMD_WIDTH_D );
  ...
  if ( i_packed_range/i_packed_blocking > 1 ) {
    ...
    libxsmm_aarch64_instruction_alu_set_i64( io_generated_code, i_gp_reg_mapping->gp_reg_help_2,
                                             i_packed_blocking*i_micro_kernel_config->vector_length*i_micro_kernel_config->datatype_size );
    libxsmm_aarch64_instruction_alu_compute_shifted_reg( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_ADD_SR,
    ...
    libxsmm_aarch64_instruction_alu_set_i64( io_generated_code, i_gp_reg_mapping->gp_reg_help_2,
                                             (i_packed_range/i_packed_blocking)*i_micro_kernel_config->vector_length*i_micro_kernel_config->datatype_size );
