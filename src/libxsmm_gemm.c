  ...
  const void* alpha, const void* beta, int flags, /*unsigned*/int ntasks)
{
  ...
  if (NULL != blob && NULL != m && 0 < ntasks) {
    ...
    if (LIBXSMM_GEMM_FLAG_TRANS_AB != (LIBXSMM_GEMM_FLAG_TRANS_AB & result.ptr->gemm_flags)) { /* NN, NT, or TN */
...
#if defined(LIBXSMM_GEMM_XCOPY_JIT)
        const libxsmm_meltw_unary_shape unary_shape = libxsmm_create_meltw_unary_shape(
          itm, itk, klda, itm, (libxsmm_datatype)iprec, (libxsmm_datatype)iprec, (libxsmm_datatype)iprec );
        result.ptr->copy_a.function = libxsmm_dispatch_meltw_unary_v2(
...
#if defined(LIBXSMM_GEMM_XCOPY_JIT)
        const libxsmm_meltw_unary_shape unary_shape = libxsmm_create_meltw_unary_shape(
          itn, itk, kldb, itk, (libxsmm_datatype)iprec, (libxsmm_datatype)iprec, (libxsmm_datatype)iprec );
...
#if defined(LIBXSMM_GEMM_XCOPY_JIT)
        const libxsmm_meltw_unary_shape unary_shape = libxsmm_create_meltw_unary_shape(
          itk, itn, kldb, itk, (libxsmm_datatype)iprec, (libxsmm_datatype)iprec, (libxsmm_datatype)iprec );
        result.ptr->copy_b.function = libxsmm_dispatch_meltw_unary_v2(
          LIBXSMM_MELTW_TYPE_UNARY_IDENTITY, unary_shape, LIBXSMM_MELTW_FLAG_UNARY_NONE );
#endif
      ...
      if (0 != (LIBXSMM_GEMM_HANDLE_FLAG_COPY_C & result.ptr->flags)) {
#if defined(LIBXSMM_GEMM_XCOPY_JIT)
        const libxsmm_meltw_unary_shape unary_shape = libxsmm_create_meltw_unary_shape(
          itm, itn, itm, kldc, (libxsmm_datatype)oprec, (libxsmm_datatype)oprec, (libxsmm_datatype)oprec );
        result.ptr->copy_o.function = libxsmm_dispatch_meltw_unary_v2(
          LIBXSMM_MELTW_TYPE_UNARY_IDENTITY, unary_shape, LIBXSMM_MELTW_FLAG_UNARY_NONE );
        if (0 == (result.ptr->gemm_flags & LIBXSMM_GEMM_FLAG_BETA_0)) { /* copy-in only if beta!=0 */
          const libxsmm_meltw_unary_shape unary_shape2 = libxsmm_create_meltw_unary_shape(
            itm, itn, kldc, itm, (libxsmm_datatype)oprec, (libxsmm_datatype)oprec, (libxsmm_datatype)oprec );
          result.ptr->copy_i.function = libxsmm_dispatch_meltw_unary_v2(
      ...
      const libxsmm_meltw_unary_shape unary_shape = libxsmm_create_meltw_unary_shape(
        itt, itt, itt, ildc, (libxsmm_datatype)oprec, (libxsmm_datatype)oprec, (libxsmm_datatype)oprec );
      result.ptr->copy_o.function = libxsmm_dispatch_meltw_unary_v2(
        LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_NORM_TO_NORMT, unary_shape, LIBXSMM_MELTW_FLAG_UNARY_NONE );
      if (0 == (result.ptr->gemm_flags & LIBXSMM_GEMM_FLAG_BETA_0)) { /* copy-in only if beta!=0 */
        const libxsmm_meltw_unary_shape unary_shape2 = libxsmm_create_meltw_unary_shape(
          itt, itt, ildc, itt, (libxsmm_datatype)oprec, (libxsmm_datatype)oprec, (libxsmm_datatype)oprec );
        result.ptr->copy_i.function = libxsmm_dispatch_meltw_unary_v2(
          LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_NORM_TO_NORMT, unary_shape2, LIBXSMM_MELTW_FLAG_UNARY_NONE );
      }
#endif
  ...
  if (begin < end) {
    ...
    if (0 == (LIBXSMM_GEMM_FLAG_BATCH_REDUCE_ADDRESS & flags)) {
      if (0 != index_stride) { /* stride arrays contain indexes */
        libxsmm_blasint i = begin * index_stride, ic = (NULL != stride_c ? (LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_c, i) - index_base) : 0);
        const char* ai = &a0[NULL != stride_a ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_a, i) - index_base) * itypesize) : 0];
        const char* bi = &b0[NULL != stride_b ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_b, i) - index_base) * itypesize) : 0];
          ...
          if (NULL != stride_a && NULL != stride_b && NULL != stride_c) {
            ...
            if (itypesize == (1 << ibits) && otypesize == (1 << obits)) {
              for (i += index_stride; i <= end1; i += index_stride) {
                const char *const an = &a0[(LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_a, i) - index_base) << ibits];
                const char *const bn = &b0[(LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_b, i) - index_base) << ibits];
                char       *const cn = &c0[(LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_c, i) - index_base) << obits];
              ...
              for (i += index_stride; i <= end1; i += index_stride) {
                const char *const an = &a0[(LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_a, i) - index_base) * itypesize];
                const char *const bn = &b0[(LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_b, i) - index_base) * itypesize];
                char       *const cn = &c0[(LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_c, i) - index_base) * otypesize];
                kernel.xmm(ai, bi, ci/*, an, bn, cn*/); /* @TODO fix prefetch */
                ai = an; bi = bn; ci = cn; /* next */
              }
            }
            ...
            for (i += index_stride; i <= end1; i += index_stride) {
              const char *const an = &a0[NULL != stride_a ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_a, i) - index_base) * itypesize) : 0];
              const char *const bn = &b0[NULL != stride_b ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_b, i) - index_base) * itypesize) : 0];
              char       *const cn = &c0[NULL != stride_c ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_c, i) - index_base) * otypesize) : 0];
              kernel.xmm(ai, bi, ci/*, an, bn, cn*/); /* @TODO fix prefetch */
              ai = an; bi = bn; ci = cn; /* next */
            }
          }
...
# endif
          ...
          if (NULL != stride_a && NULL != stride_b && NULL != stride_c) {
            for (i += index_stride; i <= end1; i += index_stride) {
              ic = LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_c, i) - index_base;
              {
                const char *const an = &a0[(LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_a, i) - index_base) * itypesize];
                const char *const bn = &b0[(LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_b, i) - index_base) * itypesize];
...
# endif
            ...
            for (i += index_stride; i <= end1; i += index_stride) {
              ic = (NULL != stride_c ? (LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_c, i) - index_base) : 0);
              {
                const char *const an = &a0[NULL != stride_a ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_a, i) - index_base) * itypesize) : 0];
                const char *const bn = &b0[NULL != stride_b ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_b, i) - index_base) * itypesize) : 0];
                char       *const cn = &c0[ic * otypesize];
...
# endif
    {
      ...
      if ((2U/*A and B matrices*/ * tasksize) <= n) {
        ...
        if (0 != index_stride) { /* stride arrays contain indexes */
          ...
          char *ci = &c0[NULL != stride_c ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_c, i) - index_base) * otypesize) : 0], *cn = ci;
          do {
            for (count = 0; i < end_stride && ci == cn; ++count) {
              const size_t j = i + index_stride;
              *ai++ = &a0[NULL != stride_a ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_a, i) - index_base) * itypesize) : 0];
              *bi++ = &b0[NULL != stride_b ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_b, i) - index_base) * itypesize) : 0];
                 cn = &c0[NULL != stride_c ? ((LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_c, j) - index_base) * otypesize) : 0];
              i = j;
            }
  ...
  libxsmm_blasint batchsize)
{
...
#endif
  {
    ...
    if (0 != index_stride) { /* stride arrays contain indexes */
      ...
      for (i = index_stride; i <= end1; i += index_stride) {
        const double *const an = &a0[NULL != stride_a ? (LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_a, i) - index_base) : 0];
        const double *const bn = &b0[NULL != stride_b ? (LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_b, i) - index_base) : 0];
        double       *const cn = &c0[NULL != stride_c ? (LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_c, i) - index_base) : 0];
  ...
  libxsmm_blasint batchsize)
{
...
#endif
  {
    ...
    if (0 != index_stride) { /* stride arrays contain indexes */
      ...
      for (i = index_stride; i <= end1; i += index_stride) {
        const float *const an = &a0[NULL != stride_a ? (LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_a, i) - index_base) : 0];
        const float *const bn = &b0[NULL != stride_b ? (LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_b, i) - index_base) : 0];
        float       *const cn = &c0[NULL != stride_c ? (LIBXSMM_ACCESS_VALUE(1, const libxsmm_blasint, stride_c, i) - index_base) : 0];
