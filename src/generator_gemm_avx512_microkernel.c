                                                                             ...
                                                                             const int                          i_offset )
{
  ...
  for ( l_n = 0; l_n < i_n_blocking; l_n++ ) {
    ...
    if (l_is_Ai8_Bbf16_gemm > 0) {
      if ((i_micro_kernel_config->is_last_k_microkernel > 0) && ((i_xgemm_desc->k % 2) != 0)) {
    ...
    if ( i_offset != (-1) ) {
      ...
      /* handle trans B */
      if ( (i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_B) > 0 ) {
        unsigned int l_k_pack_advance = (l_is_Ai8_Bbf16_gemm > 0) ? ((i_micro_kernel_config->is_last_k_microkernel > 0) && (i_xgemm_desc->k % 2 != 0) ? 1 : 2) : l_k_pack_factor;
