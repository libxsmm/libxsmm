                                                                             ...
                                                                             const int                          i_offset )
{
  ...
  for ( l_m = 0; l_m < l_m_blocking; l_m++ ) {
    libxsmm_x86_instruction_vec_move( io_generated_code,
        ...
        (i_micro_kernel_config->datatype_size_in) * (i_micro_kernel_config->vector_length) * l_m * l_k_step,
    ...
    if ( i_xgemm_desc->prefetch == LIBXSMM_GEMM_PREFETCH_AL2_AHEAD || i_xgemm_desc->prefetch == LIBXSMM_GEMM_PREFETCH_AL2BL2_VIA_C_AHEAD ) {
      libxsmm_x86_instruction_prefetch( io_generated_code,
          ...
          i_gp_reg_mapping->gp_reg_a,
          LIBXSMM_X86_GP_REG_UNDEF, 0,
          ((i_micro_kernel_config->datatype_size_in) * (i_micro_kernel_config->vector_length) * l_m * l_k_step) + (64 * l_m_blocking) );
    }
  ...
  for ( l_n = 0; l_n < i_n_blocking; l_n++ ) {
    if ( i_offset != (-1) ) {
      ...
      /* handle trans B */
      if ( (i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_B) > 0 ) {
        l_b_offset = l_n * i_micro_kernel_config->datatype_size_in * l_k_step;
      } else {
      ...
      if (l_n == i_n_blocking - 1) {
        ...
        if ( (i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_B) > 0 ) {
          ...
          l_b_offset = i_micro_kernel_config->datatype_size_in * l_k_step;
        }
    ...
    if ((l_n < l_m_blocking)  && ((i_xgemm_desc->prefetch & LIBXSMM_GEMM_PREFETCH_BRGEMM_OOB) > 0) && (LIBXSMM_DATATYPE_I8 != LIBXSMM_GETENUM_INP( i_xgemm_desc->datatype)) && (LIBXSMM_DATATYPE_BF16 != LIBXSMM_GETENUM_INP( i_xgemm_desc->datatype)) && ((i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_BATCH_REDUCE_ADDRESS) || (i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_BATCH_REDUCE_OFFSET) || (i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_BATCH_REDUCE_STRIDE))) {
      ...
      libxsmm_x86_instruction_prefetch( io_generated_code,
          ...
          i_gp_reg_mapping->gp_reg_a,
          LIBXSMM_X86_GP_REG_UNDEF, 0,
          (i_micro_kernel_config->datatype_size_in) * (i_micro_kernel_config->vector_length) * l_n * l_k_step + pf_a_cols_ahead * i_xgemm_desc->lda * i_micro_kernel_config->datatype_size_in);
    }
    ...
    for ( l_m = 0; l_m < l_m_blocking; l_m++ ) {
      ...
      if ( (l_m == 0) && (l_n == i_n_blocking-1) ) {
        libxsmm_x86_instruction_alu_imm( io_generated_code,
            i_micro_kernel_config->alu_add_instruction,
            i_gp_reg_mapping->gp_reg_a,
            (long long)i_xgemm_desc->lda * i_micro_kernel_config->datatype_size_in * l_k_step);
  ...
  for ( l_pass = 0; l_pass < 4; ++l_pass ) {
    ...
    for ( l_n = 0; l_n < i_n_blocking; l_n++ ) {
      if ( i_offset != (-1) ) {
        ...
        /* handle trans B */
        if ( (i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_B) > 0 ) {
          l_b_offset = (l_n * i_micro_kernel_config->datatype_size_in) + l_pass;
        } else {
                                                                           ...
                                                                           const unsigned int                 i_k_blocking )
{
  ...
  /* apply k blocking */
  for ( l_k = 0; l_k < i_k_blocking/l_k_step; l_k++ ) {
    if ( l_k == 0 ) {
       /* load A */
      libxsmm_x86_instruction_vec_move( io_generated_code,
                                        ...
                                        i_gp_reg_mapping->gp_reg_a,
                                        LIBXSMM_X86_GP_REG_UNDEF, 0,
                                        i_xgemm_desc->lda * l_k * i_micro_kernel_config->datatype_size_in * l_k_step,
                                        l_vec_name_ld_a,
                                        0,
      ...
      if ( i_xgemm_desc->prefetch == LIBXSMM_GEMM_PREFETCH_AL2_AHEAD ||
        ...
        libxsmm_x86_instruction_prefetch( io_generated_code,
                                          ...
                                          i_gp_reg_mapping->gp_reg_a,
                                          LIBXSMM_X86_GP_REG_UNDEF, 0,
                                          (i_xgemm_desc->lda * l_k + i_micro_kernel_config->datatype_size_in * l_k_step) + 64 );
      }
      if ( i_k_blocking/l_k_step > 1 ) {
        ...
        libxsmm_x86_instruction_vec_move( io_generated_code,
                                          ...
                                          i_gp_reg_mapping->gp_reg_a,
                                          LIBXSMM_X86_GP_REG_UNDEF, 0,
                                          i_xgemm_desc->lda * (l_k+1) * i_micro_kernel_config->datatype_size_in * l_k_step,
                                          l_vec_name_ld_a,
                                          1,
                                          i_micro_kernel_config->use_masking_a_c, 1, 0 );
        /* current A prefetch, next rows for the current column */
        if ( i_xgemm_desc->prefetch == LIBXSMM_GEMM_PREFETCH_AL2_AHEAD ||
          ...
          libxsmm_x86_instruction_prefetch( io_generated_code,
                                            ...
                                            i_gp_reg_mapping->gp_reg_a,
                                            LIBXSMM_X86_GP_REG_UNDEF, 0,
                                            (i_xgemm_desc->lda * (l_k+1) * i_micro_kernel_config->datatype_size_in * l_k_step) + 64 );
        }
      }
    } else if ( l_k < (i_k_blocking/l_k_step - 1) ) {
    ...
    if ( i_xgemm_desc->prefetch == LIBXSMM_GEMM_PREFETCH_AL2 ||
      ...
      libxsmm_x86_instruction_prefetch( io_generated_code,
                                        ...
                                        LIBXSMM_X86_GP_REG_UNDEF, 0,
                                        (i_xgemm_desc->lda * l_k * i_micro_kernel_config->datatype_size_in * l_k_step) );
      if ( l_k == (i_k_blocking/l_k_step - 1) ) {
        libxsmm_x86_instruction_alu_imm( io_generated_code,
                                         i_micro_kernel_config->alu_add_instruction,
                                         i_gp_reg_mapping->gp_reg_a_prefetch,
    ...
    if ( (i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_B) == 0 ) {
      for ( l_n = 0; l_n < i_n_blocking; l_n++) {
        ...
        unsigned int l_disp = (l_k*i_micro_kernel_config->datatype_size_in*l_k_step)+(l_n*i_xgemm_desc->ldb*i_micro_kernel_config->datatype_size_in);
      ...
      for ( l_n = 0; l_n < i_n_blocking; l_n++) {
        ...
        unsigned int l_b_idx = LIBXSMM_X86_GP_REG_UNDEF;
        unsigned int l_scale = 0;
        unsigned int l_disp = (l_k*i_xgemm_desc->ldb*i_micro_kernel_config->datatype_size_in*l_k_step) + (l_n*i_micro_kernel_config->datatype_size_in);
