                                                                             ...
                                                                             const int                          i_offset )
{
  ...
  for ( l_m = 0; l_m < l_m_blocking; l_m++ ) {
    ...
    if (l_is_Af16_Bf16_Cf16_gemm > 0 && l_use_f16_replacement_fma > 0) {
      libxsmm_x86_instruction_vec_compute_2reg( io_generated_code, LIBXSMM_X86_INSTR_VCVTPH2PS, vname_cvt, 1+l_m+l_vreg_ab_offset, 1+l_m+l_vreg_ab_offset );
    }
  ...
  for ( l_n = 0; l_n < i_n_blocking; l_n++ ) {
    if ( i_offset != (-1) ) {
      ...
      if ((l_is_Af16_Bf16_Cf16_gemm > 0 || l_is_Ai8_Bf16_Cf16_gemm > 0) && l_use_f16_replacement_fma > 0) {
    ...
    for ( l_m = 0; l_m < l_m_blocking; l_m++ ) {
      ...
      } else if ((l_is_Af16_Bf16_Cf16_gemm > 0 || l_is_Ai8_Bf16_Cf16_gemm > 0) && l_use_f16_replacement_fma > 0) {
    ...
    if ( (i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_B) == 0 ) {
      for ( l_n = 0; l_n < i_n_blocking; l_n++) {
        ...
        if ( LIBXSMM_DATATYPE_F32 == LIBXSMM_GEMM_GETENUM_AB_COMMON_PREC( i_xgemm_desc->datatype ) || LIBXSMM_DATATYPE_F16 == LIBXSMM_GEMM_GETENUM_AB_COMMON_PREC( i_xgemm_desc->datatype ) || l_is_Ai8_Bf16_Cf16_gemm > 0 || LIBXSMM_DATATYPE_F64 == LIBXSMM_GEMM_GETENUM_AB_COMMON_PREC( i_xgemm_desc->datatype ) ) {
          if (((l_is_Ai8_Bf16_Cf16_gemm > 0) || (l_is_Af16_Bf16_Cf16_gemm > 0)) && (l_use_f16_replacement_fma > 0)) {
            libxsmm_x86_instruction_vec_move( io_generated_code,
                                              io_generated_code->arch,
