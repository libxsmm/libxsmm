...
void libxsmm_generator_packed_spgemm_bcsc_bsparse_aarch64( libxsmm_generated_code*         io_generated_code,
  ...
  if (((LIBXSMM_DATATYPE_BF16 == LIBXSMM_GEMM_GETENUM_AB_COMMON_PREC( i_xgemm_desc->datatype )) || (LIBXSMM_DATATYPE_I8 == LIBXSMM_GEMM_GETENUM_AB_COMMON_PREC( i_xgemm_desc->datatype ))) && (io_generated_code->arch == LIBXSMM_AARCH64_NEOV1)) {
...
void libxsmm_generator_packed_spgemm_bcsc_bsparse_aarch64_kloop_mmla_sve( libxsmm_generated_code*            io_generated_code,
  ...
  unsigned int l_n = 0;
  unsigned int l_p = 0;
  unsigned int l_max_reg_block = (i_bn+1)/2 * i_packed_blocking;
  ...
  if (l_beta_0 > 0) {
    ...
    for ( l_n = 0; l_n < l_n_blocking; l_n++ ) {
      ...
      for ( l_p = 0; l_p < i_packed_blocking; l_p++ ) {
        ...
        /* Load n1 4m */
        if ((l_n == l_n_blocking-1) && (i_bn % 2 != 0)) {
