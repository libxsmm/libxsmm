...
LIBXSMM_API void lsfr_Xwide( unsigned int* rng_state, float* prng_out, const unsigned int width ) {
...
LIBXSMM_API void libxsmm_stochastic_convert_fp32_bf8(const float* in, libxsmm_bfloat8* out, unsigned int len, void *rng_state) {
  ...
  for ( i = 0; i < len; ++i ) {
    ...
    float vrng[16];
    lsfr_Xwide( (unsigned int*)rng_state, vrng, 1 );
    unsigned short rand = (unsigned short) vrng[1];
    ...
    if ( do_round != 0) {
      if ( (short_round & 0x7c00) == 0x0000 && (short_round & 0x7fff) > 0x0000 ) {
