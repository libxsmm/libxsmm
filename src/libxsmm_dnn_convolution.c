...
LIBXSMM_API_INLINE int libxsmm_dnn_convolution_setup_fwd_ofh_rb( libxsmm_dnn_layer* handle ) {
  ...
  if (handle->libxsmm_target_archid == LIBXSMM_X86_AVX512_SPR) {
...
LIBXSMM_API_INLINE int libxsmm_dnn_convolution_setup_blocksifm_blocking( libxsmm_dnn_layer* handle ) {
  ...
  if ((handle->desc.R == 1) && (handle->desc.S == 1) ) {
    ...
    if ((handle->libxsmm_target_archid < LIBXSMM_X86_AVX512) && (handle->desc.C >= 512) && (handle->desc.K >= 512) ) {
      result = 2;
    }
  } else {
    result = 1;
...
LIBXSMM_API_INLINE void libxsmm_dnn_convolution_setup_bf16_upd_amx( libxsmm_dnn_layer* handle ) {
  ...
  handle->use_hybrid_imgofm_parallelization = 0;
  handle->upd_linearized_tasklist = 0;
  if ((handle->libxsmm_target_archid == LIBXSMM_X86_AVX512_SPR) && (handle->ofw == 7) && (handle->desc.R == 1) && (handle->desc.S == 1) ) {
...
LIBXSMM_API_INLINE libxsmm_dnn_err_t libxsmm_dnn_convolution_setup( libxsmm_dnn_layer* handle ) {
  ...
  handle->libxsmm_target_archid = libxsmm_target_archid;
  if ( (handle->libxsmm_target_archid == LIBXSMM_X86_AVX512_SPR) && (handle->datatype_in == LIBXSMM_DNN_DATATYPE_BF16) && ((handle->desc.C % 16 != 0) || (handle->desc.K % 16 != 0)) ) {
    handle->libxsmm_target_archid = LIBXSMM_X86_AVX512_CPX;
  }
  ...
  if ( (handle->libxsmm_target_archid == LIBXSMM_X86_AVX512_SPR) && (handle->datatype_in == LIBXSMM_DNN_DATATYPE_BF16) ) {
    ...
    if (handle->desc.R == 1 && handle->desc.S == 1) {
      ...
      handle->fwd_compute_kernel_strd = libxsmm_bsmmdispatch_reducebatch_strd_unroll(handle->ofmblock, handle->fwd_gemm_pixels, handle->ifmblock, stride_a, stride_b, handle->blocksifm_blocking, &ldA, &ldx, &ldC, NULL, &beta, &l_flags, NULL);
    } else {
      const int IFW = (handle->pack_input == 1) ? handle->ofwp : handle->ifwp;
      const int IFH = (handle->pack_input == 1) ? handle->ofhp : handle->ifhp;
      ...
      handle->fwd_compute_kernel_offs = libxsmm_bsmmdispatch_reducebatch_offs(handle->ofmblock, handle->fwd_gemm_pixels, handle->ifmblock, &ldA, &ldx, &ldC, NULL, &beta, &l_flags, NULL);
    }
  ...
  if (handle->datatype_in == LIBXSMM_DNN_DATATYPE_BF16) {
    ...
    handle->fwd_cvtfp32bf16_kernel = libxsmm_dispatch_meltw_cvtfp32bf16(handle->ofmblock * handle->fwd_ofw_rb, handle->fwd_ofh_rb, &_ldi, &_ldo, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_BF16);
  }
