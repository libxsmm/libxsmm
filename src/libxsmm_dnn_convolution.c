...
LIBXSMM_API_INLINE libxsmm_dnn_err_t libxsmm_dnn_convolution_setup( libxsmm_dnn_layer* handle ) {
  ...
  if (handle->datatype_in == LIBXSMM_DNN_DATATYPE_BF16) {
    ...
    handle->fwd_cvtfp32bf16_kernel = libxsmm_dispatch_metlw_cvtfp32bf16(handle->ofmblock * handle->fwd_ofw_rb, handle->fwd_ofh_rb, &_ldi, &_ldo, LIBXSMM_DNN_DATATYPE_F32, LIBXSMM_DNN_DATATYPE_BF16);
  }
