...
void libxsmm_generator_vcvt_f32i8_aarch64( libxsmm_generated_code* io_generated_code,
  ...
  if ( io_generated_code->arch == LIBXSMM_AARCH64_V81 || io_generated_code->arch == LIBXSMM_AARCH64_V82 || io_generated_code->arch == LIBXSMM_AARCH64_APPL_M1 ) {
    ...
    libxsmm_generator_vcvt_f32i8_aarch64_sve( io_generated_code, i_vec_inout, i_scf_vec_reg, i_pred_reg );
  }
}
...
void libxsmm_generator_vcvt_i8f32_aarch64( libxsmm_generated_code* io_generated_code,
  ...
  if ( io_generated_code->arch == LIBXSMM_AARCH64_V81 || io_generated_code->arch == LIBXSMM_AARCH64_V82 || io_generated_code->arch == LIBXSMM_AARCH64_APPL_M1 ) {
    ...
    libxsmm_generator_vcvt_i8f32_aarch64_sve( io_generated_code, i_vec_inout, i_scf_vec_reg, i_pred_reg );
  }
}
...
LIBXSMM_API_INTERN
void libxsmm_generator_load_2dregblock_aarch64_sme( libxsmm_generated_code* io_generated_code,
                                                    const unsigned int      i_gp_reg_addr,
  ...
  for( int i = 0; i < 4; i++){
    libxsmm_aarch64_instruction_alu_move_imm16( io_generated_code,
                                                ...
                                                LIBXSMM_AARCH64_GP_REG_W12+i,
                                                0,
                                                i );
  }
  if( i_m_blocking == 32 && i_n_blocking == 32 ){
    libxsmm_aarch64_instruction_set_ptrue_as_counter_sve2( io_generated_code,
                                                           LIBXSMM_AARCH64_INSTR_SVE2_PTRUE_AS_COUNTER,
    ...
    for( int l = 0; l < 2; l++){
      ...
      for( int i = 0; i< 16; i++){
        ...
        libxsmm_aarch64_instruction_sme_mov( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_LD1W_2,
                                            LIBXSMM_AARCH64_SVE_REG_Z0+k,
...
void libxsmm_generated_load_64x16_aarch64_sme( libxsmm_generated_code* io_generated_code,
  ...
  libxsmm_generator_set_pn_register_aarch64_sve2( io_generated_code,
                                                    ...
                                                    LIBXSMM_AARCH64_GP_REG_X11,
                                                    1 );
  for( unsigned int l_block = 0 ; l_block < ((i_n_blocking > 8 ) ? 2 : 1); l_block++){
    /* load first block of C */
    if( l_block == 0){
      for( unsigned int l_en = 0; l_en < ((i_n_blocking > 3) ? 4 : i_n_blocking); l_en++){
        libxsmm_aarch64_instruction_sme_mov( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_LD1W_4,
                                            l_en,
                                            i_gp_reg_addr,
                                            0,
                                            LIBXSMM_AARCH64_SVE_REG_P8 );
      ...
      for( unsigned int l_en = 16; l_en < ((i_n_blocking > 7) ? 20 : (i_n_blocking + 12)); l_en++){
        libxsmm_aarch64_instruction_sme_mov( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_LD1W_4,
                                            l_en,
                                            i_gp_reg_addr,
      ...
      /* load second block of C */
      for( unsigned int l_en = 0; l_en < ((i_n_blocking > 11) ? 4 : (i_n_blocking - 8)); l_en++){
        libxsmm_aarch64_instruction_sme_mov( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_LD1W_4,
                                            l_en,
                                            i_gp_reg_addr,
      ...
      for( unsigned int l_en = 16; l_en < ((i_n_blocking > 15) ? 20 : (i_n_blocking + 4)); l_en++){
        libxsmm_aarch64_instruction_sme_mov( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_LD1W_4,
                                            l_en,
                                            i_gp_reg_addr,
    ...
    for( unsigned int i = 0; i < 2; i++){
      libxsmm_aarch64_instruction_sme_mova( io_generated_code,
                                            ...
                                            LIBXSMM_AARCH64_SVE_REG_Z0+i*4 );
    ...
    for( unsigned int i = 2; i < 4; i++){
      libxsmm_aarch64_instruction_sme_mova( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_MOVA_32_BIT_VECTOR_TO_TILE,
                                            1,
                                            LIBXSMM_AARCH64_GP_REG_W13,
                                            LIBXSMM_AARCH64_SVE_REG_Z1+i*4 );
    ...
    for( unsigned int i = 4; i < 6; i++){
      libxsmm_aarch64_instruction_sme_mova( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_MOVA_32_BIT_VECTOR_TO_TILE,
                                            2,
                                            LIBXSMM_AARCH64_GP_REG_W14,
                                            LIBXSMM_AARCH64_SVE_REG_Z2+i*4 );
    ...
    for( unsigned int i = 6; i < 8; i++){
      libxsmm_aarch64_instruction_sme_mova( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_MOVA_32_BIT_VECTOR_TO_TILE,
                                            3,
                                            LIBXSMM_AARCH64_GP_REG_W15,
                                            LIBXSMM_AARCH64_SVE_REG_Z3+i*4 );
...
LIBXSMM_API_INTERN
void libxsmm_generator_store_2dregblock_aarch64_sme( libxsmm_generated_code* io_generated_code,
                                                     const unsigned int      i_gp_reg_addr,
                                                     const unsigned int      i_gp_reg_help,
                                                     const unsigned int      i_m_blocking,
                                                     const unsigned int      i_n_blocking,
  ...
  if( i_m_blocking == 32 && i_n_blocking == 32 ){
    ...
    for( int l = 0; l < 2; l++){
      ...
      /* load values */
        ...
        for( int i = 0; i < 4; i++){
          libxsmm_aarch64_instruction_sme_mova( io_generated_code,
                                                (i_ho == 1) ? LIBXSMM_AARCH64_INSTR_SME_MOVA_32_BIT_TILE_TO_VECTOR : LIBXSMM_AARCH64_INSTR_SME_MOVA_V_TILE_TO_VECTOR,
                                                l*2,
                                                LIBXSMM_AARCH64_GP_REG_W12+l*2,
                                                (i<2) ? LIBXSMM_AARCH64_SVE_REG_Z0+i : LIBXSMM_AARCH64_SVE_REG_Z0+i+2 );
        ...
        libxsmm_aarch64_instruction_sme_mov( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_ST1W_2,
                                            LIBXSMM_AARCH64_SVE_REG_Z0+k,
                                            (l==0) ? i_gp_reg_addr : i_gp_reg_help,
                                            0,
                                            LIBXSMM_AARCH64_SVE_REG_P8);
    ...
    for( unsigned int l_en = 0; l_en < l_n_block_count ; l_en++){
      ...
      for( unsigned int l_em = 0; l_em < ((l_n_count > 16 ) ? 16 : l_n_count); l_em++){
        ...
        libxsmm_aarch64_instruction_sve_move( io_generated_code,
                                              ...
                                              LIBXSMM_AARCH64_GP_REG_UNDEF,
                                              0,                          // offset mul vl
                                              l_em+l_register_offset,     // vector register
                                              LIBXSMM_AARCH64_SVE_REG_P0 );
        if( i_m_blocking > 16 ){
          libxsmm_aarch64_instruction_sve_move( io_generated_code,
                                                LIBXSMM_AARCH64_INSTR_SVE_ST1W_I_OFF,
                                                ...
                                                LIBXSMM_AARCH64_GP_REG_UNDEF,
                                                1,                        // offset mul vl
                                                l_em+8+l_register_offset, // vector register
                                                LIBXSMM_AARCH64_SVE_REG_P2 );
        }
          /* adjust pointer */
        libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code,
...
void libxsmm_generated_store_64x16_aarch64_sme( libxsmm_generated_code* io_generated_code,
  ...
  for( unsigned int l_block = 0 ; l_block < ((i_n_blocking > 8 ) ? 2 : 1); l_block++){
    ...
    if( l_block == 0){
      for( unsigned int l_en = 0; l_en < ((i_n_blocking > 3) ? 4 : i_n_blocking); l_en++){
        libxsmm_aarch64_instruction_sme_mov( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_ST1W_4,
                                            l_en,
                                            i_gp_reg_addr,
                                            0,
                                            LIBXSMM_AARCH64_SVE_REG_P8 );
...
void libxsmm_generator_transpose_sme( libxsmm_generated_code * io_generated_code,
  ...
  if( i_n_blocking > 16){
    l_k_blocksize = (i_k_rest == 0) ? 16 : i_k_rest;
  } else {
    l_k_blocksize = (i_k_rest == 0) ? 32 : i_k_rest;
  }
  ...
  /* reset pointer gp register */
  libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_SUB,
                                                  i_gp_reg_addr, i_help_register, i_gp_reg_addr,
                                                  (long long)(i_k * i_n_blocking * 4)  );

  /* mova with vertical look to register */
  /* set w12-13 to 0-1*/
  for( int i = 0; i < 2; i++){
    libxsmm_aarch64_instruction_alu_move_imm16( io_generated_code,
                                                LIBXSMM_AARCH64_INSTR_GP_MOVZ,
  ...
  /* mov to from tile to register */
  for( int i = 0; i < 4; i++){
    libxsmm_aarch64_instruction_sme_mova( io_generated_code,
                                          LIBXSMM_AARCH64_INSTR_SME_MOVA_V_TILE_TO_VECTOR,
                                          0,
                                          LIBXSMM_AARCH64_GP_REG_W12,
                                          i );
  ...
  if( i_n_blocking > 16 || i_k_rest > 16 ){
    for( int i = 4; i < 8; i++){
      libxsmm_aarch64_instruction_sme_mova( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SME_MOVA_V_TILE_TO_VECTOR,
                                            1,
  ...
  /* set predication for i_n_blocking */
  if( i_n_blocking > 16){
    libxsmm_generator_set_p_register_aarch64_sve( io_generated_code,
                                                  LIBXSMM_AARCH64_SVE_REG_P0,
                                                  -1,
...
LIBXSMM_API_INTERN
void libxsmm_generator_store_2dregblock_aarch64_sme_relu( libxsmm_generated_code* io_generated_code,
                                                          const unsigned int      i_gp_reg_addr,
                                                          const unsigned int      i_gp_reg_help,
                                                          const unsigned int      i_m_blocking,
                                                          const unsigned int      i_n_blocking,
  ...
  if( i_m_blocking == 32 && i_n_blocking == 32 && 0 ){
    libxsmm_aarch64_instruction_set_ptrue_as_counter_sve2( io_generated_code,
                                                           LIBXSMM_AARCH64_INSTR_SVE2_PTRUE_AS_COUNTER,
                                                           8 );
    for( int l = 0; l < 2; l++){
    ...
    for( unsigned int l_en = 0; l_en < l_n_block_count ; l_en++){
      ...
      libxsmm_aarch64_instruction_sve_compute( io_generated_code,
                                               ...
                                               LIBXSMM_AARCH64_SVE_REG_Z0,
                                               LIBXSMM_AARCH64_SVE_REG_UNDEF,
                                               LIBXSMM_AARCH64_SVE_TYPE_S );
      for( unsigned int l_em = 0; l_em < 8; l_em++){
        /* just fmax all registers no matter if there it is relevant or not */
        libxsmm_aarch64_instruction_sme_fmax( io_generated_code,
                                              LIBXSMM_AARCH64_INSTR_SME_FMAX_4,
                                              LIBXSMM_AARCH64_SVE_REG_Z0,
                                              l_em*4 );
      }
...
void libxsmm_generator_set_pn_register_aarch64_sve2( libxsmm_generated_code* io_generated_code,
  ...
  if ( i_n_bits < 0 ) {
    ...
    libxsmm_aarch64_instruction_set_predication_as_counter( io_generated_code,
                                                            LIBXSMM_AARCH64_INSTR_SVE2_WHILELT,
                                                            i_p_reg,
                                                            LIBXSMM_AARCH64_GP_REG_XZR,
                                                            i_vl,
                                                            i_gp_reg_scratch,
...
void libxsmm_generator_sme_transpose_64( libxsmm_generated_code * io_generated_code,
                                         ...
                                         unsigned int             i_help_register ){
  /* set w12-15 to 0-3*/
  unsigned int l_tile_count = (i_n_blocking % 16 == 0) ? i_n_blocking/16 : (i_n_blocking/16) + 1;
  unsigned int l_k_phases[2] = {0,0};
  unsigned int l_blocking_phase[2] = {0,0};
  unsigned int l_vec_register_store[8] = {0, 1, 2, 3, 16, 17, 18, 19};
  l_blocking_phase[0] = ( i_n_blocking > 32 ) ? 32 : i_n_blocking;
  l_blocking_phase[1] = ( i_n_blocking > 32 ) ? (i_n_blocking - 32) : 0;
  l_k_phases[0] = (i_k_rest > 0 && i_k_rest < 8 ) ? i_k_rest : 8;
  if( i_k_rest == 0){
    l_k_phases[1] = 8;
  } else if( i_k_rest < 9){
    l_k_phases[1] = 0;
  } else {
    l_k_phases[1] = i_k_rest - 8;
  }

  for( int l_en = 0; l_en < l_tile_count; l_en++){
    libxsmm_aarch64_instruction_alu_move_imm16( io_generated_code,
                                                LIBXSMM_AARCH64_INSTR_GP_MOVZ,
                                                LIBXSMM_AARCH64_GP_REG_W12+l_en,
                                                0,
                                                l_en );
  }
  /* set p register for k blocks*/
  libxsmm_generator_set_p_register_aarch64_sve( io_generated_code,
                                                ...
                                                i_help_register );
  /* load to registers */
  for( unsigned int l_i = 0; l_i < l_blocking_phase[0]; l_i++){
    libxsmm_aarch64_instruction_sve_move( io_generated_code,
                                          LIBXSMM_AARCH64_INSTR_SVE_LD1W_I_OFF,
                                          i_gp_reg_addr,
                                          LIBXSMM_AARCH64_GP_REG_UNDEF,
  ...
  for( unsigned int l_i = 0; l_i < l_blocking_phase[1]; l_i++){
    libxsmm_aarch64_instruction_sve_move( io_generated_code,
                                          LIBXSMM_AARCH64_INSTR_SVE_LD1W_I_OFF,
                                          i_gp_reg_addr,
                                          LIBXSMM_AARCH64_GP_REG_UNDEF,
  ...
  /* mov to register with vertical view */
  for( int i = 0; i < 4; i++){
    libxsmm_aarch64_instruction_sme_mova( io_generated_code,
                                          LIBXSMM_AARCH64_INSTR_SME_MOVA_V_TILE_TO_VECTOR,
                                          i,
  ...
  for( int i = 4; i < 8; i++){
    libxsmm_aarch64_instruction_sme_mova( io_generated_code,
                                          LIBXSMM_AARCH64_INSTR_SME_MOVA_V_TILE_TO_VECTOR,
                                          i,
                                          LIBXSMM_AARCH64_GP_REG_W12+(i-4),
                                          i );
  ...
  for( int i = 0; i < l_k_phases[0]; i++ ){
    libxsmm_aarch64_instruction_sme_mov( io_generated_code,
                                        LIBXSMM_AARCH64_INSTR_SME_ST1W_4,
                                        l_vec_register_store[i], // vec reg
                                        LIBXSMM_AARCH64_GP_REG_XSP, // gp reg
                                        0, // imm
                                        LIBXSMM_AARCH64_SVE_REG_P8 );
    libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code,
                                                  LIBXSMM_AARCH64_INSTR_GP_META_ADD,
...
void libxsmm_generated_load_16x64_aarch64_sme( libxsmm_generated_code* io_generated_code,
  ...
  for( unsigned int l_en = 0; l_en < ((i_n_blocking < 32) ? i_n_blocking : 32); l_en++){
    libxsmm_aarch64_instruction_sve_move( io_generated_code,
                                          LIBXSMM_AARCH64_INSTR_SVE_LD1W_I_OFF,
                                          i_gp_reg_addr,
                                          LIBXSMM_AARCH64_GP_REG_UNDEF,
  ...
  /* load tile 3-4 */
  if( i_n_blocking > 32 ){
    for( unsigned int l_en = 0; l_en < (i_n_blocking-32); l_en++){
      libxsmm_aarch64_instruction_sve_move( io_generated_code,
                                            LIBXSMM_AARCH64_INSTR_SVE_LD1W_I_OFF,
                                            i_gp_reg_addr,
                                            LIBXSMM_AARCH64_GP_REG_UNDEF,
