...
void libxsmm_generator_xoshiro128p_f32_aarch64_asimd( libxsmm_generated_code* io_generated_code,
  ...
  if(io_generated_code->arch == LIBXSMM_AARCH64_A64FX){
    ...
    const libxsmm_aarch64_sve_type l_sve_type = LIBXSMM_AARCH64_SVE_TYPE_S;/* this function is specifically for fp32 */
    const unsigned char            l_pred_reg = 0;/* doesn't matter, not used */

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_ADD_V,
                                               i_vec_reg_rng_state_0, i_vec_reg_rng_state_3, 0, o_vec_reg_rng, l_pred_reg, l_sve_type );

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_LSR_I_V, /* USHR = LSR */
                                             o_vec_reg_rng, LIBXSMM_AARCH64_SVE_REG_UNDEF, 9, o_vec_reg_rng, l_pred_reg, l_sve_type );

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_ORR_V,
                                             o_vec_reg_rng, i_vec_reg_rng_one, 0, o_vec_reg_rng, l_pred_reg, l_sve_type );
    ...
    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_LSL_I_V,
                                             i_vec_reg_rng_state_1, LIBXSMM_AARCH64_SVE_REG_UNDEF, 9, i_vec_reg_rng_tmp_0, l_pred_reg, l_sve_type );

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_EOR_V,
                                             i_vec_reg_rng_state_2, i_vec_reg_rng_state_0, 0, i_vec_reg_rng_state_2, l_pred_reg, l_sve_type );

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_EOR_V,
                                             i_vec_reg_rng_state_3, i_vec_reg_rng_state_1, 0, i_vec_reg_rng_state_3, l_pred_reg, l_sve_type );

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_EOR_V,
                                             i_vec_reg_rng_state_1, i_vec_reg_rng_state_2, 0, i_vec_reg_rng_state_1, l_pred_reg, l_sve_type );

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_EOR_V,
                                             i_vec_reg_rng_state_0, i_vec_reg_rng_state_3, 0, i_vec_reg_rng_state_0, l_pred_reg, l_sve_type );

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_EOR_V,
                                             i_vec_reg_rng_state_2, i_vec_reg_rng_tmp_0, 0, i_vec_reg_rng_state_2, l_pred_reg, l_sve_type );

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_LSL_I_V,
                                             i_vec_reg_rng_state_3, LIBXSMM_AARCH64_SVE_REG_UNDEF, 11, i_vec_reg_rng_tmp_0, l_pred_reg, l_sve_type );

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_LSR_I_V, /* USHR = LSR */
                                             i_vec_reg_rng_state_3, LIBXSMM_AARCH64_SVE_REG_UNDEF, 21, i_vec_reg_rng_tmp_1, l_pred_reg, l_sve_type );

    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_ORR_V,
                                             i_vec_reg_rng_tmp_0, i_vec_reg_rng_tmp_1, 0, i_vec_reg_rng_state_3, l_pred_reg, l_sve_type );
  } else {
    libxsmm_aarch64_instruction_asimd_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_ASIMD_ADD_V,
...
void libxsmm_generator_prepare_coeffs_gelu_ps_minimax3_aarch64( libxsmm_generated_code*               io_generated_code,
  ...
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_thres,   i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, thres_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_absmask, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, absmask_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_scale,   i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, scale_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_shifter, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, shifter_array  );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_half,    i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, half_array );

  if(io_generated_code->arch == LIBXSMM_AARCH64_A64FX){
    int l_pred_tmp = 7;/* pred_tmp is set to -1; this code here will be replaced by load64bytes_const_to_vec probably anyways */
    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c0,  i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c0_array, 0 );
    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c01, i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c0_array, 2 );
    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c02, i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c0_array, 4 );
    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c03, i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c0_array, 6 );

    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c1,  i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c1_array, 0 );
    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c11, i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c1_array, 2 );
    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c12, i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c1_array, 4 );
    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c13, i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c1_array, 6 );

    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c2,  i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c2_array, 0 );
    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c21, i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c2_array, 2 );
    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c22, i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c2_array, 4 );
    libxsmm_aarch64_instruction_sve_rep16bytes_const_to_vec( io_generated_code, i_vec_c23, i_gp_reg_tmp, i_gp_reg_tmp1, l_pred_tmp, c2_array, 6 );
  } else {
  ...
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_tmp1, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, 0x03020100 );
}
...
void libxsmm_generator_gelu_ps_minimax3_aarch64( libxsmm_generated_code*               io_generated_code,
  ...
  if(io_generated_code->arch == LIBXSMM_AARCH64_A64FX){
    ...
    */
    ...
    /* todo replace TBL_4 by TBL_1 (because 128 bits ASIMD * 4 = 512 bits SVE), and modify the loading function */
...
void libxsmm_generator_prepare_coeffs_exp_ps_3dts_aarch64( libxsmm_generated_code*                        io_generated_code,
  ...
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_halves, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, vec_halves );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_log2e, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, vec_log2e );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c0, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, vec_c0 );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c1, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, vec_c1  );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c2, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, vec_c2 );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c3, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, vec_c3 );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_expmask, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, vec_expmask );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_hi_bound, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, vec_hi_bound );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_lo_bound, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, vec_lo_bound );
}
...
void libxsmm_generator_exp_ps_3dts_aarch64( libxsmm_generated_code*               io_generated_code,
  ...
  libxsmm_generator_scalefps_aarch64( io_generated_code, i_vec_z, i_vec_x, i_vec_x, i_vec_expmask, i_tupletype, i_sve_type, i_pred_reg );
}
...
void libxsmm_generator_prepare_coeffs_tanh_ps_rational_78_aarch64(  libxsmm_generated_code*                        io_generated_code,
  ...
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c0, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, c0_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c1, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, c1_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c2, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, c2_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c3, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, c3_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c1_d, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, c1_d_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c2_d, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, c2_d_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_c3_d, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, c3_d_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_hi_bound, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, hi_b_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_lo_bound, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, lo_b_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_ones, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, ones_array );
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_neg_ones, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, neg_ones_array );
}
...
void libxsmm_generator_prepare_coeffs_sigmoid_ps_rational_78_aarch64( libxsmm_generated_code*                        io_generated_code,
  ...
  unsigned long long vec_halves = 0x3f000000 ;
  libxsmm_aarch64_instruction_broadcast_scalar_to_vec ( io_generated_code, i_vec_halves, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg, vec_halves );
  libxsmm_generator_prepare_coeffs_tanh_ps_rational_78_aarch64(
        io_generated_code, i_vec_c0, i_vec_c1, i_vec_c2, i_vec_c3, i_vec_c1_d, i_vec_c2_d, i_vec_c3_d,
        i_vec_hi_bound, i_vec_lo_bound, i_vec_ones, i_vec_neg_ones, i_gp_reg_tmp, i_tupletype, i_sve_type, i_pred_reg );
}
