...
LIBXSMM_API_INTERN void libxsmm_generator_gemm_sse_avx_avx2_avx512_kloop( libxsmm_generated_code*            io_generated_code,
  ...
  if ( ( io_generated_code->arch == LIBXSMM_X86_AVX512_VL256_CLX) && ( i_m_blocking <= 16) && ( LIBXSMM_GEMM_PRECISION_F64 != LIBXSMM_GETENUM_INP( i_xgemm_desc->datatype)) ) {
    l_k_blocking = 2;
    l_k_threshold = 12;
  } else
...
LIBXSMM_API_INTERN unsigned int libxsmm_generator_gemm_sse_avx_avx2_avx512_get_initial_m_blocking( libxsmm_micro_kernel_config*    io_micro_kernel_config,
  ...
  } else if ( ( i_arch <= LIBXSMM_X86_AVX512_VL256 ) && ( ( LIBXSMM_GEMM_PRECISION_I8  == LIBXSMM_GETENUM_INP( i_xgemm_desc->datatype ) )  ||
