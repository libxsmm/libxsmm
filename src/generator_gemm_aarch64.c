...
void libxsmm_generator_gemm_aarch64_microkernel_sve_a64fx( libxsmm_generated_code*            io_generated_code,
  ...
  unsigned int l_datatype_size_in = 0;
  ...
  if ( (LIBXSMM_DATATYPE_F32 == LIBXSMM_GETENUM_INP( i_xgemm_desc->datatype ) && LIBXSMM_DATATYPE_F32 == LIBXSMM_GETENUM_OUT( i_xgemm_desc->datatype )) ||
    ...
    l_datatype_size_in = i_micro_kernel_config->datatype_size_in;
    ...
    l_datatype_size_in = 4; /* BFDOT works on BF16 tuples */
  } else {
    LIBXSMM_HANDLE_ERROR( io_generated_code, LIBXSMM_ERR_UNSUP_DATATYPE );
    return;
  }
  ...
  for ( l_m = 0; l_m < l_m_blocks[0]; l_m++ ) {
    ...
    libxsmm_aarch64_instruction_alu_compute_imm12( io_generated_code,
                                                   ...
                                                   i_gp_reg_mapping->gp_reg_a,
                                                   i_gp_reg_mapping->gp_reg_a,
                                                   i_micro_kernel_config->vector_length * l_datatype_size_in,
                                                   0 );
  }
  /* remainder load on a */
  if ( l_m_blocks[1] > 0) {
    ...
    libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code,
                                                   ...
                                                   i_gp_reg_mapping->gp_reg_help_0,
                                                   i_gp_reg_mapping->gp_reg_a,
                                                   (long long)l_remainder_size * l_datatype_size_in );
  }
