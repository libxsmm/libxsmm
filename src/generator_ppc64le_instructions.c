...
char libxsmm_ppc64le_isfree_reg( libxsmm_generated_code  *io_generated_code,
  ...
  switch (i_reg_type) {
    case LIBXSMM_PPC64LE_GPR: {
      if ( ( io_reg_tracker->gpr[i_reg] >= LIBXSMM_PPC64LE_REG_FREE ) ) {
        o_is_free = 1;
      }
    } break;
    case LIBXSMM_PPC64LE_FPR: {
      if ( ( io_reg_tracker->fpr[i_reg] >= LIBXSMM_PPC64LE_REG_FREE ) &&
           ( io_reg_tracker->vsr[i_reg] >= LIBXSMM_PPC64LE_REG_FREE )  &&
           ( io_reg_tracker->acc[i_reg / 4] >= LIBXSMM_PPC64LE_REG_FREE ) ) {
        o_is_free = 1;
      }
    } break;
    case LIBXSMM_PPC64LE_VR: {
      if ( ( io_reg_tracker->vr[i_reg] >= LIBXSMM_PPC64LE_REG_FREE ) &&
           ( io_reg_tracker->vsr[i_reg + LIBXSMM_PPC64LE_FPR_NMAX] >= LIBXSMM_PPC64LE_REG_FREE ) ) {
        o_is_free = 1;
      }
    } break;
    case LIBXSMM_PPC64LE_VSR: {
      ...
      } else if ( io_reg_tracker->vsr[i_reg] >= LIBXSMM_PPC64LE_REG_FREE ) {
        o_is_free = 1;
      }
    } break;
    case LIBXSMM_PPC64LE_ACC: {
      if ( io_reg_tracker->acc[i_reg] >= LIBXSMM_PPC64LE_REG_FREE ) {
        int i;
        o_is_free = 1;
        for ( i = 4; i < 4; ++i ) {
          o_is_free &= ( io_reg_tracker->fpr[i_reg*4 + i] >= LIBXSMM_PPC64LE_REG_FREE );
          o_is_free &= ( io_reg_tracker->vsr[i_reg*4 + i] >= LIBXSMM_PPC64LE_REG_FREE );
        }
      }
    } break;
    default: {
...
void libxsmm_ppc64le_get_sequential_reg( libxsmm_generated_code  *io_generated_code,
  ...
  i = 0;
  l_alloc = 0;
  while ( i <= (int)(l_n_reg - i_n) && !l_alloc ) {
    char  l_search = 1;
    for ( j = 0; j < (int)i_n && l_search; ++j ) {
...
void libxsmm_ppc64le_set_reg( libxsmm_generated_code  *io_generated_code,
  ...
  switch ( i_reg_type ) {
    ...
    case LIBXSMM_PPC64LE_FPR: {
      ...
      io_reg_tracker->acc[i_reg / 4] = i_value;
    } break;
    case LIBXSMM_PPC64LE_VR: {
...
void libxsmm_ppc64le_instr_transpose_f32_4x4( libxsmm_generated_code *io_generated_code,
                                              ...
                                              unsigned int           *i_v,
                                              unsigned int           *o_v ) {
#define LIBXSMM_PPC64LE_NSCRATCH 4
  static unsigned int i, l_scratch[LIBXSMM_PPC64LE_NSCRATCH];
  for ( i = 0; i < LIBXSMM_PPC64LE_NSCRATCH ; ++i ) {
...
#undef LIBXSMM_PPC64LE_NSCRATCH
}
...
void libxsmm_ppc64le_instr_transpose_f32_3x4( libxsmm_generated_code *io_generated_code,
                                              ...
                                              unsigned int           *o_v ) {
#define LIBXSMM_PPC64LE_NSCRATCH 4
  unsigned int i, l_scratch[LIBXSMM_PPC64LE_NSCRATCH];
  for ( i = 0; i < LIBXSMM_PPC64LE_NSCRATCH ; ++i ) {
    l_scratch[i] = libxsmm_ppc64le_get_reg( io_generated_code, io_reg_tracker, LIBXSMM_PPC64LE_VSR );
  }
...
void libxsmm_ppc64le_instr_transpose_f32_2x4( libxsmm_generated_code *io_generated_code,
                                              ...
                                              unsigned int           *i_v,
                                              unsigned int           *o_v ) {
#define LIBXSMM_PPC64LE_NSCRATCH 2
  unsigned int i, l_scratch[LIBXSMM_PPC64LE_NSCRATCH];
  for ( i = 0; i < LIBXSMM_PPC64LE_NSCRATCH ; ++i ) {
    l_scratch[i] = libxsmm_ppc64le_get_reg( io_generated_code, io_reg_tracker, LIBXSMM_PPC64LE_VSR );
  }
...
void libxsmm_ppc64le_instr_set_imm64( libxsmm_generated_code *io_generated_code,
  ...
  if ( LIBXSMM_PPC64LE_VSX == io_generated_code->arch ) {
    ...
    unsigned int l_h0 = (unsigned int)( ( 0xffff & ( i_val >> 48 ) ) + ( 0x01 & ( l_h1 >> 15 ) ) );
    unsigned int l_reg = 0;

    if ( 0 != l_h0 ) {
      libxsmm_ppc64le_instr_3( io_generated_code, LIBXSMM_PPC64LE_INSTR_ADDIS, i_dst, l_reg, l_h0 );
      l_reg = i_dst;
    }

    if ( 0 != l_h1 ) {
      libxsmm_ppc64le_instr_3( io_generated_code, LIBXSMM_PPC64LE_INSTR_ADDI, i_dst, l_reg, l_h1 );
    }
    ...
    if ( 0 != l_h2 ) {
      libxsmm_ppc64le_instr_3( io_generated_code, LIBXSMM_PPC64LE_INSTR_ADDIS, i_dst, l_reg, l_h2 );
      l_reg = i_dst;
    }

    if ( 0 != l_h3 ) {
      libxsmm_ppc64le_instr_3( io_generated_code, LIBXSMM_PPC64LE_INSTR_ADDI, i_dst, l_reg, l_h3 );
      l_reg = i_dst;
    }
  } else if ( LIBXSMM_PPC64LE_MMA <= io_generated_code->arch ) {
    unsigned int l_reg = 0;
    unsigned int l_w1 = (unsigned int)( 0xffffffff & i_val );
    unsigned int l_w0 = (unsigned int)( 0xffffffff & ( i_val >> 32 ) );
    if ( 0 != l_w0 ) {
      libxsmm_ppc64le_instr_add_value( io_generated_code, io_reg_tracker, l_reg, i_dst, l_w0 );
      libxsmm_ppc64le_instr_set_shift_left( io_generated_code, i_dst, i_dst, 32 );
      l_reg = i_dst;
    }
    if ( 0 != l_w1 ) {
      libxsmm_ppc64le_instr_add_value( io_generated_code, io_reg_tracker, l_reg, i_dst, l_w1 );
    }
  }
}
...
void libxsmm_ppc64le_instr_load_splat( libxsmm_generated_code *io_generated_code,
  ...
  if ( i_offset != 0 ) {
    ...
    libxsmm_ppc64le_instr_add_value( io_generated_code, io_reg_tracker, 0, l_offset, i_offset );
  } else {
    l_offset = 0;
  }
