...
void libxsmm_generator_gemm_amx_paired_tilestore( libxsmm_generated_code*            io_generated_code,
  ...
  for (col = 0; col < (unsigned int)n_cols; col++) {
    ...
    if (eager_result_store == 1) {
      ...
      int l_displacement = ( libxsmm_is_runtime_set_ldc_gemm( i_xgemm_desc ) != 0 ) ?  im_offset * out_tsize : ((in_offset+col) * i_xgemm_desc->ldc + im_offset) * out_tsize;
      int l_idx_register = ( libxsmm_is_runtime_set_ldc_gemm( i_xgemm_desc ) != 0 ) ? i_gp_reg_mapping->gp_reg_ldc : LIBXSMM_X86_GP_REG_UNDEF;
      int l_scale = ( libxsmm_is_runtime_set_ldc_gemm( i_xgemm_desc ) != 0 ) ? out_tsize : 0;

      if ( libxsmm_is_runtime_set_ldc_gemm( i_xgemm_desc ) != 0 ) {
        libxsmm_generator_gemm_getval_stack_var( io_generated_code, i_micro_kernel_config, LIBXSMM_GEMM_STACK_VAR_LDC_VAL, i_gp_reg_mapping->gp_reg_ldc );
        libxsmm_x86_instruction_alu_imm( io_generated_code, LIBXSMM_X86_INSTR_IMUL, i_gp_reg_mapping->gp_reg_ldc, in_offset+col);
      }
...
void libxsmm_generator_gemm_amx_single_tilestore( libxsmm_generated_code*            io_generated_code,
  ...
  /* Mixed A/B FP8: AB_COMMON_PREC is UNSUPPORTED so the pure-flavor F32 store gate below misses it;
   * treat it like the pure BF8/HF8 case so C=F32 uses a direct TILESTORED (not the fp8 downconvert). */
  ...
  if ((LIBXSMM_DATATYPE_F32 == LIBXSMM_GEMM_GETENUM_C_PREC( i_xgemm_desc->datatype ) &&
    ...
    unsigned long long store_offset = (in_offset * i_xgemm_desc->ldc + im_offset) * 4;
    if ( libxsmm_is_runtime_set_ldc_gemm( i_xgemm_desc ) != 0 ) {
      libxsmm_x86_instruction_push_reg( io_generated_code, i_gp_reg_mapping->gp_reg_help_0 );
      libxsmm_generator_gemm_getval_stack_var( io_generated_code, i_micro_kernel_config, LIBXSMM_GEMM_STACK_VAR_LDC_VAL, i_gp_reg_mapping->gp_reg_ldc );
      libxsmm_x86_instruction_alu_reg( io_generated_code, i_micro_kernel_config->alu_mov_instruction, i_gp_reg_mapping->gp_reg_ldc, i_gp_reg_mapping->gp_reg_help_0);
      libxsmm_x86_instruction_alu_imm( io_generated_code, LIBXSMM_X86_INSTR_IMUL, i_gp_reg_mapping->gp_reg_help_0, in_offset * 4);
    ...
    libxsmm_x86_instruction_tile_move( io_generated_code,
        ...
        tile);
    if ( libxsmm_is_runtime_set_ldc_gemm( i_xgemm_desc ) != 0 ) {
      libxsmm_x86_instruction_alu_reg( io_generated_code, LIBXSMM_X86_INSTR_SUBQ, i_gp_reg_mapping->gp_reg_help_0, i_gp_reg_mapping->gp_reg_c );
      libxsmm_x86_instruction_pop_reg( io_generated_code, i_gp_reg_mapping->gp_reg_help_0 );
    }
      ...
      if (fused_eltwise == 1) {
        ...
        if (i_micro_kernel_config->vnni_format_C == 0) {
          for (col = 0; col < (unsigned int)n_cols; col++) {
            ...
            long long l_displacement = ( libxsmm_is_runtime_set_ldc_gemm( i_xgemm_desc ) != 0 ) ?  (long long)im_offset * LIBXSMM_TYPESIZE((libxsmm_datatype)LIBXSMM_GEMM_GETENUM_C_PREC( i_xgemm_desc->datatype )) : (long long)((in_offset+col) * i_xgemm_desc->ldc + im_offset) * LIBXSMM_TYPESIZE((libxsmm_datatype)LIBXSMM_GEMM_GETENUM_C_PREC( i_xgemm_desc->datatype ));
            int l_idx_register = ( libxsmm_is_runtime_set_ldc_gemm( i_xgemm_desc ) != 0 ) ? i_gp_reg_mapping->gp_reg_ldc : LIBXSMM_X86_GP_REG_UNDEF;
            int l_scale = ( libxsmm_is_runtime_set_ldc_gemm( i_xgemm_desc ) != 0 ) ? LIBXSMM_TYPESIZE((libxsmm_datatype)LIBXSMM_GEMM_GETENUM_C_PREC( i_xgemm_desc->datatype )) : 0;
...
void libxsmm_generator_gemm_amx_decompress_Kx32_A_block_kloop(libxsmm_generated_code*     io_generated_code,
  ...
  libxsmm_x86_instruction_alu_imm( io_generated_code, LIBXSMM_X86_INSTR_ADDQ, i_gp_reg_mapping->gp_reg_bitmap_a, ((long long)i_k_CL*32)/8);
...
void libxsmm_generator_gemm_amx_microkernel( libxsmm_generated_code*            io_generated_code,
  ...
  unsigned int l_gpr_dyn_aux = i_gp_reg_mapping->gp_reg_help_0;
  ...
  if (LIBXSMM_DATATYPE_BF16 == LIBXSMM_GEMM_GETENUM_AB_COMMON_PREC( i_xgemm_desc->datatype ) || l_is_Abf8_Bbf16_gemm > 0 || l_is_Ahf8_Bbf16_gemm > 0 || l_is_Amxfp4_Bbf16_gemm > 0) {
    ...
    /* mixed A/B FP8 flavors (BF8=E5M2, HF8=E4M3). The tile-compute encoder maps tsrc1(VEX.vvvv)=A
     * and tsrc2(ModRM.rm)=B; the DMR opcode's two flavor letters are ordered (tsrc2, tsrc1), i.e.
     * TDPBHF8PS = tsrc1(A)=HF8, tsrc2(B)=BF8. So A=BF8,B=HF8 -> TDPHBF8PS and A=HF8,B=BF8 -> TDPBHF8PS. */
...

  {
    ...
    if (((i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_A) == 0)  && ((i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_B) == 0)) {
      ...
      if ( libxsmm_is_runtime_set_ldb_gemm( i_xgemm_desc ) != 0 ) {
        libxsmm_x86_instruction_push_reg( io_generated_code, l_gpr_dyn_aux );
        for (i = 0; i < 4; i++) {
          _B_offsets[i] = offset_B;
        }
      }
    }
  }
  ...
  for (i = 0; i < m_tiles*n_tiles; i++) {
    ...
    if (_B_tile_id_load[i] > 0) {
      unsigned int l_gp_reg_b = i_gp_reg_mapping->gp_reg_b;
      /* TODO: catch int overdlow in B_offset */
      if ( libxsmm_is_runtime_set_ldb_gemm( i_xgemm_desc ) != 0 ) {
        if (_in_offset_prefix_sums[_in_tileloads_B[i]] == 0) {
          l_gp_reg_b = i_gp_reg_mapping->gp_reg_b;
        } else {
          libxsmm_generator_gemm_getval_stack_var( io_generated_code, i_micro_kernel_config,
                                                   LIBXSMM_GEMM_STACK_VAR_LDB_VAL, l_gpr_dyn_aux );
          libxsmm_x86_instruction_alu_imm( io_generated_code, LIBXSMM_X86_INSTR_IMUL, l_gpr_dyn_aux,
                                           ((long long)_in_offset_prefix_sums[_in_tileloads_B[i]] * i_micro_kernel_config->datatype_size_in ));
          libxsmm_x86_instruction_alu_reg( io_generated_code, LIBXSMM_X86_INSTR_ADDQ, i_gp_reg_mapping->gp_reg_b,
                                           l_gpr_dyn_aux );
          l_gp_reg_b = l_gpr_dyn_aux;
        }
      }
  ...
  if ( libxsmm_is_runtime_set_ldb_gemm( i_xgemm_desc ) != 0 ) {
    libxsmm_x86_instruction_pop_reg( io_generated_code, l_gpr_dyn_aux );
  }
}
...
void libxsmm_generator_gemm_amx_kernel_kloop( libxsmm_generated_code*            io_generated_code,
  ...
  if (LIBXSMM_DATATYPE_BF16 == LIBXSMM_GEMM_GETENUM_AB_COMMON_PREC( i_xgemm_desc->datatype ) || LIBXSMM_DATATYPE_F16 == LIBXSMM_GEMM_GETENUM_AB_COMMON_PREC( i_xgemm_desc->datatype ) || l_is_Amxfp4_Bbf16_gemm > 0 || l_is_Abf8_Bbf16_gemm > 0 || l_is_Abf8_Bf16_gemm > 0 || l_is_Ahf8_Bbf16_gemm > 0) {
    l_k_blocking = 32;
             ...
             l_is_Abyte_Bi8_gemm > 0 || l_is_mixed_fp8 > 0) {
    l_k_blocking = 64;
  }
  ...
  if (((i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_DECOMPRESS_A_VIA_BITMASK) > 0) && (i_xgemm_desc->k > 1024) && ((i_xgemm_desc->k/l_k_blocking) % 2 == 0)) {
    ...
    if (((l_k_trips * l_brgemm_unroll_factor) > l_unroll_kernel_limit) && ((i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_DECOMPRESS_A_VIA_BITMASK) == 0)) {
      ...
      /* K loop here */
      is_last_k = 0;
      if ( libxsmm_is_runtime_set_lda_gemm( i_xgemm_desc ) != 0 && i_micro_kernel_config->avnni_gemm_sw_pipeline == 0) {
      ...
      for (k = 0; k < l_k_unroll_limit * l_k_blocking; k+= l_k_blocking) {
        ...
        if ((i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_A) > 0) {
          ...
          if ( libxsmm_is_runtime_set_lda_gemm( i_xgemm_desc ) != 0 && i_micro_kernel_config->avnni_gemm_sw_pipeline == 0) {
            offset_A = A_offs;
            if (k == 0) {
              libxsmm_generator_gemm_getval_stack_var( io_generated_code, i_micro_kernel_config, LIBXSMM_GEMM_STACK_VAR_LDA_VAL, i_gp_reg_mapping->gp_reg_mloop );
              libxsmm_x86_instruction_alu_imm( io_generated_code, LIBXSMM_X86_INSTR_IMUL, i_gp_reg_mapping->gp_reg_mloop, ((long long)l_k_blocking*l_a_dtype_size) );
            }
          } else {
        ...
        if ( libxsmm_is_runtime_set_lda_gemm( i_xgemm_desc ) != 0  && i_micro_kernel_config->avnni_gemm_sw_pipeline == 0) {
          libxsmm_x86_instruction_alu_reg( io_generated_code, LIBXSMM_X86_INSTR_ADDQ, i_gp_reg_mapping->gp_reg_mloop, i_gp_reg_mapping->gp_reg_a );
        }
      }
      if ( libxsmm_is_runtime_set_lda_gemm( i_xgemm_desc ) == 0 || i_micro_kernel_config->avnni_gemm_sw_pipeline > 0 ) {
      ...
      if ( libxsmm_is_runtime_set_lda_gemm( i_xgemm_desc ) != 0 && l_k_trips > 1 && i_micro_kernel_config->avnni_gemm_sw_pipeline == 0) {
      ...
      for (k = 0; k < i_xgemm_desc->k; k+= l_k_blocking) {
        ...
        if ((i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_A) > 0) {
          ...
          if ( libxsmm_is_runtime_set_lda_gemm( i_xgemm_desc ) != 0 && i_micro_kernel_config->avnni_gemm_sw_pipeline == 0) {
            offset_A = A_offs;
            if (k == 0 && l_k_trips > 1) {
              libxsmm_generator_gemm_getval_stack_var( io_generated_code, i_micro_kernel_config, LIBXSMM_GEMM_STACK_VAR_LDA_VAL, i_gp_reg_mapping->gp_reg_nloop );
              libxsmm_x86_instruction_alu_imm( io_generated_code, LIBXSMM_X86_INSTR_IMUL, i_gp_reg_mapping->gp_reg_nloop, ((long long)l_k_blocking*l_a_dtype_size) );
            }
          } else {
            offset_A = (k * l_lda * l_a_dtype_size)/i_micro_kernel_config->sparsity_factor_A + A_offs;
          }
        ...
        libxsmm_generator_gemm_amx_microkernel(io_generated_code, io_loop_label_tracker, i_gp_reg_mapping, i_micro_kernel_config, i_xgemm_desc, n_blocking_info, m_blocking_info, offset_A, offset_B, is_last_k, i_brgemm_loop, fully_unrolled_brloop);
        if ( libxsmm_is_runtime_set_lda_gemm( i_xgemm_desc ) != 0 && l_k_trips > 1 && i_micro_kernel_config->avnni_gemm_sw_pipeline == 0) {
          libxsmm_x86_instruction_alu_reg( io_generated_code, LIBXSMM_X86_INSTR_ADDQ, i_gp_reg_mapping->gp_reg_nloop, i_gp_reg_mapping->gp_reg_a );
        }
      }
