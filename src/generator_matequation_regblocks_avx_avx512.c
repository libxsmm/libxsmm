  ...
  if (cur_node->type == LIBXSMM_MATRIX_EQN_NODE_ARG) {
    ...
    if (cur_node->info.u_op.type == LIBXSMM_MELTW_TYPE_UNARY_DUMP) {
      ...
      libxsmm_x86_instruction_alu_mem( io_generated_code,
          ...
          cur_node->info.u_op.op_arg_pos*((int)sizeof(libxsmm_matrix_op_arg)),
          temp_reg,
          0 );
      libxsmm_generator_meqn_getaddr_stack_tmpaddr_i( io_generated_code, n_args * 8 + cur_pos * ((int)sizeof(libxsmm_matrix_op_arg)), temp_reg2);
      libxsmm_x86_instruction_alu_mem( io_generated_code,
          i_micro_kernel_config->alu_mov_instruction,
          temp_reg2,
          LIBXSMM_X86_GP_REG_UNDEF, 0,
    ...
    if (cur_node->info.arg.in_pos >= 0) {
      if (cur_pos < i_micro_kernel_config->n_avail_gpr) {
        libxsmm_x86_instruction_alu_mem( io_generated_code,
            ...
            input_reg,
            LIBXSMM_X86_GP_REG_UNDEF, 0,
            cur_node->info.arg.in_pos*((int)sizeof(libxsmm_matrix_arg)),
...
void libxsmm_generator_mateqn_store_2d_reg_block( libxsmm_generated_code*          io_generated_code,
  ...
  if ( LIBXSMM_MEQN_GETENUM_OUT_PREC(i_meqn_desc->datatype) == LIBXSMM_DATATYPE_BF16 || LIBXSMM_MEQN_GETENUM_OUT_PREC(i_meqn_desc->datatype) == LIBXSMM_DATATYPE_F16 ) {
    if (io_generated_code->arch < LIBXSMM_X86_AVX512_VL256_SKX) {
      if (i_mask_last_m_chunk > 0) {
  ...
  for (in = 0; in < i_n_blocking; in++) {
    for (im = 0; im < i_m_blocking; im++) {
      unsigned int vmove_instr = (((LIBXSMM_DATATYPE_BF8 == ((libxsmm_datatype)LIBXSMM_MEQN_GETENUM_OUT_PREC(i_meqn_desc->datatype))) || (LIBXSMM_DATATYPE_HF8 == ((libxsmm_datatype)LIBXSMM_MEQN_GETENUM_OUT_PREC(i_meqn_desc->datatype)))) && (io_generated_code->arch < LIBXSMM_X86_AVX512_SKX) && !((i_mask_last_m_chunk == 1) && (im == i_m_blocking - 1)) ) ? LIBXSMM_X86_INSTR_VMOVSD : libxsmm_generator_matequation_regblocks_vmove_instruction((libxsmm_datatype)LIBXSMM_MEQN_GETENUM_OUT_PREC(i_meqn_desc->datatype));
      cur_vreg = i_start_vreg + in * i_m_blocking + im;
      ...
      libxsmm_x86_instruction_unified_vec_move( io_generated_code,
          ...
          LIBXSMM_X86_GP_REG_UNDEF, 0,
          (im * i_vlen + in * i_meqn_desc->ldo) * LIBXSMM_TYPESIZE(LIBXSMM_MEQN_GETENUM_OUT_PREC(i_meqn_desc->datatype)),
...
void libxsmm_generator_mateqn_store_reduce_to_scalar_output( libxsmm_generated_code*          io_generated_code,
  ...
  libxsmm_x86_instruction_unified_vec_move( io_generated_code,
      libxsmm_generator_matequation_regblocks_vmove_instruction((libxsmm_datatype)LIBXSMM_MEQN_GETENUM_OUT_PREC(i_meqn_desc->datatype)),
      i_gp_reg_mapping->gp_reg_out,
      LIBXSMM_X86_GP_REG_UNDEF, 0,
      0,
      ((i_micro_kernel_config->cvt_result_to_bf8 == 1) || (i_micro_kernel_config->cvt_result_to_hf8 == 1) || ((i_micro_kernel_config->cvt_result_to_bf16 == 1 || i_micro_kernel_config->cvt_result_to_f16 == 1 ) && (io_generated_code->arch < LIBXSMM_X86_AVX512_SKX)) ) ? 'x' : ((i_micro_kernel_config->cvt_result_to_bf16 == 1 || i_micro_kernel_config->cvt_result_to_f16 == 1 ) || (io_generated_code->arch < LIBXSMM_X86_AVX512_SKX)) ? 'y' : 'z',
...
void libxsmm_configure_reserved_zmms_and_masks(libxsmm_generated_code* io_generated_code,
  ...
  if ((eqn_root_dtype == LIBXSMM_DATATYPE_F32) && (LIBXSMM_MEQN_GETENUM_OUT_PREC(i_mateqn_desc->datatype) == LIBXSMM_DATATYPE_F16)) {
    i_micro_kernel_config->cvt_result_to_f16 = 1;
  } else {
    i_micro_kernel_config->cvt_result_to_f16 = 0;
  }

  if ((eqn_root_dtype == LIBXSMM_DATATYPE_F32) && (LIBXSMM_MEQN_GETENUM_OUT_PREC(i_mateqn_desc->datatype) == LIBXSMM_DATATYPE_BF8)) {
  ...
  if ((has_hf8_input > 0) || ((eqn_root_dtype == LIBXSMM_DATATYPE_F32) && (LIBXSMM_MEQN_GETENUM_OUT_PREC(i_mateqn_desc->datatype) == LIBXSMM_DATATYPE_HF8)) ) {
  ...
  if ((eqn_root_dtype == LIBXSMM_DATATYPE_F32) && (LIBXSMM_MEQN_GETENUM_OUT_PREC(i_mateqn_desc->datatype) == LIBXSMM_DATATYPE_HF8)) {
