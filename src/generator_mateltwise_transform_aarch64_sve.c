...
LIBXSMM_API_INTERN
void libxsmm_generator_transform_Xway_unpack_network_aarch64_sve( libxsmm_generated_code* io_generated_code,
                                                                    ...
                                                                    const libxsmm_aarch64_sve_type i_tupletype ) {
  unsigned int l_i = 0;
  ...
  for ( l_i = 0 ; l_i < i_ways ; ++l_i ) {
    ...
    libxsmm_aarch64_instruction_sve_compute( io_generated_code, l_shuffle_instr,
                                               in0, in1, 0, dst, i_tupletype );
  }
}

LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_to_normt_64bit_4x4_shufflenetwork_aarch64_sve( libxsmm_generated_code*                 io_generated_code,
  ...
  /* load 8 vectors */
  libxsmm_generator_load_2dregblock_aarch64_sve( io_generated_code, i_gp_reg_in, i_gp_reg_scratch, 2, 32, 4, 4, i_mateltwise_desc->ldi * i_micro_kernel_config->datatype_size_in, 0 );
  ...
  /* shuffle */
  {
    ...
    libxsmm_generator_transform_Xway_unpack_network_aarch64_sve( io_generated_code, l_in_idx, l_out_idx, l_src_start, l_dst_start, 2,
                                                                   LIBXSMM_AARCH64_INSTR_sve_TRN1, LIBXSMM_AARCH64_INSTR_sve_TRN2, 8, LIBXSMM_AARCH64_sve_TUPLETYPE_2D );
  }

  /* store 8 vectors */
  libxsmm_generator_store_2dregblock_aarch64_sve( io_generated_code, i_gp_reg_out, i_gp_reg_scratch, 2, 24, 4, 4, i_mateltwise_desc->ldo * i_micro_kernel_config->datatype_size_out );
}

LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_to_normt_32bit_4x4_shufflenetwork_aarch64_sve( libxsmm_generated_code*                 io_generated_code,
                                                                                       const unsigned int                      i_gp_reg_in,
                                                                                       const unsigned int                      i_gp_reg_out,
                                                                                       const unsigned int                      i_gp_reg_scratch,
                                                                                       const unsigned int                      i_m_valid,
  ...
  /* load 8 vectors */
  libxsmm_generator_load_2dregblock_aarch64_sve( io_generated_code, i_gp_reg_in, i_gp_reg_scratch, 4, 32, 4, 4, i_mateltwise_desc->ldi * i_micro_kernel_config->datatype_size_in, 0 );
  ...
  /* shuffle 64 bit */
  {
    ...
    libxsmm_generator_transform_Xway_unpack_network_aarch64_sve( io_generated_code, l_in_idx, l_out_idx, l_src_start, l_dst_start, 2,
                                                                   LIBXSMM_AARCH64_INSTR_sve_TRN1, LIBXSMM_AARCH64_INSTR_sve_TRN2, 4, LIBXSMM_AARCH64_sve_TUPLETYPE_2D );
  }
  ...
  /* shuffle 32 bit */
  {
    ...
    libxsmm_generator_transform_Xway_unpack_network_aarch64_sve( io_generated_code, l_in_idx, l_out_idx, l_src_start, l_dst_start, 2,
                                                                   LIBXSMM_AARCH64_INSTR_sve_TRN1, LIBXSMM_AARCH64_INSTR_sve_TRN2, 4, LIBXSMM_AARCH64_sve_TUPLETYPE_4S );
  }

  /* store 8 vectors */
  libxsmm_generator_store_2dregblock_aarch64_sve( io_generated_code, i_gp_reg_out, i_gp_reg_scratch, 4, 32, 4, 4, i_mateltwise_desc->ldo * i_micro_kernel_config->datatype_size_out );
}

LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_to_normt_32bit_8x8_shufflenetwork_aarch64_sve( libxsmm_generated_code*                 io_generated_code,
                                                                                       const unsigned int                      i_gp_reg_in,
                                                                                       const unsigned int                      i_gp_reg_out,
                                                                                       const unsigned int                      i_gp_reg_scratch,
                                                                                       const unsigned int                      i_m_valid,
  ...
  /* load 8 vectors */
  libxsmm_generator_load_2dregblock_aarch64_sve( io_generated_code, i_gp_reg_in, i_gp_reg_scratch, 4, 32, 8, 8, i_mateltwise_desc->ldi * i_micro_kernel_config->datatype_size_in, 0 );
  ...
  /* shuffle 64 bit */
  {
    ...
    libxsmm_generator_transform_Xway_unpack_network_aarch64_sve( io_generated_code, l_in_idx, l_out_idx, l_src_start, l_dst_start, 4,
                                                                   LIBXSMM_AARCH64_INSTR_sve_TRN1, LIBXSMM_AARCH64_INSTR_sve_TRN2, 16, LIBXSMM_AARCH64_sve_TUPLETYPE_2D );
  }
  ...
  /* shuffle 32 bit */
  {
    ...
    unsigned int  l_dst_start = 16;
    libxsmm_generator_transform_Xway_unpack_network_aarch64_sve( io_generated_code, l_in_idx, l_out_idx, l_src_start, l_dst_start, 2,
                                                                   LIBXSMM_AARCH64_INSTR_sve_TRN1, LIBXSMM_AARCH64_INSTR_sve_TRN2, 16, LIBXSMM_AARCH64_sve_TUPLETYPE_4S );
  }

  /* store 8 vectors */
  libxsmm_generator_store_2dregblock_aarch64_sve( io_generated_code, i_gp_reg_out, i_gp_reg_scratch, 4, 32, 8, 8, i_mateltwise_desc->ldo * i_micro_kernel_config->datatype_size_out );
}

LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_to_normt_mbit_scalar_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                                      ...
                                                                                      const libxsmm_mateltwise_kernel_config* i_micro_kernel_config,
                                                                                      const libxsmm_meltw_descriptor*         i_mateltwise_desc ) {
  libxsmm_aarch64_sve_width l_load_instr_width;
  libxsmm_aarch64_sve_width l_store_instr_width;

  /* select load and store instructions */
  if ( i_micro_kernel_config->datatype_size_in == 8 ) {
    l_load_instr_width = LIBXSMM_AARCH64_sve_WIDTH_D;
    l_store_instr_width = LIBXSMM_AARCH64_sve_WIDTH_D;
  } else if ( i_micro_kernel_config->datatype_size_in == 4 ) {
    l_load_instr_width = LIBXSMM_AARCH64_sve_WIDTH_S;
    l_store_instr_width = LIBXSMM_AARCH64_sve_WIDTH_S;
  } else if ( i_micro_kernel_config->datatype_size_in == 2 ) {
    l_load_instr_width = LIBXSMM_AARCH64_sve_WIDTH_H;
    l_store_instr_width = LIBXSMM_AARCH64_sve_WIDTH_H;
  } else if ( i_micro_kernel_config->datatype_size_in == 1 ) {
    l_load_instr_width = LIBXSMM_AARCH64_sve_WIDTH_B;
    l_store_instr_width = LIBXSMM_AARCH64_sve_WIDTH_B;
  } else {
    /* should not happen */
    l_load_instr_width = LIBXSMM_AARCH64_sve_WIDTH_B;
    l_store_instr_width = LIBXSMM_AARCH64_sve_WIDTH_B;
  ...
  /* actual transpose */
  libxsmm_aarch64_instruction_sve_move( io_generated_code, LIBXSMM_AARCH64_INSTR_sve_LDR_I_OFF,
                                          i_gp_reg_in, LIBXSMM_AARCH64_GP_REG_UNDEF, 0, 0, l_load_instr_width );

  libxsmm_aarch64_instruction_sve_move( io_generated_code, LIBXSMM_AARCH64_INSTR_sve_STR_I_OFF,
...
LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_to_vnni_mbit_scalar_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                                     libxsmm_loop_label_tracker*             io_loop_label_tracker,
                                                                                     const unsigned int                      i_gp_reg_in,
                                                                                     const unsigned int                      i_gp_reg_out,
                                                                                     const unsigned int                      i_gp_reg_m_loop,
  ...
  if ( i_mateltwise_desc->n >= 2 ) {
    ...
    libxsmm_aarch64_instruction_sve_move( io_generated_code, LIBXSMM_AARCH64_INSTR_sve_STR_I_POST,
  ...
  if ( (i_mateltwise_desc->n % 2 == 1) && (i_pad_vnni == 1) ) {
    ...
    libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_sve_EOR_V,
                                               1, 1, 0, 1,LIBXSMM_AARCH64_sve_TUPLETYPE_16B );

    /* m loop header */
    libxsmm_generator_loop_header_aarch64( io_generated_code, io_loop_label_tracker, i_gp_reg_m_loop, i_mateltwise_desc->m );

    /* actual transpose */
    libxsmm_aarch64_instruction_sve_move( io_generated_code, LIBXSMM_AARCH64_INSTR_sve_LDR_I_POST,
...
LIBXSMM_API_INTERN
void libxsmm_generator_transform_vnni_to_vnnit_mbit_scalar_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                                      libxsmm_loop_label_tracker*             io_loop_label_tracker,
                                                                                      const unsigned int                      i_gp_reg_in,
                                                                                      const unsigned int                      i_gp_reg_out,
                                                                                      const unsigned int                      i_gp_reg_m_loop,
...
LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_padnm_mod2_mbit_scalar_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                                        libxsmm_loop_label_tracker*             io_loop_label_tracker,
                                                                                        const unsigned int                      i_gp_reg_in,
                                                                                        const unsigned int                      i_gp_reg_out,
                                                                                        const unsigned int                      i_gp_reg_m_loop,
...
LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_to_normt_64bit_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                                libxsmm_loop_label_tracker*             io_loop_label_tracker,
                                                                                const unsigned int                      i_gp_reg_in,
                                                                                const unsigned int                      i_gp_reg_out,
                                                                                const unsigned int                      i_gp_reg_m_loop,
  ...
  if ( i_mateltwise_desc->m % 4 == 0 && i_mateltwise_desc->n % 4 == 0 ) {
    ...
    /* shuffle network */
    libxsmm_generator_transform_norm_to_normt_64bit_4x4_shufflenetwork_aarch64_sve( io_generated_code, i_gp_reg_in, i_gp_reg_out, i_gp_reg_scratch,
    ...
    libxsmm_generator_transform_norm_to_normt_mbit_scalar_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
...
LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_to_normt_32bit_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                                libxsmm_loop_label_tracker*             io_loop_label_tracker,
                                                                                const unsigned int                      i_gp_reg_in,
                                                                                const unsigned int                      i_gp_reg_out,
                                                                                const unsigned int                      i_gp_reg_m_loop,
...
LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_to_normt_16bit_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                                libxsmm_loop_label_tracker*             io_loop_label_tracker,
                                                                                const unsigned int                      i_gp_reg_in,
                                                                                const unsigned int                      i_gp_reg_out,
                                                                                const unsigned int                      i_gp_reg_m_loop,
...
LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_to_normt_08bit_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                                libxsmm_loop_label_tracker*             io_loop_label_tracker,
                                                                                const unsigned int                      i_gp_reg_in,
                                                                                const unsigned int                      i_gp_reg_out,
                                                                                const unsigned int                      i_gp_reg_m_loop,
...
LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_to_vnni_16bit_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                               libxsmm_loop_label_tracker*             io_loop_label_tracker,
                                                                               const unsigned int                      i_gp_reg_in,
                                                                               const unsigned int                      i_gp_reg_out,
                                                                               const unsigned int                      i_gp_reg_m_loop,
                                                                               ...
                                                                               const libxsmm_meltw_descriptor*         i_mateltwise_desc,
                                                                               const unsigned int                      i_pad_vnni ) {
  libxsmm_generator_transform_norm_to_vnni_mbit_scalar_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
                                                                                  i_gp_reg_in, i_gp_reg_out, i_gp_reg_m_loop, i_gp_reg_n_loop,
                                                                                  i_gp_reg_scratch, i_micro_kernel_config, i_mateltwise_desc, i_pad_vnni );
}

LIBXSMM_API_INTERN
void libxsmm_generator_transform_vnni_to_vnnit_16bit_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                                libxsmm_loop_label_tracker*             io_loop_label_tracker,
                                                                                const unsigned int                      i_gp_reg_in,
                                                                                const unsigned int                      i_gp_reg_out,
                                                                                const unsigned int                      i_gp_reg_m_loop,
                                                                                ...
                                                                                const libxsmm_mateltwise_kernel_config* i_micro_kernel_config,
                                                                                const libxsmm_meltw_descriptor*         i_mateltwise_desc ) {
  libxsmm_generator_transform_vnni_to_vnnit_mbit_scalar_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
                                                                                   i_gp_reg_in, i_gp_reg_out, i_gp_reg_m_loop, i_gp_reg_n_loop,
                                                                                   i_gp_reg_scratch, i_micro_kernel_config, i_mateltwise_desc );
}

LIBXSMM_API_INTERN
void libxsmm_generator_transform_norm_padnm_mod2_16bit_aarch64_sve_microkernel( libxsmm_generated_code*                 io_generated_code,
                                                                                  libxsmm_loop_label_tracker*             io_loop_label_tracker,
                                                                                  const unsigned int                      i_gp_reg_in,
                                                                                  const unsigned int                      i_gp_reg_out,
                                                                                  const unsigned int                      i_gp_reg_m_loop,
                                                                                  ...
                                                                                  const libxsmm_mateltwise_kernel_config* i_micro_kernel_config,
                                                                                  const libxsmm_meltw_descriptor*         i_mateltwise_desc ) {
  libxsmm_generator_transform_norm_padnm_mod2_mbit_scalar_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
                                                                                     i_gp_reg_in, i_gp_reg_out, i_gp_reg_m_loop, i_gp_reg_n_loop,
                                                                                     i_gp_reg_scratch, i_micro_kernel_config, i_mateltwise_desc );
}
...
void libxsmm_generator_transform_aarch64_sve_microkernel( libxsmm_generated_code*                        io_generated_code,
  ...
  if ( LIBXSMM_GEMM_PRECISION_F64 == LIBXSMM_GETENUM_INP( i_mateltwise_desc->datatype ) &&
    ...
    if (i_mateltwise_desc->param == LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_NORM_TO_NORMT) {
      libxsmm_generator_transform_norm_to_normt_64bit_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
    ...
    if (i_mateltwise_desc->param == LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_NORM_TO_NORMT) {
      libxsmm_generator_transform_norm_to_normt_32bit_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
                                                                                 i_gp_reg_mapping->gp_reg_in, i_gp_reg_mapping->gp_reg_out,
                                                                                 i_gp_reg_mapping->gp_reg_m_loop, i_gp_reg_mapping->gp_reg_n_loop,
                                                                                 i_gp_reg_mapping->gp_reg_scratch_0, i_micro_kernel_config, i_mateltwise_desc );
    } else {
    ...
    switch(i_mateltwise_desc->param){
      case LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_NORM_TO_NORMT:
        libxsmm_generator_transform_norm_to_normt_16bit_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
                                                                                 i_gp_reg_mapping->gp_reg_in, i_gp_reg_mapping->gp_reg_out,
                                                                                 i_gp_reg_mapping->gp_reg_m_loop, i_gp_reg_mapping->gp_reg_n_loop,
                                                                                 i_gp_reg_mapping->gp_reg_scratch_0, i_micro_kernel_config, i_mateltwise_desc );
        break;
      case LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_VNNI_TO_VNNIT:
        libxsmm_generator_transform_vnni_to_vnnit_16bit_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
                                                                                 i_gp_reg_mapping->gp_reg_in, i_gp_reg_mapping->gp_reg_out,
                                                                                 i_gp_reg_mapping->gp_reg_m_loop, i_gp_reg_mapping->gp_reg_n_loop,
                                                                                 i_gp_reg_mapping->gp_reg_scratch_0, i_micro_kernel_config, i_mateltwise_desc );
        break;
      case LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_NORM_TO_VNNI:
        libxsmm_generator_transform_norm_to_vnni_16bit_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
                                                                                i_gp_reg_mapping->gp_reg_in, i_gp_reg_mapping->gp_reg_out,
                                                                                i_gp_reg_mapping->gp_reg_m_loop, i_gp_reg_mapping->gp_reg_n_loop,
                                                                                i_gp_reg_mapping->gp_reg_scratch_0, i_micro_kernel_config, i_mateltwise_desc, 0 ); 
        break;
      case LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_NORM_TO_VNNI_PAD:
        libxsmm_generator_transform_norm_to_vnni_16bit_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
                                                                                i_gp_reg_mapping->gp_reg_in, i_gp_reg_mapping->gp_reg_out,
                                                                                i_gp_reg_mapping->gp_reg_m_loop, i_gp_reg_mapping->gp_reg_n_loop,
      ...
      case LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_PADNM_MOD2:
        libxsmm_generator_transform_norm_padnm_mod2_16bit_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
                                                                                   i_gp_reg_mapping->gp_reg_in, i_gp_reg_mapping->gp_reg_out,
                                                                                   i_gp_reg_mapping->gp_reg_m_loop, i_gp_reg_mapping->gp_reg_n_loop,
                                                                                   i_gp_reg_mapping->gp_reg_scratch_0, i_micro_kernel_config, i_mateltwise_desc );
        break;
    ...
    if (i_mateltwise_desc->param == LIBXSMM_MELTW_TYPE_UNARY_TRANSFORM_NORM_TO_NORMT) {
      libxsmm_generator_transform_norm_to_normt_08bit_aarch64_sve_microkernel( io_generated_code, io_loop_label_tracker,
                                                                                 i_gp_reg_mapping->gp_reg_in, i_gp_reg_mapping->gp_reg_out,
                                                                                 i_gp_reg_mapping->gp_reg_m_loop, i_gp_reg_mapping->gp_reg_n_loop,
                                                                                 i_gp_reg_mapping->gp_reg_scratch_0, i_micro_kernel_config, i_mateltwise_desc );
    } else {
