...
void libxsmm_generator_spgemm_csr_asparse_reg( libxsmm_generated_code*         io_generated_code,
  ...
  unsigned int* l_idx_fma_target;
  unsigned int l_block_row;
  unsigned int l_col;
  unsigned int l_col_target;
  unsigned int l_max_unique;
  ...
  if ( io_generated_code->arch == LIBXSMM_X86_AVX2 ) {
    if ( l_unique > 16 - l_n_blocking ) {
      l_m_blocking = getenv("M_BLOCKING") ? atof(getenv("M_BLOCKING")) : 1;
    } else {
      l_m_blocking = 1;
    }
  } else {
...
/*  l_gp_reg_mapping.gp_reg_c_prefetch = LIBXSMM_X86_GP_REG_R9;*/
  ...
  } else if (l_unique <= l_psreg_unique) {
  ...
  for (l_base_row = 0; l_base_row < (unsigned int)i_xgemm_desc->m; l_base_row += l_m_blocking) {

    /* calculate the number of rows in this block*/
    l_block_rows = ((unsigned int)i_xgemm_desc->m - l_base_row) < l_m_blocking ? ((unsigned int)i_xgemm_desc->m - l_base_row) : l_m_blocking;

    for (l_block_row = 0; l_block_row < l_block_rows; l_block_row++) {
      l_idx_fma_target[l_block_row] = i_row_idx[l_base_row + l_block_row];
    }

    for (l_block_row = 0; l_block_row < l_block_rows; l_block_row++) {
      l_m = l_base_row + l_block_row;
        ...
        for ( l_n = 0; l_n < l_n_blocking; l_n++ ) {
          ...
          if ( 0 == (LIBXSMM_GEMM_FLAG_BETA_0 & i_xgemm_desc->flags) ) { /* Beta=1 */
            libxsmm_x86_instruction_vec_move( io_generated_code,
                                              ...
                                              l_micro_kernel_config.vector_name,
                                              l_base_acc_reg + l_block_row*l_n_blocking + l_n, 0, 1, 0 );
          } else {
            libxsmm_x86_instruction_vec_compute_reg( io_generated_code,
                                                    l_micro_kernel_config.instruction_set,
                                                    l_micro_kernel_config.vxor_instruction,
                                                    l_micro_kernel_config.vector_name,
                                                    l_base_acc_reg + l_block_row*l_n_blocking + l_n,
                                                    l_base_acc_reg + l_block_row*l_n_blocking + l_n,
                                                    l_base_acc_reg + l_block_row*l_n_blocking + l_n );
          }
    ...
    for (l_col = 0; l_col < (unsigned int)i_xgemm_desc->k; l_col++) {
      for (l_block_row = 0; l_block_row < l_block_rows; l_block_row++) {
        l_m = l_base_row + l_block_row;

        if (l_idx_fma_target[l_block_row] >= i_row_idx[l_m + 1]) {
          /* this row contains no more non-zero element */
          continue;
        }

        l_col_target = i_column_idx[l_idx_fma_target[l_block_row]];

        if (l_col < l_col_target) {
          /* current element is zero */
          continue;

        } else if (l_col == l_col_target) {
          /* hit! need to FMA */

          const unsigned int u = l_idx_fma_target[l_block_row];
          ...
          if (l_unique > l_breg_unique ) {
            if ( l_fp64 ) {
              /* broadcast packed A element from memory */
              if ( l_unique > l_max_unique ) {
                libxsmm_x86_instruction_vec_move(io_generated_code,
                                                  l_micro_kernel_config.instruction_set,
              ...
              /* load permute selector operand if not stored in registers */
                ...
                /* broadcast packed A element from vector register */
                libxsmm_x86_instruction_vec_compute_reg(io_generated_code,
                                                        l_micro_kernel_config.instruction_set,
          ...
          for ( l_n = 0; l_n < l_n_blocking; l_n++ ) {
            ...
            if (l_unique > l_max_unique) {
              l_unique_reg = l_block_row;
            } else if (l_unique > l_breg_unique && io_generated_code->arch != LIBXSMM_X86_AVX2 ) {
              l_unique_reg = l_bcast_reg;
            } else {
    ...
    /* Store the C values back to memory */
    for (l_block_row = 0; l_block_row < l_block_rows; l_block_row++) {
        ...
        for ( l_n = 0; l_n < l_n_blocking; l_n++ ) {
          ...
          libxsmm_x86_instruction_vec_move( io_generated_code,
                                            ...
                                            l_m*i_xgemm_desc->ldc*l_micro_kernel_config.datatype_size_out +
                                              l_n*l_micro_kernel_config.datatype_size_out*l_micro_kernel_config.vector_length,
                                            l_micro_kernel_config.vector_name,
                                            l_base_acc_reg + l_block_row*l_n_blocking + l_n, 0, 0, 1 );
        }
      }
    }
  }
