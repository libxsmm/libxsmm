...
void libxsmm_generator_copy_opargs_aarch64(libxsmm_generated_code*        io_generated_code,
  ...
  if (cur_node->type == LIBXSMM_MATRIX_EQN_NODE_ARG) {
    ...
    if (cur_node->info.u_op.type == LIBXSMM_MELTW_TYPE_UNARY_DUMP) {
      ...
      libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD, input_reg, temp_reg, temp_reg2, (long long)cur_node->info.u_op.op_arg_pos*48 );
  ...
  if (cur_node->type == LIBXSMM_MATRIX_EQN_NODE_ARG) {
    ...
    if (cur_node->info.arg.in_pos >= 0){
      if (cur_pos < i_micro_kernel_config->n_avail_gpr) {
        libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD, input_reg, temp_reg, temp_reg2, (long long)cur_node->info.arg.in_pos*48 );
