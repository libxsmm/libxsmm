  ...
  if (cur_node->type == LIBXSMM_MATRIX_EQN_NODE_ARG) {
    ...
    if (cur_node->info.arg.in_pos >= 0){
      if (cur_pos < i_micro_kernel_config->n_avail_gpr) {
        libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD, input_reg, temp_reg, temp_reg2, (unsigned long long)cur_node->info.arg.in_pos*32 );
...
void libxsmm_meqn_setup_input_output_masks_aarch64( libxsmm_generated_code*                 io_generated_code,
  ...
  if ( l_is_sve ) {
    ...
    if (use_m_input_masking) {
      /* F32 mask */
      libxsmm_generator_set_p_register_aarch64_sve( io_generated_code, reserved_mask_regs, (i_m % i_vlen_in) * 4, i_gp_reg_mapping->gp_reg_scratch_0 );
    ...
    if (use_m_output_masking) {
      /* F32 mask */
      libxsmm_generator_set_p_register_aarch64_sve( io_generated_code, reserved_mask_regs, (i_m % i_vlen_out) * 4, i_gp_reg_mapping->gp_reg_scratch_0 );
    ...
    i_mask_reg_in  = i_m % i_vlen_in;
    i_mask_reg_out = i_m % i_vlen_out;
  }
...
void libxsmm_generator_mateqn_load_arg_to_2d_reg_block_aarch64( libxsmm_generated_code*          io_generated_code,
  ...
  for (in = 0; in < i_n_blocking; in++) {
    for (im = 0; im < i_m_blocking; im++) {
      unsigned int l_masked_elements = (LIBXSMM_DATATYPE_F32 == arg_info[i_arg_id].dtype) ? (im == i_m_blocking - 1) ? i_mask_last_m_chunk : 0
                                                                                          : (im == i_m_blocking - 1) ? (i_mask_last_m_chunk > 0) ? i_mask_last_m_chunk : i_vlen : i_vlen;
      unsigned int l_mask_load = (LIBXSMM_DATATYPE_F32 == arg_info[i_arg_id].dtype) ? (im == i_m_blocking - 1) ? (i_mask_last_m_chunk > 0) ? i_micro_kernel_config->in_f32_mask : 0 : 0
      ...
      if (arg_info[i_arg_id].bcast_type == LIBXSMM_MATRIX_EQN_BCAST_TYPE_NONE) {
        ...
        libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, input_reg, i_gp_reg_mapping->gp_reg_scratch_0, cur_vreg, LIBXSMM_TYPESIZE(arg_info[i_arg_id].dtype), l_masked_elements, 1, 0, l_mask_load);
    ...
    if (arg_info[i_arg_id].bcast_type == LIBXSMM_MATRIX_EQN_BCAST_TYPE_NONE) {
      if ( l_ld_bytes != l_m_adjust ) {
        libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD, input_reg, i_gp_reg_mapping->gp_reg_scratch_0, input_reg, (unsigned long long)((unsigned long long)(l_ld_bytes-l_m_adjust)) );
      }
    } else {
      if (arg_info[i_arg_id].bcast_type == LIBXSMM_MATRIX_EQN_BCAST_TYPE_ROW) {
        if ( l_ld_bytes != LIBXSMM_TYPESIZE(arg_info[i_arg_id].dtype) ) {
          libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD, input_reg, i_gp_reg_mapping->gp_reg_scratch_0, input_reg, (unsigned long long)((unsigned long long)(l_ld_bytes-LIBXSMM_TYPESIZE(arg_info[i_arg_id].dtype))) );
        }
      }
    }
  }
  libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_SUB,i input_reg, i_gp_reg_mapping->gp_reg_scratch_0, input_reg, (unsigned long long)((unsigned long long)(offset)) );
}
...
void libxsmm_generator_mateqn_store_2d_reg_block_aarch64( libxsmm_generated_code*          io_generated_code,
  ...
  for (in = 0; in < i_n_blocking; in++) {
    for (im = 0; im < i_m_blocking; im++) {
      ...
      libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, i_gp_reg_mapping->gp_reg_out, i_gp_reg_mapping->gp_reg_scratch_0, cur_vreg, LIBXSMM_TYPESIZE(LIBXSMM_GETENUM_OUT(i_meqn_desc->datatype)), l_masked_elements, 1, 1, l_mask_store);
    }
    if ( l_ld_bytes != l_m_adjust ) {
      libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD, i_gp_reg_mapping->gp_reg_out, i_gp_reg_mapping->gp_reg_scratch_0, i_gp_reg_mapping->gp_reg_out, (unsigned long long)((unsigned long long)(l_ld_bytes-l_m_adjust)) );
    }
  }
  libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_SUB, i_gp_reg_mapping->gp_reg_out, i_gp_reg_mapping->gp_reg_scratch_0, i_gp_reg_mapping->gp_reg_out, (unsigned long long)((unsigned long long)(l_ld_bytes*i_n_blocking)) );
}
...
void libxsmm_generator_mateqn_unpackstore_2d_reg_block_aarch64( libxsmm_generated_code*          io_generated_code,
  ...
  for (in = 0; in < i_n_blocking; in++) {
    for (im = 0; im < i_m_blocking; im++) {
      ...
      libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, i_gp_reg_mapping->gp_reg_out, i_gp_reg_mapping->gp_reg_scratch_1, i_micro_kernel_config->tmp_vreg, LIBXSMM_TYPESIZE(LIBXSMM_GETENUM_OUT(i_meqn_desc->datatype)), l_masked_elements, 1, 1, l_mask_store);
    }
    if ( l_ld_bytes != l_m_adjust ) {
  ...
  for (in = 0; in < i_n_blocking; in++) {
    for (im = 0; im < i_m_blocking; im++) {
      ...
      libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, i_gp_reg_mapping->gp_reg_out, i_gp_reg_mapping->gp_reg_scratch_1, i_micro_kernel_config->tmp_vreg,
                                                            LIBXSMM_TYPESIZE(LIBXSMM_GETENUM_OUT(i_meqn_desc->datatype)), l_masked_elements, 1, 1, l_mask_store);
    }
    if ( l_ld_bytes != l_m_adjust ) {
      libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD,
                                                      i_gp_reg_mapping->gp_reg_out, i_gp_reg_mapping->gp_reg_scratch_0, i_gp_reg_mapping->gp_reg_out,
...
void libxsmm_generator_mateqn_store_reduce_to_scalar_output_aarch64( libxsmm_generated_code*          io_generated_code,
  ...
  libxsmm_aarch64_asimd_tupletype asimd_type = LIBXSMM_AARCH64_ASIMD_TUPLETYPE_4S;
...
void libxsmm_generator_mateqn_compute_unary_op_2d_reg_block_aarch64( libxsmm_generated_code*     io_generated_code,
  ...
  unsigned char l_is_sve = (io_generated_code->arch >= LIBXSMM_AARCH64_SVE128) && (io_generated_code->arch <= LIBXSMM_AARCH64_ALLFEAT);
  libxsmm_aarch64_sve_type l_sve_type = libxsmm_generator_aarch64_get_sve_type(4);
  libxsmm_aarch64_asimd_tupletype l_tupletype = LIBXSMM_AARCH64_ASIMD_TUPLETYPE_4S;
  ...
  for (in = 0; in < i_n_blocking; in++) {
    for (im = 0; im < i_m_blocking; im++) {
      ...
      if (i_op_type == LIBXSMM_MELTW_TYPE_UNARY_X2) {
        ...
        unsigned int reduce_instr = is_sve ? LIBXSMM_AARCH64_INSTR_SVE_FADD_V : LIBXSMM_AARCH64_INSTR_ASIMD_FADD_V;
        if ( l_is_sve ) {
        ...
        if( l_is_sve ) {
          libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_FADD_V,c ur_vreg, i_micro_kernel_config->vec_ones, 0, cur_vreg, l_pred_reg, l_sve_type );
        } else {
  ...
  for (in = 0; in < i_n_blocking; in++) {
    for (im = 0; im < i_m_blocking; im++) {
      ...
      if (i_op_type == LIBXSMM_MELTW_TYPE_BINARY_PACK) {
        libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_UUNPKLO_V, left_vreg, LIBXSMM_AARCH64_SVE_REG_UNDEF, 0, left_vreg, 0, libxsmm_generator_aarch64_get_sve_type(4) );
        libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_UUNPKLO_V, right_vreg, LIBXSMM_AARCH64_SVE_REG_UNDEF, 0, right_vreg, 0, libxsmm_generator_aarch64_get_sve_type(4));
        libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_LSL_I_V, right_vreg LIBXSMM_AARCH64_SVE_REG_UNDEF, 16, right_vreg, 0, libxsmm_generator_aarch64_get_sve_type(4) );
        ...
        if( l_is_sve ){
          ...
          libxsmm_aarch64_instruction_asimd_compute( io_generated_code, binary_op_instr, left_vreg, right_vreg, 0, dst_vreg, LIBXSMM_AARCH64_ASIMD_TUPLETYPE_4S);
        }
      }
    }
  }
...
void libxsmm_generator_mateqn_2d_microkernel_aarch64( libxsmm_generated_code*                    io_generated_code,
  ...
  libxsmm_meqn_setup_input_output_masks_aarch64( io_generated_code, i_micro_kernel_config, i_meqn_desc,
...
void libxsmm_configure_reserved_zmms_and_masks_aarch64(libxsmm_generated_code* io_generated_code,
  ...
  for (i = 0 ; i < 64; i++) {
    if (i_micro_kernel_config->unary_ops_pool[i] > 0) {
      ...
      libxsmm_configure_unary_aarch64_kernel_vregs_masks( io_generated_code, meltw_config, i, 0, i_gp_reg_mapping->temp_reg, LIBXSMM_X86_GP_REG_UNDEF, LIBXSMM_X86_GP_REG_UNDEF);
    }
  }
