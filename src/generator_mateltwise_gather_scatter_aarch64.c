...
void libxsmm_generator_gather_scatter_rows_aarch64_mn_loop_unrolled( libxsmm_generated_code*                              io_generated_code,
  ...
  for (l_in = 0; l_in < i_n_unroll_factor; l_in++) {
    for (l_im = 0; l_im < i_m_unroll_factor; l_im++) {
      unsigned int idx_vreg_id = i_idx_vreg_start + l_im;
      unsigned int aux_vreg = i_idx_vreg_start + i_m_unroll_factor + l_im + l_in * i_m_unroll_factor;
      ...
      if (i_is_gather == 1) {
        ...
        if (l_is_sve > 0) {
          libxsmm_aarch64_instruction_sve_move( io_generated_code, i_gather_instr, i_gp_idx_mat_reg, idx_vreg_id, 0, aux_vreg, l_idx_mask_load);
          if (l_is_16bit_gs || l_is_64bit_idx) {
            ...
            libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_UZP1_V, aux_vreg, aux_vreg, 0, aux_vreg, 0, libxsmm_generator_aarch64_get_sve_type(i_dtype_size_reg_mat) );
          }
        } else {
          libxsmm_generator_gather_scatter_vreg_asimd_aarch64( io_generated_code, i_gp_idx_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, i_gp_reg_mapping->gp_reg_scratch_1,
              idx_vreg_id, i_idx_tsize, aux_vreg, i_dtype_size_reg_mat, l_idx_masked_elements, i_is_gather);
        }
        /* Store gathered vector  */
        libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, i_gp_reg_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, aux_vreg,
        ...
        if (l_is_sve > 0) {
          if (l_is_16bit_gs || l_is_64bit_idx) {
            libxsmm_aarch64_instruction_sve_compute( io_generated_code, LIBXSMM_AARCH64_INSTR_SVE_ZIP1_V, aux_vreg, aux_vreg, 0, aux_vreg, 0, libxsmm_generator_aarch64_get_sve_type(i_dtype_size_reg_mat) );
            if ((l_is_16bit_gs > 0) && (l_is_64bit_idx> 0)) {
          ...
          libxsmm_aarch64_instruction_sve_move( io_generated_code, i_scatter_instr, i_gp_idx_mat_reg, idx_vreg_id, 0, aux_vreg, l_idx_mask_load);
        } else {
          libxsmm_generator_gather_scatter_vreg_asimd_aarch64( io_generated_code, i_gp_idx_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, i_gp_reg_mapping->gp_reg_scratch_1,
              idx_vreg_id, i_idx_tsize, aux_vreg, i_dtype_size_reg_mat, l_idx_masked_elements, i_is_gather);
        }
...
void libxsmm_generator_gather_scatter_rows_aarch64_microkernel( libxsmm_generated_code*                        io_generated_code,
  ...
  unsigned char l_is_sve = (io_generated_code->arch >= LIBXSMM_AARCH64_SVE128) && (io_generated_code->arch <= LIBXSMM_AARCH64_ALLFEAT);
  unsigned int l_vector_length = libxsmm_cpuid_vlen(io_generated_code->arch);
  unsigned int vlen = l_vector_length/l_idx_tsize; /* The granularity of work is determined by the idx size */
  ...
  gather_instr  = (i_micro_kernel_config->datatype_size_in == 4) ? ((l_idx_tsize == 4) ? LIBXSMM_AARCH64_INSTR_SVE_LD1W_V_OFF_SCALE : LIBXSMM_AARCH64_INSTR_SVE_LD1W_V_OFF64_SCALE)
                                                                 : ((l_idx_tsize == 4) ? LIBXSMM_AARCH64_INSTR_SVE_LD1H_V_OFF_SCALE : LIBXSMM_AARCH64_INSTR_SVE_LD1H_V_OFF64_SCALE);
  scatter_instr = (i_micro_kernel_config->datatype_size_in == 4) ? ((l_idx_tsize == 4) ? LIBXSMM_AARCH64_INSTR_SVE_ST1W_V_OFF_SCALE : LIBXSMM_AARCH64_INSTR_SVE_ST1W_V_OFF64_SCALE)
  ...
  if (m_remainder_elements > 0) {
    if (l_is_sve > 0) {
      ...
      if (i_micro_kernel_config->datatype_size_in == 4 || i_micro_kernel_config->datatype_size_in == 2) {
        ...
        libxsmm_generator_set_p_register_aarch64_sve( io_generated_code, idx_mask_reg, m_remainder_elements * l_idx_tsize, i_gp_reg_mapping->gp_reg_scratch_0 );
      } else {
        /* should not happen */
#if defined(LIBXSMM_GENERATOR_MATELTWISE_GATHER_SCATTER_AARCH64_JUMP_LABEL_TRACKER_MALLOC)
        free(p_jump_label_tracker);
  ...
  if (m_trips_loop >= 1) {
    ...
    libxsmm_generator_gather_scatter_rows_aarch64_mn_loop_unrolled( io_generated_code, i_gp_reg_mapping, m_unroll_factor, n_unroll_factor,
        idx_vreg_start, l_idx_tsize, idx_mask_reg, ld_idx_mat, gather_instr, scatter_instr, vlen, 0,
  ...
  if (peeled_m_trips > 0) {
    ...
    libxsmm_generator_gather_scatter_rows_aarch64_mn_loop_unrolled( io_generated_code, i_gp_reg_mapping, peeled_m_trips, n_unroll_factor,
        idx_vreg_start, l_idx_tsize, idx_mask_reg, ld_idx_mat, gather_instr, scatter_instr, vlen, m_remainder_elements,
        mask_reg, mask_reg_full_frac_vlen, is_gather, gp_idx_mat_reg, gp_reg_mat_reg, dtype_size_reg_mat, ld_reg_mat );
