...
void libxsmm_generator_gather_scatter_cols_aarch64_microkernel( libxsmm_generated_code*                        io_generated_code,
  ...
  unsigned int m, n, im = 0, m_remainder_elements, m_trips, max_m_unrolling = 4, m_unroll_factor = 1, m_trips_loop = 0, peeled_m_trips = 0;
  ...
  unsigned int gp_idx_mat_reg = 0, gp_idx_mat_reg_precision = 0, gp_reg_mat_reg = 0, gp_idx_mat_base_reg = 0;
  unsigned int gp_reg_ld = 0;
  unsigned int aux_vreg_start = 0;
  ...
  gp_idx_mat_reg_precision = (idx_tsize == 8) ? LIBXSMM_AARCH64_GP_REG_X14 : LIBXSMM_AARCH64_GP_REG_W14;
  gp_reg_ld             = LIBXSMM_AARCH64_GP_REG_X15;
  ...
  if (is_gather == 1) {
    libxsmm_aarch64_instruction_alu_set_imm64( io_generated_code, gp_reg_ld, (long long)i_mateltwise_desc->ldi * i_micro_kernel_config->datatype_size_in );
  } else {
    libxsmm_aarch64_instruction_alu_set_imm64( io_generated_code, gp_reg_ld, (long long)i_mateltwise_desc->ldo * i_micro_kernel_config->datatype_size_in );
  }
  ...
  libxsmm_aarch64_instruction_alu_compute_shifted_reg( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_EOR_SR, gp_idx_mat_reg, gp_idx_mat_reg, gp_idx_mat_reg, 0, LIBXSMM_AARCH64_SHIFTMODE_LSL );
  libxsmm_aarch64_instruction_alu_move( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_LDR_I_POST, i_gp_reg_mapping->gp_reg_ind_base, LIBXSMM_AARCH64_GP_REG_UNDEF, idx_tsize, gp_idx_mat_reg_precision );
  libxsmm_aarch64_instruction_alu_compute_shifted_reg( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_MUL, gp_idx_mat_reg, gp_reg_ld, gp_idx_mat_reg, 0, LIBXSMM_AARCH64_SHIFTMODE_LSL );
  libxsmm_aarch64_instruction_alu_compute_shifted_reg( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_ADD_SR, gp_idx_mat_reg, gp_idx_mat_base_reg, gp_idx_mat_reg, 0, LIBXSMM_AARCH64_SHIFTMODE_LSL );
  ...
  if (m_trips_loop >= 1) {
    ...
    for (im = 0; im < m_unroll_factor; im++) {
      unsigned int aux_vreg = aux_vreg_start + im;
      if (is_gather == 1) {
        /* Load gather vector  */
        libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, gp_idx_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, aux_vreg, dtype_size_reg_mat, 0, 1, 0, 0 );
        /* Store gathered vector  */
        libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, gp_reg_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, aux_vreg, dtype_size_reg_mat, 0, 1, 1, 0 );
      } else {
        /* Load vector to be scattered */
        libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, gp_reg_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, aux_vreg, dtype_size_reg_mat, 0, 1, 0, 0 );
        /* Store vector to indexed column*/
        libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, gp_idx_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, aux_vreg, dtype_size_reg_mat, 0, 1, 1, 0 );
      }
    }
  ...
  if (peeled_m_trips > 0) {
    for (im = 0; im < peeled_m_trips; im++) {
      unsigned int l_masked_reg_elements = (im == peeled_m_trips - 1) ? m_remainder_elements : 0;
      unsigned int l_mask_reg = ((im == peeled_m_trips - 1) && (m_remainder_elements > 0)) ? mask_reg : 0;
      unsigned int aux_vreg = aux_vreg_start + im;
      if (is_gather == 1) {
        /* Load gather vector  */
        libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, gp_idx_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, aux_vreg, dtype_size_reg_mat, l_masked_reg_elements, 1, 0, LIBXSMM_CAST_UCHAR(l_mask_reg) );
        /* Store gathered vector  */
        libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, gp_reg_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, aux_vreg, dtype_size_reg_mat, l_masked_reg_elements, 1, 1, LIBXSMM_CAST_UCHAR(l_mask_reg) );
      } else {
        /* Load vector to be scattered */
        libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, gp_reg_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, aux_vreg, dtype_size_reg_mat, l_masked_reg_elements, 1, 0, LIBXSMM_CAST_UCHAR(l_mask_reg) );
        /* Store vector to indexed column*/
        libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, gp_idx_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, aux_vreg, dtype_size_reg_mat, l_masked_reg_elements, 1, 1, LIBXSMM_CAST_UCHAR(l_mask_reg) );
      }
    }
  }

  if (((long long)ld_reg_mat * dtype_size_reg_mat - (long long)m * dtype_size_reg_mat) > 0) {
    libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD,
                                                   gp_reg_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, gp_reg_mat_reg, (long long)ld_reg_mat * dtype_size_reg_mat -(long long)m * dtype_size_reg_mat );
  }
...
void libxsmm_generator_gather_scatter_rows_aarch64_mn_loop_unrolled( libxsmm_generated_code*                        io_generated_code,
  ...
  for (in = 0; in < n_unroll_factor; in++) {
    for (im = 0; im < m_unroll_factor; im++) {
      unsigned int idx_vreg_id = idx_vreg_start + im;
      unsigned int aux_vreg = idx_vreg_start + m_unroll_factor + im + in * m_unroll_factor;
    ...
    libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD,
                                                  gp_idx_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, gp_idx_mat_reg,
                                                  ((long long)ld_idx_mat) * dtype_size_reg_mat);
  ...
  libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_SUB,
                                               gp_idx_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, gp_idx_mat_reg,
                                               (long long)ld_idx_mat*dtype_size_reg_mat*n_unroll_factor );
  libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_SUB,
                                               gp_reg_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, gp_reg_mat_reg,
...
void libxsmm_generator_gather_scatter_rows_aarch64_microkernel( libxsmm_generated_code*                        io_generated_code,
    ...
    const libxsmm_mateltwise_kernel_config*        i_micro_kernel_config,
    const libxsmm_meltw_descriptor*                i_mateltwise_desc ) {
  unsigned int m, n, im = 0, m_remainder_elements, m_trips, max_m_unrolling = 4, m_unroll_factor = 1, n_unroll_factor = 4, m_trips_loop = 0, peeled_m_trips = 0;
  unsigned int idx_tsize = ((i_mateltwise_desc->flags & LIBXSMM_MELTW_FLAG_UNARY_IDX_SIZE_8BYTES) > 0 ) ? 8 : 4;
  ...
  if (m_trips_loop >= 1) {
    ...
    libxsmm_generator_gather_scatter_rows_aarch64_mn_loop_unrolled( io_generated_code, i_gp_reg_mapping, m_unroll_factor, n_unroll_factor,
        idx_vreg_start, idx_tsize, idx_mask_reg, ld_idx_mat, gather_instr, scatter_instr, vlen, 0,
    ...
    libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD,
                                                   gp_idx_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, gp_idx_mat_reg, (long long)n_unroll_factor * ld_idx_mat * dtype_size_reg_mat );
    ...
    libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_SUB,
                                                   gp_reg_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, gp_reg_mat_reg, (long long)n * ld_reg_mat * dtype_size_reg_mat - (long long) m_unroll_factor * vlen * dtype_size_reg_mat);
    libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_SUB,
                                                   gp_idx_mat_reg, i_gp_reg_mapping->gp_reg_scratch_0, gp_idx_mat_reg, (long long)n * ld_idx_mat * dtype_size_reg_mat );
  ...
  if (peeled_m_trips > 0) {
    ...
    for (im = 0; im < peeled_m_trips; im++) {
      unsigned int idx_vreg_id = idx_vreg_start + im;
      unsigned int l_idx_masked_elements = (im == peeled_m_trips - 1) ? m_remainder_elements : 0;
      unsigned int l_idx_mask_load = (im == peeled_m_trips - 1) ? ((m_remainder_elements > 0) ? idx_mask_reg : 0) : 0;
      libxsmm_generator_vloadstore_masked_vreg_aarch64( io_generated_code, i_gp_reg_mapping->gp_reg_ind_base, i_gp_reg_mapping->gp_reg_scratch_0, idx_vreg_id,
                                                        idx_tsize, l_idx_masked_elements, 1, 0, LIBXSMM_CAST_UCHAR(l_idx_mask_load) );
    }
    ...
    libxsmm_generator_gather_scatter_rows_aarch64_mn_loop_unrolled( io_generated_code, i_gp_reg_mapping, peeled_m_trips, n_unroll_factor,
        idx_vreg_start, idx_tsize, idx_mask_reg, ld_idx_mat, gather_instr, scatter_instr, vlen, m_remainder_elements,
        mask_reg, mask_reg_full_frac_vlen, is_gather, gp_idx_mat_reg, gp_reg_mat_reg, dtype_size_reg_mat, ld_reg_mat );
