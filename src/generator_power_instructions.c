...
unsigned int libxsmm_power_instruction_generic_2( unsigned int i_instr,
  ...
  if( i_instr == LIBXSMM_POWER_INSTR_VSX_XXBRD ||
    ...
    fprintf(stderr, "libxsmm_power_instruction_generic_2: unsupported instruction!\n");
    exit(-1);
  }
...
unsigned int libxsmm_power_instruction_generic_3( unsigned int i_instr,
  ...
  else if( i_instr == LIBXSMM_POWER_INSTR_FIP_ADDI ) {
    l_instr = libxsmm_power_instruction_fip_arithmetic( i_instr,
                                                        i_arg0,
                                                        i_arg1,
                                                        i_arg2 );
  }
  else if( i_instr == LIBXSMM_POWER_INSTR_FIP_ORI ) {
    l_instr = libxsmm_power_instruction_fip_logical( i_instr,
                                                     i_arg0,
                                                     i_arg1,
                                                     i_arg2 );
  }
  else if( i_instr == LIBXSMM_POWER_INSTR_FLP_LFD ||
           i_instr == LIBXSMM_POWER_INSTR_FLP_STFD ) {
    l_instr = libxsmm_power_instruction_flp_storage_access( i_instr,
                                                            i_arg0,
                                                            i_arg1,
                                                            i_arg2 );
  }
  else if( i_instr == LIBXSMM_POWER_INSTR_VEC_LVX ||
           i_instr == LIBXSMM_POWER_INSTR_VEC_STVX ) {
    l_instr = libxsmm_power_instruction_vec_storage_access( i_instr,
                                                            i_arg0,
                                                            i_arg1,
                                                            i_arg2 );
  }
  else if( i_instr == LIBXSMM_POWER_INSTR_VSX_LXVW4X ||
           i_instr == LIBXSMM_POWER_INSTR_VSX_STXVW4X ||
           i_instr == LIBXSMM_POWER_INSTR_VSX_LXVWSX ||
           i_instr == LIBXSMM_POWER_INSTR_VSX_LXVLL ||
           i_instr == LIBXSMM_POWER_INSTR_VSX_STXVLL ) {
    l_instr = libxsmm_power_instruction_vsx_storage_access( i_instr,
                                                            i_arg0,
                                                            i_arg1,
                                                            i_arg2 );
  }
  else if( i_instr == LIBXSMM_POWER_INSTR_VSX_XVMADDASP ) {
    l_instr = libxsmm_power_instruction_vsx_vector_bfp_madd( i_instr,
                                                             i_arg0,
                                                             i_arg1,
                                                             i_arg2 );
  }
  else {
    fprintf(stderr, "libxsmm_power_instruction_generic_3: unsupported instruction!\n");
    exit(-1);
  }
...
unsigned int libxsmm_power_instruction_generic_4( unsigned int i_instr,
  ...
  if( i_instr == LIBXSMM_POWER_INSTR_FIP_CMPI ) {
    ...
    fprintf(stderr, "libxsmm_power_instruction_generic_4: unsupported instruction!\n");
    exit(-1);
  }
...
void libxsmm_power_instruction_2( libxsmm_generated_code * io_generated_code,
  ...
  if ( io_generated_code->code_type > 1 ) {
    ...
    l_code[l_code_head] = libxsmm_power_instruction_generic_2( i_instr,
                                                               i_arg0,
                                                               i_arg1 );
    io_generated_code->code_size += 4;
  }
  else {
    fprintf(stderr, "libxsmm_power_instruction_2: inline/pure assembly print is not supported!\n");
    exit(-1);
  }
}
...
void libxsmm_power_instruction_3( libxsmm_generated_code * io_generated_code,
  ...
  if ( io_generated_code->code_type > 1 ) {
    unsigned int   l_code_head = io_generated_code->code_size / 4;
    unsigned int * l_code      = (unsigned int*) io_generated_code->generated_code;
    l_code[l_code_head] = libxsmm_power_instruction_generic_3( i_instr,
                                                               i_arg0,
                                                               i_arg1,
                                                               i_arg2 );
    io_generated_code->code_size += 4;
    ...
    fprintf(stderr, "libxsmm_power_instruction_3: inline/pure assembly print is not supported!\n");
    exit(-1);
  }
}
...
void libxsmm_power_instruction_4( libxsmm_generated_code * io_generated_code,
  ...
  if ( io_generated_code->code_type > 1 ) {
    unsigned int   l_code_head = io_generated_code->code_size / 4;
    unsigned int * l_code      = (unsigned int*) io_generated_code->generated_code;
    l_code[l_code_head] = libxsmm_power_instruction_generic_4( i_instr,
                                                               i_arg0,
                                                               i_arg1,
                                                               i_arg2,
                                                               i_arg3 );
    ...
    fprintf(stderr, "libxsmm_power_instruction_4: inline/pure assembly print is not supported!\n");
    exit(-1);
  }
}
