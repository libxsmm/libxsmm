...
void libxsmm_store_2d_reg_block( libxsmm_generated_code*                 io_generated_code,
  ...
  if ((i_mateltwise_desc->operation == LIBXSMM_MELTW_OPERATION_UNARY) && (i_mateltwise_desc->param == LIBXSMM_MELTW_TYPE_UNARY_UNPACK_TO_BLOCKS)) {
    ...
    vname = (i_vlen * i_micro_kernel_config->datatype_size_out == 32) ? 'z' : ( (i_vlen * i_micro_kernel_config->datatype_size_out == 16) ? 'y' : 'x' );
    for (in = 0; in < i_n_blocking; in++) {
      for (im = 0; im < i_m_blocking; im++) {
        cur_vreg = i_start_vreg + in * i_m_blocking + im;
        if (bcast_row == 1) {
        ...
        libxsmm_x86_instruction_vec_move( io_generated_code,
            ...
            ((i_mask_last_m_chunk == 1) && (im == i_m_blocking - 1)) ? i_mask_reg : 0,
            0, 1);
        libxsmm_x86_instruction_vec_compute_2reg_imm8( io_generated_code, LIBXSMM_X86_INSTR_VPSRAD_I, vname, i_micro_kernel_config->vec_tmp1, i_micro_kernel_config->vec_tmp1, 16 );
        libxsmm_x86_instruction_vec_move( io_generated_code,
            io_generated_code->arch,
            LIBXSMM_X86_INSTR_VPMOVDW,
            i_gp_reg_mapping->gp_reg_out,
            ...
            0, 1);
        if ( i_mateltwise_desc->param == LIBXSMM_MELTW_TYPE_UNARY_DECOMP_FP32_TO_BF16X3 ) {
          libxsmm_x86_instruction_vec_compute_2reg_imm8( io_generated_code, LIBXSMM_X86_INSTR_VPSRAD_I, vname, i_micro_kernel_config->vec_tmp2, i_micro_kernel_config->vec_tmp2, 16 );
          libxsmm_x86_instruction_vec_move( io_generated_code,
              io_generated_code->arch,
              LIBXSMM_X86_INSTR_VPMOVDW,
              i_gp_reg_mapping->gp_reg_out,
