...
/*  l_gp_reg_mapping.gp_reg_c_prefetch = LIBXSMM_X86_GP_REG_UNDEF;*/
  ...
  if ( l_simd_packed_iters_full > 0 ) {
    libxsmm_generator_packed_spgemm_csr_asparse_avx_avx2_avx512_n_loop( io_generated_code,
                                                                        i_xgemm_desc,
...
void libxsmm_generator_packed_spgemm_csr_asparse_avx_avx2_avx512_n_loop( libxsmm_generated_code*            io_generated_code,
  ...
  libxsmm_generator_packed_spgemm_csr_asparse_avx_avx2_avx512_m_loop( io_generated_code, i_xgemm_desc, io_loop_label_tracker, i_micro_kernel_config, i_gp_reg_mapping,
    ...
    if (l_row_elements > 0) {
      ...
      for ( l_z = 0; l_z < l_row_elements; l_z++ ) {
        ...
        libxsmm_x86_instruction_vec_move( io_generated_code,
                                          ...
                                          LIBXSMM_X86_GP_REG_UNDEF, 0,
                                          (i_row_idx[l_m] + l_z) * i_micro_kernel_config->datatype_size_in,
                                          i_micro_kernel_config->vector_name,
                                          i_num_c_cols, 0, 1, 0 );
        /* multiply with B */
        for ( l_n = 0; l_n < i_num_c_cols; l_n++ ) {
          l_b_offset = ((i_column_idx[i_row_idx[l_m] + l_z]*i_micro_kernel_config->datatype_size_in*i_packed_width*i_xgemm_desc->ldb)
                                                     +(l_n*i_packed_width*i_micro_kernel_config->datatype_size_in))-l_b_total_offset;
