...
void libxsmm_generator_mateqn_adjust_args_addr(libxsmm_generated_code*        io_generated_code,
  ...
  unsigned int output_tsize = 4 /*FIXME :libxsmm_typesize(i_mateqn_desc->datatype)*/;
  unsigned int temp_reg = i_gp_reg_mapping->temp_reg;
  unsigned int temp_reg2 = i_gp_reg_mapping->temp_reg2;
...
void libxsmm_generator_matequation_tmp_register_block_avx_avx512_kernel( libxsmm_generated_code*        io_generated_code,
  ...
  l_gp_reg_mapping.temp_reg    = LIBXSMM_X86_GP_REG_R10;
  l_gp_reg_mapping.temp_reg2   = LIBXSMM_X86_GP_REG_R11;
  l_gp_reg_mapping.gp_reg_m_loop = LIBXSMM_X86_GP_REG_R12;
  l_gp_reg_mapping.gp_reg_n_loop = LIBXSMM_X86_GP_REG_R13;

  temp_reg = l_gp_reg_mapping.temp_reg;
  ...
  while (cur_n != i_mateqn_desc->n) {
    ...
    while (cur_m != i_mateqn_desc->m) {
      ...
      libxsmm_generator_mateqn_2d_microkernel(io_generated_code, &l_loop_label_tracker, &l_gp_reg_mapping, &l_kernel_config, i_mateqn_desc, eqn, m_microkernel, n_microkernel);
      cur_m += m_microkernel;
