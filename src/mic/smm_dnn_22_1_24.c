...
void smm_dnn_22_1_24(const double* a, const double* b, double* c){
#ifdef __MIC__
int i;
__m512d xa0;
__m512d xa1;
...
__m512d xc0;
 ...
 xb23 = _MM512_MASK_LOADU_PD(&b[23+0],1);
for(i=0;i<22;i+=1){
    ...
    xc0=_mm512_mask3_fmadd_pd(xa23,xb23,xc0,1);
    _MM512_MASK_STOREU_PD(&c[i*1+0],xc0,1);
}
#else
printf("cppgemm_2_22_24_1\n");
for(int m=0;m<22;m++){
   for(int n=0;n<1;n++){
      for(int k=0;k<24;k++){
         c[m*1+n]+=a[m*24+k]*b[k*1+n];
      }
   }
}
#endif
