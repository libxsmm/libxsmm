...
void smm_dnn_13_7_24(const double* a, const double* b, double* c){
#ifdef __MIC__
int i;
__m512d xa0;
__m512d xa1;
...
__m512d xc0;
 ...
 xb22 = _MM512_MASK_LOADU_PD(&b[154+0],127);
 xb23 = _MM512_MASK_LOADU_PD(&b[161+0],127);
for(i=0;i<13;i+=1){
    ...
    xc0=_mm512_mask3_fmadd_pd(xa23,xb23,xc0,127);
    _MM512_MASK_STOREU_PD(&c[i*7+0],xc0,127);
}
#else
printf("cppgemm_2_13_24_7\n");
for(int m=0;m<13;m++){
   for(int n=0;n<7;n++){
      for(int k=0;k<24;k++){
         c[m*7+n]+=a[m*24+k]*b[k*7+n];
      }
   }
}
#endif
