...
void smm_dnn_19_2_24(const double* a, const double* b, double* c){
#ifdef __MIC__
int i;
__m512d xa0;
__m512d xa1;
...
__m512d xc0;
 ...
 xb22 = _MM512_MASK_LOADU_PD(&b[44+0],3);
 xb23 = _MM512_MASK_LOADU_PD(&b[46+0],3);
for(i=0;i<19;i+=1){
    ...
    xc0=_mm512_mask3_fmadd_pd(xa23,xb23,xc0,3);
    _MM512_MASK_STOREU_PD(&c[i*2+0],xc0,3);
}
#else
printf("cppgemm_2_19_24_2\n");
for(int m=0;m<19;m++){
   for(int n=0;n<2;n++){
      for(int k=0;k<24;k++){
         c[m*2+n]+=a[m*24+k]*b[k*2+n];
      }
   }
}
#endif
