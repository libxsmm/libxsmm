...
void smm_dnn_20_17_17(const double* a, const double* b, double* c){
#ifdef __MIC__
int i;
__m512d xa0;
__m512d xa1;
...
__m512d xc0;
 ...
 xb16 = _MM512_MASK_LOADU_PD(&b[272+0],255);
for(i=0;i<20;i+=1){
 ...
 xb16 = _MM512_MASK_LOADU_PD(&b[272+8],255);
for(i=0;i<20;i+=1){
 ...
 xb16 = _MM512_MASK_LOADU_PD(&b[272+16],1);
for(i=0;i<20;i+=1){
...
#else
printf("cppgemm_2_20_17_17\n");
for(int m=0;m<20;m++){
   for(int n=0;n<17;n++){
      for(int k=0;k<17;k++){
         c[m*17+n]+=a[m*17+k]*b[k*17+n];
      }
   }
}
#endif
