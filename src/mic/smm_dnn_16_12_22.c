...
void smm_dnn_16_12_22(const double* a, const double* b, double* c){
#ifdef __MIC__
int i;
__m512d xa0;
__m512d xa1;
...
__m512d xc0;
 ...
 xb21 = _MM512_MASK_LOADU_PD(&b[252+0],255);
for(i=0;i<16;i+=1){
 ...
 xb21 = _MM512_MASK_LOADU_PD(&b[252+8],15);
for(i=0;i<16;i+=1){
...
#else
printf("cppgemm_2_16_22_12\n");
for(int m=0;m<16;m++){
   for(int n=0;n<12;n++){
      for(int k=0;k<22;k++){
         c[m*12+n]+=a[m*22+k]*b[k*12+n];
      }
   }
}
#endif
