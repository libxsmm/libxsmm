...
void smm_dnn_11_4_24(const double* a, const double* b, double* c){
#ifdef __MIC__
int i;
__m512d xa0;
__m512d xa1;
...
__m512d xc0;
 ...
 xb21 = _MM512_MASK_LOADU_PD(&b[84+0],15);
 xb22 = _MM512_MASK_LOADU_PD(&b[88+0],15);
 xb23 = _MM512_MASK_LOADU_PD(&b[92+0],15);
for(i=0;i<11;i+=1){
    ...
    xc0=_mm512_mask3_fmadd_pd(xa23,xb23,xc0,15);
    _MM512_MASK_STOREU_PD(&c[i*4+0],xc0,15);
}
#else
printf("cppgemm_2_11_24_4\n");
for(int m=0;m<11;m++){
   for(int n=0;n<4;n++){
      for(int k=0;k<24;k++){
         c[m*4+n]+=a[m*24+k]*b[k*4+n];
      }
   }
}
#endif
