...
void smm_dnn_14_5_24(const double* a, const double* b, double* c){
#ifdef __MIC__
int i;
__m512d xa0;
__m512d xa1;
...
__m512d xc0;
 ...
 xb23 = _MM512_MASK_LOADU_PD(&b[115+0],31);
for(i=0;i<14;i+=1){
    ...
    xc0=_mm512_mask3_fmadd_pd(xa23,xb23,xc0,31);
    _MM512_MASK_STOREU_PD(&c[i*5+0],xc0,31);
}
#else
printf("cppgemm_2_14_24_5\n");
for(int m=0;m<14;m++){
   for(int n=0;n<5;n++){
      for(int k=0;k<24;k++){
         c[m*5+n]+=a[m*24+k]*b[k*5+n];
      }
   }
}
#endif
