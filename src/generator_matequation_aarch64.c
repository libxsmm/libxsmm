  ...
  for (eqn_tree_id = 0; eqn_tree_id < queue_size; eqn_tree_id++) {
    ...
    if (eqn_tree_id == (queue_size - 1)) {
      ...
      if (cur_eqn->eqn_root->tmp.id >= 0) {
        ...
        libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD, temp_reg, l_gp_reg_mapping.temp_reg2, temp_reg, (long long)arg_tmp_id*48);
