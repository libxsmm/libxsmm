...
void libxsmm_generator_matequation_set_input_in_stack_param_struct_aarch64( libxsmm_generated_code*   io_generated_code,
  ...
  if (cur_node->type == LIBXSMM_MATRIX_EQN_NODE_ARG) {
    if (cur_node->info.arg.in_pos >= 0) {
      ...
      libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD, temp_reg, i_gp_reg_mapping->gp_reg_scratch_0, temp_reg, (long long)cur_node->info.arg.in_pos*((int)sizeof(libxsmm_matrix_arg)) );
  ...
  if ((cur_node->up->type == LIBXSMM_MATRIX_EQN_NODE_UNARY) &&
    ...
    if ((cur_node->type == LIBXSMM_MATRIX_EQN_NODE_ARG) && (cur_node->info.arg.in_pos >= 0)) {
      libxsmm_aarch64_instruction_alu_move( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_LDR_I_OFF, i_gp_reg_mapping->gp_reg_param_struct, LIBXSMM_AARCH64_GP_REG_UNDEF, 8, temp_reg );
      libxsmm_aarch64_instruction_alu_compute_imm64( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_META_ADD, temp_reg, i_gp_reg_mapping->gp_reg_scratch_0, temp_reg, (long long)cur_node->info.arg.in_pos*((int)sizeof(libxsmm_matrix_arg))+8 );
      libxsmm_aarch64_instruction_alu_move( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_LDR_I_OFF, temp_reg, LIBXSMM_AARCH64_GP_REG_UNDEF, 0, temp_reg );
...
void libxsmm_generator_matequation_set_output_in_stack_param_struct_aarch64(libxsmm_generated_code*   io_generated_code,
  ...
  if (cur_node->type == LIBXSMM_MATRIX_EQN_NODE_UNARY) {
    ...
    libxsmm_generator_meqn_setval_stack_var_aarch64( io_generated_code, LIBXSMM_MEQN_STACK_VAR_PARAM_STRUCT_PTR22, i_gp_reg_mapping->gp_reg_scratch_0, temp_reg );
  } else {
    /* Should not happen */
  }
  ...
  if ((cur_node->type == LIBXSMM_MATRIX_EQN_NODE_UNARY) &&
    ...
    if (is_last_op > 0) {
      ...
      libxsmm_generator_meqn_setval_stack_var_aarch64( io_generated_code, LIBXSMM_MEQN_STACK_VAR_PARAM_STRUCT_PTR11, i_gp_reg_mapping->gp_reg_scratch_0, temp_reg );
    } else {
...
void libxsmm_generator_matequation_tmp_stack_scratch_aarch64_kernel( libxsmm_generated_code* io_generated_code,
  ...
  for (timestamp = 0; timestamp <= last_timestamp; timestamp++) {
...
#if 0
    libxsmm_datatype out_precision = (timestamp == last_timestamp) ? (libxsmm_datatype) LIBXSMM_MEQN_GETENUM_OUT_PREC(i_mateqn_desc->datatype) : cur_op->tmp.dtype;
    ...
    if (timestamp == last_timestamp) {
      out_precision = (libxsmm_datatype) LIBXSMM_MEQN_GETENUM_OUT_PREC(i_mateqn_desc->datatype);
    ...
    if (((cur_op->type == LIBXSMM_MATRIX_EQN_NODE_BINARY) && (cur_op->info.b_op.type == LIBXSMM_MELTW_TYPE_BINARY_MATMUL)) ||
      ...
      if (cur_op->type == LIBXSMM_MATRIX_EQN_NODE_TERNARY) {
        ...
        if ((cur_op->type == LIBXSMM_MATRIX_EQN_NODE_UNARY) && (cur_op->info.u_op.type == LIBXSMM_MELTW_TYPE_UNARY_DUMP)) {
          ...
          libxsmm_aarch64_instruction_alu_move( io_generated_code, LIBXSMM_AARCH64_INSTR_GP_LDR_I_OFF, temp_reg, LIBXSMM_AARCH64_GP_REG_UNDEF, cur_op->info.u_op.op_arg_pos*((int)sizeof(libxsmm_matrix_op_arg)), temp_reg );
          libxsmm_generator_meqn_setval_stack_var_aarch64( io_generated_code, LIBXSMM_MEQN_STACK_VAR_PARAM_STRUCT_PTR11, i_gp_reg_mapping->gp_reg_scratch_0, temp_reg );
        }
