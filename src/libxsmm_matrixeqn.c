...
LIBXSMM_API_INTERN libxsmm_blasint libxsmm_matrix_eqn_can_overwrite_unary_input(libxsmm_matrix_eqn_elem* cur_node) {
  ...
  if (((cur_node->le->tmp.dtype == LIBXSMM_DATATYPE_BF16) || (cur_node->le->tmp.dtype == LIBXSMM_DATATYPE_BF8) || (cur_node->le->tmp.dtype == LIBXSMM_DATATYPE_HF8)) && (cur_node->tmp.dtype == LIBXSMM_DATATYPE_F32)) {
    result = 0;
  }
