...
# endif
      ...
      { /* calibrate timer */
        ...
        if (0 != info.constant_tsc && t0 != t1 && 0.0 < libxsmm_timer_scale) {
          const libxsmm_timer_tickint dt = LIBXSMM_DELTA(t0, t1);
...
# endif
        {
          ...
          /* adopt scheme which allows kernel names of LIBXSMM to appear in order (Intel VTune, etc.) */
          LIBXSMM_SNPRINTF(jit_name, sizeof(jit_name), "libxsmm_%s_%s_%c%c_%ux%ux%u_%u_%u_%u_w%i_a%i_b%i_p%i_nnz%u.scsoa", target_arch, tname,
...
# endif
    ...
    case LIBXSMM_BUILD_KIND_SREG: { /* sparse register kernel */
...
#endif
      {
        ...
        nflops = 2 * libxsmm_cpuid_vlen32(libxsmm_target_archid)/2 * request->descriptor.sreg->gemm->n * nnz;
