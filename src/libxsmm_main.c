...
LIBXSMM_API LIBXSMM_ATTRIBUTE_DTOR void libxsmm_finalize(void)
{
  ...
  if (NULL != registry) {
    int i;
#if (0 != LIBXSMM_SYNC)
    if (NULL != registry && LIBXSMM_LOCK_TRYLOCK(LIBXSMM_LOCK, &libxsmm_lock_global) && NULL != registry) {
# if (1 < INTERNAL_REGLOCK_MAXN)
...
# endif
        {
          ...
          if ( kernabi == 0 ) {
            ...
            LIBXSMM_SNPRINTF(jit_name, sizeof(jit_name), "libxsmm_abi%i_%s_%s_%c%c_%ux%ux%u_%u_%u_%u_a%i_b%i_p%i_br%i_uh%u_si%i_tc-%s_avnni%i_bvnni%i_cvnni%i.mxm", kernabi, target_arch, tname,
