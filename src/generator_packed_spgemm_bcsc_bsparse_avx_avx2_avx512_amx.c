...
void libxsmm_generator_packed_spgemm_bcsc_bsparse_avx_avx2_avx512_amx( libxsmm_generated_code*         io_generated_code,
  ...
  if (l_is_AT_CT_kernel == 0) {
    ...
    l_simd_packed_remainder = i_bk % l_simd_packed_width;
  ...
  if (l_emit_compute > 0) {
    ...
    while ( l_packed_done != l_simd_packed_iters ) {
      ...
      if (l_is_AT_CT_kernel > 0) {
        ...
        libxsmm_x86_instruction_alu_imm( io_generated_code, LIBXSMM_X86_INSTR_IMUL, l_cur_column_gpr, i_bn * l_micro_kernel_config.datatype_size_out );
  ...
  if (l_is_amx_kernel > 0) {
    ...
    if ( (((LIBXSMM_GEMM_FLAG_NO_RESET_TILECONFIG & i_xgemm_desc->flags) == 0) && ((LIBXSMM_GEMM_FLAG_NO_SETUP_TILECONFIG & i_xgemm_desc->flags) != 0)) ||
      ...
      libxsmm_x86_instruction_tile_control( io_generated_code, 0, io_generated_code->arch,
          LIBXSMM_X86_INSTR_TILERELEASE, LIBXSMM_X86_GP_REG_UNDEF, 0, NULL );
    }
    /* free stack */
    if (l_emit_compute > 0){
      if (LIBXSMM_DATATYPE_BF16 == LIBXSMM_GEMM_GETENUM_C_PREC( i_xgemm_desc->datatype )) {
...
void libxsmm_generator_packed_spgemm_bcsc_bsparse_kloop_bfdot_avx512(libxsmm_generated_code*            io_generated_code,
  ...
  if ((i_xgemm_desc->flags & LIBXSMM_GEMM_FLAG_TRANS_A) > 0) {
    l_is_AT_CT_kernel = 1;
    l_simd_packed_remainder = i_bk % i_simd_packed_width;
  }
  ...
  for (l_n_block_id = 0; l_n_block_id < l_n_blocks; l_n_block_id++) {
    ...
    if (l_is_AT_CT_kernel > 0) {
      ...
      libxsmm_x86_instruction_alu_imm( io_generated_code, LIBXSMM_X86_INSTR_IMUL, i_gp_reg_mapping->gp_reg_kloop, i_bk * i_micro_kernel_config->datatype_size_in);
    ...
    if (l_is_AT_CT_kernel > 0) {
      for (l_fma_i = 0; l_fma_i < l_fma_iters; l_fma_i++) {
        ...
        for ( l_p = 0; l_p < i_packed_blocking; l_p++ ) {
          libxsmm_x86_instruction_unified_vec_move( io_generated_code,
            ...
            LIBXSMM_X86_GP_REG_UNDEF, 0,
            ((1ull * i_packed_processed * i_xgemm_desc->lda) + l_fma_i * i_simd_packed_width + (l_p * i_xgemm_desc->lda))* i_micro_kernel_config->datatype_size_in,
            i_micro_kernel_config->vector_name, l_max_reg_block+l_p,
            ((l_simd_packed_remainder > 0) && (l_fma_i == l_fma_iters - 1)) ? 1 : 0,
            ((l_simd_packed_remainder > 0) && (l_fma_i == l_fma_iters - 1)) ? l_input_bf16_mask : 0, 0 );
        }
        ...
        for ( l_n_in_block = 0; l_n_in_block < l_n_cols_kernel; l_n_in_block++ ) {
          ...
          libxsmm_x86_instruction_unified_vec_move( io_generated_code,
            ...
            LIBXSMM_X86_GP_REG_UNDEF, 0,
            ((long long) i_bk * l_n_in_block + l_fma_i * i_simd_packed_width + l_n_block_id * l_n_cols_kernel * i_bk) * i_micro_kernel_config->datatype_size_in,
            i_micro_kernel_config->vector_name, l_max_reg_block+i_packed_blocking,
            ((l_simd_packed_remainder > 0) && (l_fma_i == l_fma_iters - 1)) ? 1 : 0,
            ((l_simd_packed_remainder > 0) && (l_fma_i == l_fma_iters - 1)) ? l_input_bf16_mask : 0, 0 );
      ...
      if ( l_assm_fma_iters > 1 ) {
        libxsmm_x86_instruction_alu_imm( io_generated_code, i_micro_kernel_config->alu_add_instruction, i_gp_reg_mapping->gp_reg_help_2, l_fma_iters * l_vnni_block_size * i_micro_kernel_config->datatype_size_in);
        libxsmm_x86_instruction_alu_imm( io_generated_code, i_micro_kernel_config->alu_add_instruction, i_gp_reg_mapping->gp_reg_kloop, l_fma_iters * i_packed_width * l_vnni_block_size * i_micro_kernel_config->datatype_size_in);
    ...
    if (l_is_AT_CT_kernel > 0) {
      ...
      for ( l_n = 0; l_n < l_n_cols_kernel; l_n++ ) {
        for ( l_p = 0; l_p < i_packed_blocking; l_p++ ) {
          unsigned int l_reg0 = (l_n*i_packed_blocking) + l_p;
          libxsmm_generator_hinstrps_avx512( io_generated_code, LIBXSMM_X86_INSTR_VADDPS, l_reg0, l_max_reg_block+1, l_max_reg_block);
          ...
          if (l_beta_0 == 0) {
            libxsmm_x86_instruction_vec_move( io_generated_code,
                ...
                l_c_vname,
                l_max_reg_block+i_packed_blocking, 0, 1, 0 );
            libxsmm_x86_instruction_vec_compute_3reg( io_generated_code, LIBXSMM_X86_INSTR_VADDPS, i_micro_kernel_config->vector_name, l_reg0, l_max_reg_block+i_packed_blocking, l_reg0);
          }
  ...
  if ( i_packed_range/i_packed_blocking > 1 ) {
    if (l_is_AT_CT_kernel > 0) {
      ...
      libxsmm_x86_instruction_alu_imm( io_generated_code, i_micro_kernel_config->alu_add_instruction, i_gp_reg_mapping->gp_reg_a, (long long)i_packed_blocking*i_xgemm_desc->lda*l_vnni_block_size*i_micro_kernel_config->datatype_size_in );
