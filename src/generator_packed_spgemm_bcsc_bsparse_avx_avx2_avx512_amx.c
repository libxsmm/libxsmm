  ...
  if ((LIBXSMM_DATATYPE_BF16 == LIBXSMM_GETENUM_INP( i_xgemm_desc->datatype ) || LIBXSMM_DATATYPE_I8 == LIBXSMM_GETENUM_INP( i_xgemm_desc->datatype ) ) &&
    ...
    if (l_is_amx_kernel > 0) {
      ...
      if (l_emit_compute > 0) {
        ...
        } else if (LIBXSMM_DATATYPE_I32 == LIBXSMM_GETENUM_OUT( i_xgemm_desc->datatype )) {
...
void libxsmm_generator_packed_spgemm_bcsc_bsparse_kloop_bfdot_avx512(libxsmm_generated_code*            io_generated_code,
  ...
  char l_c_vname = ( LIBXSMM_DATATYPE_BF16 == LIBXSMM_GETENUM_OUT( i_xgemm_desc->datatype ) ) ? 'y' : i_micro_kernel_config->vector_name;
...
void libxsmm_generator_packed_spgemm_bcsc_bsparse_kloop_amx(         libxsmm_generated_code*            io_generated_code,
  ...
  if (l_beta_0 > 0) {
    ...
    if ( LIBXSMM_DATATYPE_I32 == LIBXSMM_GETENUM_OUT( i_xgemm_desc->datatype ) ) {
      for ( l_n_tile = 0; l_n_tile < l_n_blocking*l_n_tiles_bn; l_n_tile++) {
        for ( l_p = 0; l_p < i_packed_blocking; l_p++ ) {
          ...
          libxsmm_x86_instruction_tile_move( io_generated_code,
                  ...
                  4,
                  ((1ull * i_packed_processed * i_simd_packed_width) +
                  (1ull * i_packed_width * l_n_tile * (i_bn/l_n_tiles_bn)) + (l_p * i_simd_packed_width)) * (i_micro_kernel_config->datatype_size_out),
                  l_tile);
        }
      }
        ...
        for ( _l_n = 0; _l_n < ((i_bn+1)/2)/l_n_tiles_bn; _l_n++) {
          ...
          for ( l_p = 0; l_p < i_packed_blocking; l_p++ ) {
            ...
            libxsmm_x86_instruction_vec_move( io_generated_code,
                ...
                LIBXSMM_X86_GP_REG_UNDEF, 0,
                ((1ull * i_packed_processed * i_simd_packed_width ) +
                 (1ull * i_packed_width * l_n * 2) + (l_p * i_simd_packed_width)) * (i_micro_kernel_config->datatype_size_out),
                'y', l_reg0,
            ...
            if (i_bn != 1) {
              ...
              libxsmm_x86_instruction_vec_move( io_generated_code,
                  ...
                  LIBXSMM_X86_GP_REG_UNDEF, 0,
                  ((1ull * i_packed_processed * i_simd_packed_width) +
                   (1ull * i_packed_width * l_n * 2 + i_packed_width) + (l_p * i_simd_packed_width)) * (i_micro_kernel_config->datatype_size_out),
                  'y', l_reg1,
                  ((i_packed_remainder > 0) && (l_p == i_packed_blocking - 1)) ? l_output_bf16_mask : 0, 1, 0 );
              /* up-convert bf16 to fp32 */
              libxsmm_generator_cvtbf16ps_avx2_avx512( io_generated_code, 'z', l_reg1, l_reg1 );
  ...
  for ( l_n = 0; l_n < l_n_blocking * l_n_tiles_bn; l_n++ ) {
    ...
    for ( l_p = 0; l_p < i_packed_blocking; l_p++ ) {
      unsigned int tile_comp_instr = ( LIBXSMM_DATATYPE_BF16 == LIBXSMM_GETENUM_INP( i_xgemm_desc->datatype ) ) ? LIBXSMM_X86_INSTR_TDPBF16PS : LIBXSMM_X86_INSTR_TDPBSSD;
      unsigned int tile_a = l_a_tile_offset + l_p;
