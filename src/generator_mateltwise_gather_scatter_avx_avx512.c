...
void libxsmm_generator_gather_scatter_offs_avx_avx512_microkernel( libxsmm_generated_code*                        io_generated_code,
  ...
  if (io_generated_code->arch >= LIBXSMM_X86_AVX512) {
    ...
    ones_mask_reg = 13;
    help_mask_reg = 12;
    libxsmm_generator_mateltwise_initialize_avx_mask(io_generated_code, ones_mask_reg, 8);
  }
  ...
  if (use_m_masking == 1) {
    if (io_generated_code->arch >= LIBXSMM_X86_AVX512) {
      ...
      if (i_micro_kernel_config->datatype_size_in == 4) {
        if (idx_tsize == 8) {
          libxsmm_generator_mateltwise_initialize_avx512_mask(io_generated_code, LIBXSMM_X86_GP_REG_RAX, mask_reg, mask_inout_count, LIBXSMM_DATATYPE_F64);
        } else {
          libxsmm_generator_mateltwise_initialize_avx512_mask(io_generated_code, LIBXSMM_X86_GP_REG_RAX, mask_reg, mask_inout_count, LIBXSMM_DATATYPE_F32);
        }
...
void libxsmm_generator_gather_scatter_cols_avx_avx512_microkernel( libxsmm_generated_code*                        io_generated_code,
  ...
  if ((n_pf_iters > 0) && (pf_dist > 0)) {
    ...
    libxsmm_generator_gather_scatter_cols_avx_avx512_m_loop( io_generated_code, io_loop_label_tracker, i_gp_reg_mapping, i_micro_kernel_config, m_trips_loop, m_unroll_factor, peeled_m_trips,
        vload_instr, vstore_instr, pf_instr, vlen, vname, use_m_masking, mask_reg, pf_dist,
  ...
  libxsmm_generator_gather_scatter_cols_avx_avx512_m_loop( io_generated_code, io_loop_label_tracker, i_gp_reg_mapping, i_micro_kernel_config, m_trips_loop, m_unroll_factor, peeled_m_trips,
      vload_instr, vstore_instr, pf_instr, vlen, vname, use_m_masking, mask_reg, 0,
      is_gather, gp_idx_mat_reg, gp_idx_mat_pf_reg, gp_reg_mat_reg, dtype_size_idx_mat, dtype_size_reg_mat );
