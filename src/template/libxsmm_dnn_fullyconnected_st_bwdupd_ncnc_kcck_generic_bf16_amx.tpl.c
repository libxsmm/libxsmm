...
bwd_tile_config_kernel(NULL, NULL, NULL);
  ...
  for ( mb1ofm1 = eltwise_thr_begin; mb1ofm1 < eltwise_thr_end; ++mb1ofm1 ) {
    ...
    for ( iteri = 0; iteri < handle->bn; ++iteri ) {
      for ( iterj = 0; iterj < handle->bk; iterj += 32 ) {
        ...
        __mmask32 relumask_reg = LIBXSMM_INTRINSICS_MM512_LOAD_MASK32 (&LIBXSMM_VLA_ACCESS(4, relubitmask, mb1, ofm1, iteri, iterj/32, nBlocksOFm, handle->bn, handle->bk/32));
        cur_out_reg = _mm512_mask_blend_epi16 (relumask_reg, zero_reg, cur_out_reg);
#endif
#ifdef LIBXSMM_DNN_FC_BWD_FUSE_SIGMOID
...
#ifdef USE_CLDEMOTE
        _mm_cldemote(&LIBXSMM_VLA_ACCESS(4, doutput, mb1, ofm1, iteri, iterj, nBlocksOFm, handle->bn, handle->bk));
#endif
      }
    }
    ...
    if (BF > 1) {
      for ( ofm1 = 0; ofm1 < BF; ++ofm1 ) {
        for (ifm1 = my_in_start; ifm1 < my_in_end; ++ifm1) {
          for (mb1 = my_im_start; mb1 < my_im_end; ++mb1) {
            ...
            prefetchwt_chunk((char*)&LIBXSMM_VLA_ACCESS(4, dinput_f32,    mb1,  ifm1, 0, 0, nBlocksIFm, bn, bc), handle->bn*handle->bc*sizeof(float));
            if ( ofm1 == BF-1  ) {
              prefetchwt_chunk((char*)&LIBXSMM_VLA_ACCESS(4, dinput,    mb1,  ifm1, 0, 0, nBlocksIFm, bn, bc), handle->bn*handle->bc*sizeof(libxsmm_bfloat16));
            }
#endif
      ...
      for (ifm1 = my_in_start; ifm1 < my_in_end; ++ifm1) {
        for (mb1 = my_im_start; mb1 < my_im_end; ++mb1) {
          ...
          prefetchwt_chunk((char*)&LIBXSMM_VLA_ACCESS(4, dinput,    mb1,  ifm1, 0, 0, nBlocksIFm, bn, bc), handle->bn*handle->bc*sizeof(libxsmm_bfloat16));
#endif
          bf16_batchreduce_kernel_bwd_zerobeta( &LIBXSMM_VLA_ACCESS(5, filter_tr, ifm1, 0, 0, 0, 0, nBlocksOFm, bk_lp, bc, lpb),
  ...
  if (use_2d_blocking == 1) {
...
#ifdef WR_PREFETCH_OUTPUT
          prefetchwt_chunk((char*)&LIBXSMM_VLA_ACCESS(5, dfilter, ofm1, ifm1, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb), bbc*bbk*sizeof(libxsmm_bfloat16));
#endif
          bf16_batchreduce_kernel_upd_zerobeta(&LIBXSMM_VLA_ACCESS(5, doutput_tr, ofm1, 0, 0, ofm2*bbk, 0, nBlocksMB, bn_lp, bk, lpb), &LIBXSMM_VLA_ACCESS(4, input_tr, ifm1, 0, ifm2*bbc, 0, nBlocksMB, bc, bn), &LIBXSMM_VLA_ACCESS(5, dfilter, ofm1, ifm1, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb), &blocks);
        }
      }
    } else {
...
#ifdef WR_PREFETCH_OUTPUT
            prefetchwt_chunk((char*)&LIBXSMM_VLA_ACCESS(4, dfilter_f32, ofm1, ifm1, 0, 0, nBlocksIFm, bc, bk), bbc*bbk*sizeof(float));
            if ( bfn == BF-1  ) {
              prefetchwt_chunk((char*)&LIBXSMM_VLA_ACCESS(5, dfilter, ofm1, ifm1, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb), bbc*bbk*sizeof(libxsmm_bfloat16));
            }
#endif
...
#ifdef USE_CLDEMOTE
                    _mm_cldemote(&LIBXSMM_VLA_ACCESS(5, dfilter, ofm1, ifm1, (ifm2*bbc+jc)/lpb, ofm2*bbk+jk, 0, nBlocksIFm, bc_lp, bk, lpb));
#endif
                  }
                }
              } else {
    ...
    if (BF == 1) {
      for ( ifm1ofm1 = thr_begin; ifm1ofm1 < thr_end; ++ifm1ofm1 ) {
        ...
        prefetchwt_chunk((char*)&LIBXSMM_VLA_ACCESS(5, dfilter, ofm1, ifm1, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb), bbc*bbk*sizeof(libxsmm_bfloat16));
#endif
        bf16_batchreduce_kernel_upd_zerobeta(&LIBXSMM_VLA_ACCESS(5, doutput_tr, ofm1, 0, 0, ofm2*bbk, 0, nBlocksMB, bn_lp, bk, lpb), &LIBXSMM_VLA_ACCESS(4, input_tr, ifm1, 0, ifm2*bbc, 0, nBlocksMB, bc, bn), &LIBXSMM_VLA_ACCESS(5, dfilter, ofm1, ifm1, (ifm2*bbc)/lpb, ofm2*bbk, 0, nBlocksIFm, bc_lp, bk, lpb), &blocks);
      }
    } else {
      for (bfn = 0; bfn < BF; bfn++) {
        for ( ifm1ofm1 = thr_begin; ifm1ofm1 < thr_end; ++ifm1ofm1 ) {
...
#ifdef WR_PREFETCH_OUTPUT
          prefetchwt_chunk((char*)&LIBXSMM_VLA_ACCESS(4, dfilter_f32, ofm1, ifm1, ifm2*bbc, ofm2*bbk, nBlocksIFm, bc, bk), bbc*bbk*sizeof(float));
          if ( bfn == BF-1  ) {
            prefetchwt_chunk((char*)&LIBXSMM_VLA_ACCESS(5, dfilter, ofm1, ifm1, (ifm2*bbc)/lpb, ofm2*bbk, 0, nBlocksIFm, bc_lp, bk, lpb), bbc*bbk*sizeof(libxsmm_bfloat16));
          }
#endif
          batchreduce_kernel_upd(&LIBXSMM_VLA_ACCESS(5, doutput_tr, ofm1, bfn*blocks, 0, ofm2*bbk, 0, nBlocksMB, bn_lp, bk, lpb), &LIBXSMM_VLA_ACCESS(4, input_tr, ifm1, bfn*blocks, ifm2*bbc, 0, nBlocksMB, bc, bn), &LIBXSMM_VLA_ACCESS(4, dfilter_f32, ofm1, ifm1, ifm2*bbc, ofm2*bbk, nBlocksIFm, bc, bk), &blocks);
          /* Downconvert result to BF16 and vnni format */
