...
if ( (handle->options & LIBXSMM_DNN_CONV_OPTION_BWD_NO_FILTER_TRANSPOSE) == 0 ) {
  if ((handle->ifmblock % 16 == 0) && (handle->ofmblock % 16 == 0)) {
    for (task = transpose_thr_begin; task < transpose_thr_end; ++task) {
      ...
      bf16_vnni_transpose_kernel(  &LIBXSMM_VLA_ACCESS(7, wt, ofm1, ifm1, kj, ki, 0, 0, 0, handle->blocksifm, handle->desc.R, handle->desc.S, ifmblock_lp, handle->ofmblock, lpb),
                  &LIBXSMM_VLA_ACCESS(7, tr_wt, ifm1, ofm1, handle->desc.R-1-kj , handle->desc.S-1-ki, 0, 0, 0, handle->blocksofm, handle->desc.R, handle->desc.S, ofmblock_lp, handle->ifmblock, lpb),
                  handle->ifmblock, handle->ofmblock, handle->ifmblock, handle->ofmblock);
    }
  } else {
    for (task = transpose_thr_begin; task < transpose_thr_end; ++task) {
      ifm1 = task/(handle->blocksofm * handle->desc.R * handle->desc.S);
        ...
        for (ifm1 = ifmb; ifm1 < LIBXSMM_MIN(ifmb+handle->block_bwd_ifm, my_ifm_end); ifm1++) {
          for (oj = ojb; oj < LIBXSMM_MIN(ojb+handle->block_bwd_oj,handle->ofh); oj += handle->bwd_ofh_rb) {
            for (oi = 0; oi < handle->ofw; oi += handle->bwd_ofw_rb) {
              ...
              br_gemm_kernel_strd( &LIBXSMM_VLA_ACCESS(7, weight, ifm1, 0, 0, 0, 0, 0, 0, handle->blocksofm, handle->desc.R, handle->desc.S, ofmblock_lp, handle->ifmblock, lpb),
                  &LIBXSMM_VLA_ACCESS(5,  output,  img, 0, oj, oi, 0, handle->blocksofm, handle->ofhp, handle->ofwp, handle->ofmblock), out_ptr, &n_blocks);
      ...
      for (ojb = 0; ojb < handle->ofh; ojb += handle->block_bwd_oj) {
        for (ifm1 = ifmb; ifm1 < LIBXSMM_MIN(ifmb+handle->block_bwd_ifm, my_ifm_end); ifm1++) {
          for (oj = ojb; oj < LIBXSMM_MIN(ojb+handle->block_bwd_oj,handle->ofh); oj += handle->bwd_ofh_rb) {
            for (oi = 0; oi < handle->ofw; oi += handle->bwd_ofw_rb) {
              /* Batch-reduce GEMM call  */
              br_gemm_kernel_offs( &LIBXSMM_VLA_ACCESS(7, weight, ifm1, 0, 0, 0, 0, 0, 0, handle->blocksofm, handle->desc.R, handle->desc.S, ofmblock_lp, handle->ifmblock, lpb),
                  &LIBXSMM_VLA_ACCESS(5,  output,  img, 0, oj, oi, 0, handle->blocksofm, handle->ofhp, handle->ofwp, handle->ofmblock), out_ptr, &n_blocks, handle->A_offsets_bwd, handle->B_offsets_bwd);
              /* Downconvert accumulated tiles to BF16  */
              for (ojj = 0; ojj < handle->bwd_ofh_rb; ojj++) {
                LIBXSMM_DNN_CONVERT_BUFFER_F32_BF16( &LIBXSMM_VLA_ACCESS( 3, scratch_fp32, ojj, 0, 0, scratch_ifwp, handle->ifmblock), &LIBXSMM_VLA_ACCESS( 5, del_input, img, ifm1, oj+ojj, oi, 0, handle->blocksifm, IFH, IFW, handle->ifmblock), handle->bwd_ofw_rb * handle->ifmblock);
              }
                      ...
                      for (ki = 0; ki < handle->desc.S; ki++) {
                        ...
                        if (kj == 0 && oj == 0) {
                          ...
                          if (handle->avoid_acc_load_bwd == 1) {
                            for (ojj = 0; ojj < handle->bwd_ofh_rb; ojj++) {
                              LIBXSMM_DNN_CONVERT_BUFFER_F32_BF16( &LIBXSMM_VLA_ACCESS( 3, scratch_fp32, ojj, 0, 0, ifwp_scratch, handle->ifmblock),
