  ...
  for ( unsigned int l_n = 0; l_n < l_n_blocks; ++l_n ) {
    ...
    for ( unsigned int l_m = 0; l_m < l_m_blocks; ++l_m ) {
      ...
      for ( unsigned int l_vec = 0; l_vec < l_n_ele; ++l_vec ) {
        unsigned int l_a = l_a_ptr[l_vec_ele*l_n + l_vec];
        long l_offset = l_m*l_vec_len + l_offsets[l_vec_ele*l_n + l_vec];
        libxsmm_ppc64le_instr_load( io_generated_code, l_a, l_offset, l_scratch[l_vec]);
      }
    ...
    if ( !l_packed ) {
      ...
      for ( unsigned int l_vec = 0; l_vec < l_n_ele; ++l_vec ) {
        unsigned int l_a = l_a_ptr[l_vec_ele*l_n + l_vec];
        long l_offset = l_m_blocks*l_vec_len + l_offsets[l_vec_ele*l_n + l_vec];
        libxsmm_ppc64le_instr_load_part( io_generated_code, io_reg_tracker, l_a, l_offset, l_m_part, l_scratch[l_vec] );
      }
...
void libxsmm_generator_gemm_mma_block_store_acc( libxsmm_generated_code *io_generated_code,
  ...
  for ( unsigned int l_n = 0; l_n < (i_n + l_acc_vec - 1 )/ l_acc_vec ; ++l_n ) {
    for ( unsigned int l_m = 0; l_m < l_m_blocks; ++l_m ) {
      unsigned int l_t = io_t[l_n*i_ldt + l_m];
