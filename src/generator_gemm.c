...
void libxsmm_generator_gemm_kernel( libxsmm_generated_code*        io_generated_code,
  ...
  if ( (LIBXSMM_DATATYPE_I16S == LIBXSMM_GETENUM_OUT( l_xgemm_desc_mod.datatype )) ||
  ...
  if ( io_generated_code->arch <= LIBXSMM_TARGET_ARCH_GENERIC ) {
  ...
  } else if ( ( io_generated_code->arch >= LIBXSMM_X86_AVX2 ) && ( io_generated_code->arch <= LIBXSMM_X86_AVX2_ADL ) && ( (LIBXSMM_DATATYPE_I16S  == LIBXSMM_GETENUM_INP( l_xgemm_desc_mod.datatype )) ) ) {
    l_vector_length = 8;
    ...
    l_vector_length = 16;
              ...
              ( LIBXSMM_DATATYPE_I16S == LIBXSMM_GETENUM_INP( l_xgemm_desc_mod.datatype ) ) ) {
    l_vector_length = 8;
    /* some checks as we cannot mask everything */
  ...
  if ( (l_xgemm_desc_mod.flags & LIBXSMM_GEMM_FLAG_TRANS_B) > 0 ) {
    if ( (LIBXSMM_DATATYPE_I16S  == LIBXSMM_GETENUM_INP( l_xgemm_desc_mod.datatype )) ||
         (LIBXSMM_DATATYPE_I8   == LIBXSMM_GETENUM_INP( l_xgemm_desc_mod.datatype ))    ) {
