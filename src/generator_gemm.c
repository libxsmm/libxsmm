...
void libxsmm_generator_gemm_kernel( libxsmm_generated_code*        io_generated_code,
  ...
  if ( (l_is_Amxfp8_Bmxfp8_gemm > 0) || (l_is_Amxfp6_Bmxfp6_gemm > 0) || (l_is_Amxfp4_Bmxfp4_gemm > 0) ) {
  ...
  if ( io_generated_code->arch <= LIBXSMM_TARGET_ARCH_GENERIC ) {
    ...
    if( LIBXSMM_DATATYPE_F32 == LIBXSMM_GEMM_GETENUM_AB_COMMON_PREC(l_xgemm_desc_mod.datatype) &&
        ((i_xgemm_desc->flags == LIBXSMM_GEMM_FLAG_USE_XGEMM_ABI ) ||
         ...
         (i_xgemm_desc->flags == LIBXSMM_GEMM_FLAG_USE_XGEMM_ABI + LIBXSMM_GEMM_FLAG_TRANS_A ) ||
         (i_xgemm_desc->flags == LIBXSMM_GEMM_FLAG_USE_XGEMM_ABI + LIBXSMM_GEMM_FLAG_TRANS_A + LIBXSMM_GEMM_FLAG_TRANS_B ) ||
         (i_xgemm_desc->flags == LIBXSMM_GEMM_FLAG_USE_XGEMM_ABI + LIBXSMM_GEMM_FLAG_TRANS_A + LIBXSMM_GEMM_FLAG_TRANS_B + LIBXSMM_GEMM_FLAG_BATCH_REDUCE_STRIDE )
         ) ){
