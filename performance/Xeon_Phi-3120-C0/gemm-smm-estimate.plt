...
stats BASENAME.".txt" using (log(column(FLOPS)*2*NCORES)) nooutput; GEO = sprintf("%.1f", exp(STATS_sum/STATS_records))
stats BASENAME.".txt" using (column(FLOPS)*2*NCORES) nooutput; MED = sprintf("%.1f", STATS_median)
...
plot BASENAME.".txt" using (IX(column(MPARM), column(NPARM), XN)):(column(FLOPS)*2*NCORES) smooth unique
unset table
set table "gemx-cdf.txt"
plot BASENAME.".txt" using (column(FLOPS)*2*NCORES):(1.0) smooth cumulative
unset table
...
if (GEN==1) set output "gemm-smm-estimate.".EXT
if (PREC eq "f64") {
  PEAK = PEAK_DP_GFLOPS
  PRECISION = "D"
} else {
...
if (exists("FUNCB") && (FUNCB ne "") && exists("NAMEB") && (NAMEB ne "")) {
  ...
  stats "< paste ".BASENAME.".txt ".GEMX."-".FUNCB."-".PREC.".txt" using ((column(FLOPS)-column(FLOPS+SHIFT))*2*NCORES) nooutput
  ...
  splot "< paste ".BASENAME.".txt ".GEMX."-".FUNCB."-".PREC.".txt" using MPARM:NPARM:KPARM:((column(FLOPS)-column(FLOPS+SHIFT))*2*NCORES) notitle with points pointtype 7 linetype palette
}
...
set format x "%g"; set format y "%g"; set format z "%g"; set format cb "%g"
splot BASENAME.".txt" using MPARM:NPARM:KPARM:(column(FLOPS)*2*NCORES) notitle with points pointtype 7 linetype palette
...
if (0 < PEAK) {
  ...
  set y2range [0:PEAK]
  if (exists("FUNCB") && (FUNCB ne "") && exists("NAMEB") && (NAMEB ne "")) {
    plot BASENAME.".txt" using KPARM:((MN==column(MPARM)&&MN==column(NPARM))?(100.0*column(FLOPS)*2*NCORES/PEAK):(1/0)) title NAME smooth sbezier with points pointtype 7 pointsize 0.5, \
         GEMX."-".FUNCB."-".PREC.".txt" using KPARM:((MN==column(MPARM)&&MN==column(NPARM))?(100.0*column(FLOPS)*2*NCORES/PEAK):(1/0)) title NAMEB smooth sbezier with points pointtype 7 pointsize 0.5
  } else {
    plot BASENAME.".txt" using KPARM:((MN==column(MPARM)&&MN==column(NPARM))?(100.0*column(FLOPS)*2*NCORES/PEAK):(1/0)) notitle smooth sbezier with points pointtype 7 pointsize 0.5
  }
}
