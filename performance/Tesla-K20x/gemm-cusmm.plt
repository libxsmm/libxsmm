...
plot BASENAME.".txt" using (IX(column(MPARM), column(NPARM), XN)):FLOPS smooth unique
unset table
set table "gemx-cdf.txt"
plot BASENAME.".txt" using FLOPS:(1.0) smooth cumulative
unset table
...
if (exists("FUNCB") && (FUNCB ne "") && exists("NAMEB") && (NAMEB ne "")) {
  ...
  stats "< paste ".BASENAME.".txt ".GEMX."-".FUNCB."-".PREC.".txt" using (column(FLOPS)-column(FLOPS+SHIFT)) nooutput
  ...
  splot "< paste ".BASENAME.".txt ".GEMX."-".FUNCB."-".PREC.".txt" using MPARM:NPARM:KPARM:(column(FLOPS)-column(FLOPS+SHIFT)) notitle with points pointtype 7 linetype palette
}
...
set format x "%g"; set format y "%g"; set format z "%g"; set format cb "%g"
splot BASENAME.".txt" using MPARM:NPARM:KPARM:FLOPS notitle with points pointtype 7 linetype palette
...
if (0 < PEAK) {
  ...
  set y2range [0:PEAK]
  if (exists("FUNCB") && (FUNCB ne "") && exists("NAMEB") && (NAMEB ne "")) {
    plot BASENAME.".txt" using KPARM:((MN==column(MPARM)&&MN==column(NPARM))?(100.0*column(FLOPS)/PEAK):(1/0)) title NAME smooth sbezier with points pointtype 7 pointsize 0.5, \
         GEMX."-".FUNCB."-".PREC.".txt" using KPARM:((MN==column(MPARM)&&MN==column(NPARM))?(100.0*column(FLOPS)/PEAK):(1/0)) title NAMEB smooth sbezier with points pointtype 7 pointsize 0.5
  } else {
    plot BASENAME.".txt" using KPARM:((MN==column(MPARM)&&MN==column(NPARM))?(100.0*column(FLOPS)/PEAK):(1/0)) notitle smooth sbezier with points pointtype 7 pointsize 0.5
  }
}
