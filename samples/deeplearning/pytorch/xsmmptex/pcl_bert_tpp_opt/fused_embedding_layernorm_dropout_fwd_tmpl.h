...
******************************************************************************/
  ...
  PCL_ASSERT((!in_ids_null || !in_emb_null), "Either Input_ids or input_embeddings must be non-empty");
  ...
  if (training) t_emb_out = t_gamma.new_empty({B, S1, N, S2, H});
  ...
  if (p > 0) t_dp_mask = at::empty({B, S1, (N*S2*H+15)/16}, at::kShort);
  ...
  auto layer_norm_fwd_tpp = SCOPEIT(LayerNormFwdTPP<T>(N, S2, H, eps), LAYER_NORM);
      ...
      for(int b = 0; b < B; b++) {
        for(int s1 = 0; s1 < S1; s1++) {
          for(int s2 = 0; s2 < S2; s2++) {
            ...
            if (!in_ids_null) w_id = in_ids[b][s1][s2];
            if (!pos_ids_null) pos_id = pos_ids[b][s1][s2];
            if (!tt_ids_null) tt_id = tt_ids[b][s1][s2];
            for (int n = 0; n < N; n++) {
              for(int h = 0; h < H; h++) {
                float sum = 0.0f;
                if (!in_ids_null) {
                  if (w_id != pad_id) sum += word_emb[w_id][n][h];
                } else {
...
#ifdef NO_TPP_EF
          pcl_layer_norm_fwd(N, S2, H, emb_out[b][s1][0][0], gamma[0], beta[0], mean[b][s1], var[b][s1], out[b][s1][0][0], eps);
          if(p > 0) {
            pcl_dropout_fwd(N*S2*H, out[b][s1][0][0], out[b][s1][0][0], dp_mask[b][s1], p);
          }
#else
          layer_norm_fwd_tpp(emb_out[b][s1][0][0], gamma[0], beta[0], mean[b][s1], var[b][s1], out[b][s1][0][0]);
          if(p > 0) {
            dropout_fwd_tpp(out[b][s1][0][0], (void*)rnd_state, out[b][s1][0][0], dp_mask[b][s1]);
          }
#endif
        }
      }
