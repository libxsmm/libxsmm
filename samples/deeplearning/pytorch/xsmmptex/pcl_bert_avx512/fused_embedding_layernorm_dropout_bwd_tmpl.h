...
#pragma omp parallel
      {
        ...
        xsmm_set_zero(N*H, prv_grad_gamma[0]);
        xsmm_set_zero(N*H, prv_grad_beta[0]);
...
#ifdef NO_TPP_EB
            if (p > 0) {
              pcl_dropout_bwd(N*S2*H, grad_out[b][s1][0][0], grad_emb_out[b][s1][0][0], dp_mask[b][s1], p);
            }
            pcl_layer_norm_bwd(N, S2, H, grad_emb_out[b][s1][0][0], emb_out[b][s1][0][0], mean[b][s1], var[b][s1], gamma[0], grad_emb_out[b][s1][0][0], prv_grad_gamma[0], prv_grad_beta[0]);
#else
            if (p > 0) {
              drop_out_bwd_tpp(grad_out[b][s1][0][0], grad_emb_out[b][s1][0][0], dp_mask[b][s1]);
            }
            layer_norm_bwd_tpp(grad_emb_out[b][s1][0][0], emb_out[b][s1][0][0], mean[b][s1], var[b][s1], gamma[0], grad_emb_out[b][s1][0][0], prv_grad_gamma[0], prv_grad_beta[0]);
#endif
          }
        }
#pragma omp barrier
        xsmm_reduce_buf(num_threads, N*H, gamma_ptrs, grad_gamma[0]);
        xsmm_reduce_buf(num_threads, N*H, beta_ptrs, grad_beta[0]);
      }
    }
  }
  {
    ...
    for(int b = 0; b < B; b++) {
      for(int s1 = 0; s1 < S1; s1++) {
        for(int s2 = 0; s2 < S2; s2++) {
          ...
          if (in_emb_null) w_id = in_ids[b][s1][s2];
          ...
          for (int n = 0; n < N; n++) {
            for(int h = 0; h < H; h++) {
              ...
              if (in_emb_null) {
                if (w_id != pad_id) grad_word_emb[w_id][n][h] += grad;
              } else {
  ...
  return std::vector<at::Tensor>({t_grad_in_emb, t_grad_gamma, t_grad_beta, t_grad_word_emb, t_grad_pos_emb, t_grad_tt_emb});
