...
void norm_to_vnni_16b(libxsmm_bfloat16* in, libxsmm_bfloat16* out, int N, int M)
{
  ...
  libxsmm_meltwfunction_transform trans_kernel = libxsmm_dispatch_meltw_transform(M, N, &M, &M, LIBXSMM_DATATYPE_BF16, LIBXSMM_DATATYPE_BF16, trans_flags);
...
inline void bf16_copy(int N, int M, int LDO, int LDI, libxsmm_meltw_unary_param  *params)
{
  ...
  libxsmm_meltwfunction_unary kernel = libxsmm_dispatch_meltw_unary(M, N, &LDI, &LDO, LIBXSMM_DATATYPE_BF16, LIBXSMM_DATATYPE_BF16, compute_dtype, unary_flags, unary_type);
...
inline void add_bf16_bf16(int N, int M, libxsmm_meltw_binary_param *binary_param)
{
  ...
  libxsmm_meltwfunction_binary add_kernel = libxsmm_dispatch_meltw_binary(M, N, NULL, NULL, LIBXSMM_DATATYPE_BF16, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_BF16, binary_flags, binary_type);
...
// void delbias_bf16_f32_gold(int N, int K, libxsmm_bfloat16* delout, float* delbias)
// {
//   for(int mm=0; mm<N; mm++) {
//     int kk;
//     for(kk=0; kk<LIBXSMM_ALIGNDOWN(K, 16); kk+=16) {
//       __m512 vout = _mm512_castsi512_ps(_mm512_slli_epi32(_mm512_cvtepi16_epi32(_mm256_loadu_si256((__m256i*)(delout+mm*K+kk))),16));
//       __m512 vbias = _mm512_loadu_ps(delbias+kk);
//       _mm512_storeu_ps(delbias+kk, _mm512_add_ps(vbias, vout));
//     }
//     if(kk < K)
//     {
//       __mmask16 mask = (1 << (K - kk)) - 1;
//       __m512 vout = _mm512_castsi512_ps(_mm512_slli_epi32(_mm512_cvtepi16_epi32(_mm256_maskz_loadu_epi16(mask, (__m256i*)(delout+mm*K+kk))),16));
//       __m512 vbias = _mm512_maskz_loadu_ps(mask, delbias+kk);
//       _mm512_mask_storeu_ps(delbias+kk, mask, _mm512_mask_add_ps(_mm512_setzero_ps(), mask, vbias, vout));
//     }
//   }
// }
...
class MLPCell_BF16
{
  ...
  std::vector<at::Tensor> fwd(std::vector<at::Tensor> inputs)
  {
    ...
    libxsmm_meltw_dropout_flags dropout_flags = LIBXSMM_MELTW_FLAG_DROPOUT_FWD_BITMASK;
      ...
      for(int m=tb; m<te; m++) {
        ...
        if(ptrain && pp > 0)
        {
          for(int k=0; k<nk; k++)
          {
            ...
            dropout_bf16_fwd(bn, bk, &dropout_params);
          }
        }
    ...
    if(rn > 0)
    {
      ...
      if(ptrain && pp > 0)
      {
        for(int k=0; k<nk; k++)
        {
          ...
          dropout_bf16_fwd(rn, bk, &dropout_params);
        }
      }
...
std::vector<at::Tensor> bwd(std::vector<at::Tensor> inputs)
  {
    ...
    libxsmm_meltw_unary_param cvt_params;
    libxsmm_meltw_dropout_flags dropout_flags = LIBXSMM_MELTW_FLAG_DROPOUT_BWD_BITMASK;
        ...
        for(int m=tb; m<te; m++) {
          ...
          for(int k=0; k<nk; k++) {
            if(ptrain && pp > 0) {
              ...
              dropout_bf16_bwd(bn, bk, &dropout_params);
            }
    ...
    if(rn > 0)
    {
      ...
      if(ptrain && pp > 0)
      {
        for(int k=0; k<nk; k++) {
          ...
          dropout_bf16_bwd(rn, bk, &dropout_params);
        }
      }
