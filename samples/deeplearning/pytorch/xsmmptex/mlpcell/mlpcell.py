...
class XsmmMLPCellFunction(Function):
    ...
    def forward(ctx, handle, data_type, *inputs):
        ...
        # print("mlpcell forward : ", time.time()-tic)
    ...
    def backward(ctx, *grad_outs):
        ...
        # print("mlpcell backward : ", time.time()-tic)
...
class DropoutFn(Function):
  ...
  def forward(ctx, input, p, train, data_type):
    # tic = time.time()
    # print("Dropout fwdpass", flush=True)
    output, mask = mlpcell_ext.dropout_forward(input, p, train, data_type)
    # print("DropoutFn Forward : ", time.time()-tic)
    # print("DP-----Fwd")
  ...
  def backward(ctx, grad_output):
    ...
    # print("Dropout backpass", flush=True)

    # tic = time.time()
    grad_input = mlpcell_ext.dropout_backward(grad_output, mask, p, data_type)
    # print("DropoutFn Backward : ", time.time()-tic)
...
class LeakyReluFn(Function):
  @staticmethod
  def forward(ctx, input, alpha, data_type):
    output, mask = mlpcell_ext.leakyrelu_forward(input, alpha, data_type)
    # print("DP-----Fwd")
    ctx.save_for_backward(mask)
    ctx.backward_cache = data_type
    ctx.alpha = alpha
  ...
  def backward(ctx, grad_output):
    ...
    grad_input = mlpcell_ext.leakyrelu_backward(grad_output, mask, alpha, data_type)
    return (grad_input, None, None, None)
...
class LeakyRelu(nn.Module):
  ...
  # def __init__(self, data_type, alpha: float = 1e-2, inplace: bool = False):
  ...
  def forward(self, input):
    ...
    if input.dtype == torch.float32:
      ...
      data_type = "bf16"
    output = LeakyReluFn.apply(input, self.alpha, data_type)
    return output
