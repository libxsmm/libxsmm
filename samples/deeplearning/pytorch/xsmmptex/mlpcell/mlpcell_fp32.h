...
inline void delbias_f32(int N, int M, int LD_N, int LD_M, libxsmm_meltw_unary_param *delbias_params)
{
  ...
  libxsmm_meltwfunction_unary delbias_kernel = libxsmm_dispatch_meltw_unary(M, N, &LD_M, &LD_N, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32, unary_flags, unary_type);
...
inline void add_f32_f32(int N, int M, libxsmm_meltw_binary_param *binary_param)
{
  ...
  libxsmm_meltwfunction_binary add_kernel = libxsmm_dispatch_meltw_binary(M, N, NULL, NULL, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32, binary_flags, binary_type);
  if ( add_kernel == NULL ){
...
class MLPCell_F32
{
  ...
  std::vector<at::Tensor> fwd(std::vector<at::Tensor> inputs)
  {
    ...
    // std::cout << "bn: " << bn << " bk: " << bk << " nn: " << nn << " rn: " << rn << " nk: " << nk << " K: " << K << " pN: " << pN <<std::endl;
      ...
      for(int m=tb; m<te; m++) {
        ...
        if(ptrain && pp > 0)
        {
          for(int k=0; k<nk; k++)
          {
            ...
            dropout_f32_fwd(bn, bk, &dropout_params);
          }
        }
    ...
    if(rn > 0)
    {
      ...
      if(ptrain && pp > 0)
      {
        for(int k=0; k<nk; k++)
        {
          ...
          dropout_f32_fwd(rn, bk, &dropout_params);
        }
      }
...
std::vector<at::Tensor> bwd(std::vector<at::Tensor> inputs)
  {
    ...
    time_taken *= 1e-9;
    // std::cout << "Time Initial bwd: " << time_taken << " sec" << std::endl;
...
#pragma omp parallel reduction(+: grad_wt_f32_l[:nk][:nc][:bk][:bc], grad_bias_l[:K], grad_wt_f32_r[:nk][:nc][:bk][:bc], grad_bias_r[:K])
    ...
    auto start3 = std::chrono::high_resolution_clock::now();
            ...
            if(ptrain && pp > 0) {
              ...
              dropout_f32_bwd(bn, bk, &dropout_params);
            }
#if TIMER
      auto end3 = std::chrono::high_resolution_clock::now();
    double time_taken3 = std::chrono::duration_cast<std::chrono::nanoseconds>(end3 - start3).count();
    time_taken3 *= 1e-9;
    std::cout << "Time Dropout1 bwd: " << time_taken3 << " sec" << std::endl;
    ...
    double time_taken4 = std::chrono::duration_cast<std::chrono::nanoseconds>(end4 - start4).count();
    time_taken4 *= 1e-9;
    std::cout << "Time Relu1 bwd: " << time_taken4 << " sec" << std::endl;
    ...
    double time_taken5 = std::chrono::duration_cast<std::chrono::nanoseconds>(end5 - start5).count();
    time_taken5 *= 1e-9;
    std::cout << "Time copy1 bwd: " << time_taken5 << " sec" << std::endl;
#endif
          }
...
#endif
         // std::cout << "BRGEMM1 bn " << bn << " bc " << bc << " bk " << bk  << std::endl;
          int count=1;
          brgemm_f32_f32(bn, bc, bk, bn*bk, 0, grad_output_bN[tid][0][0], wt_f32_l[0][0][0], grad_input_bN_l[tid][0][0], count, 0.0);

#if TIMER
      auto end6 = std::chrono::high_resolution_clock::now();
    double time_taken6 = std::chrono::duration_cast<std::chrono::nanoseconds>(end6 - start6).count();
    time_taken6 *= 1e-9;
    std::cout << "Time brgemm1 bwd: " << time_taken6 << " sec" << std::endl;
    ...
    double time_taken7 = std::chrono::duration_cast<std::chrono::nanoseconds>(end7 - start7).count();
    time_taken7 *= 1e-9;
    std::cout << "Time copy2 bwd: " << time_taken7 << " sec" << std::endl;
    ...
    double time_taken8 = std::chrono::duration_cast<std::chrono::nanoseconds>(end8 - start8).count();
    time_taken8 *= 1e-9;
    std::cout << "Time brgemm2 bwd: " << time_taken8 << " sec" << std::endl;
    ...
    double time_taken9 = std::chrono::duration_cast<std::chrono::nanoseconds>(end9 - start9).count();
    time_taken9 *= 1e-9;
    std::cout << "Time copy3 bwd: " << time_taken9 << " sec" << std::endl;
    ...
    double time_taken10 = std::chrono::duration_cast<std::chrono::nanoseconds>(end10 - start10).count();
    time_taken10 *= 1e-9;
    std::cout << "Time brgemm3 bwd: " << time_taken10 << " sec" << std::endl;
    ...
    double time_taken11 = std::chrono::duration_cast<std::chrono::nanoseconds>(end11 - start11).count();
    time_taken11 *= 1e-9;
    std::cout << "Time bias bwd: " << time_taken11 << " sec" << std::endl;
    ...
    time_taken1 *= 1e-9;
    std::cout << "Time mult thread compute bwd: " << time_taken1 << " sec" << std::endl;
#endif
    }
    else
    {
...
#pragma omp parallel reduction(+: grad_wt_f32_l[:nk][:nc][:bk][:bc], grad_bias_l[:K])
    ...
    time_taken2 *= 1e-9;
    std::cout << "Time Multi thread else bwd: " << time_taken2 << " sec" << std::endl;
    ...
    if(rn > 0)
    {
      ...
      if(ptrain && pp > 0)
      {
        for(int k=0; k<nk; k++) {
          ...
          dropout_f32_bwd(rn, bk, &dropout_params);
        }
      }
    ...
    double time_taken3 = std::chrono::duration_cast<std::chrono::nanoseconds>(end3 - start3).count();
    time_taken3 *= 1e-9;
    // std::cout << "Time Single-thread bwd: " << time_taken3 << " sec" << std::endl;
#endif
