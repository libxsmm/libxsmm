...
#pragma omp parallel for
  for (int n = 0; n < N; n++)
  {
    ...
    embbag.ref(output[n], weight[0], &input[start], end-start);
  }
#else // NO_TPP
#pragma omp parallel for
  for (int n = 0; n < N; n++)
...
#pragma omp parallel for
  for (int n = 0; n < N; n++) {
    auto start = offsets[n];
    auto end = (n < N - 1 ? offsets[n + 1] : NS);
    embbag_bwd.ref(gradout[n], values[start], end-start);
  }
#else
#pragma omp parallel for
  for (int n = 0; n < N; n++)
...
#ifndef NO_TPP_UPD
              split_sgd_kernel.ref((at::BFloat16 *)ha, (at::BFloat16 *)la, va, lr);
#else
...
#ifndef NO_TPP_UPD
      split_sgd_kernel.ref((at::BFloat16 *)(hi_ptr+i), (at::BFloat16 *)(lo_ptr+i), grad_ptr+i, lr);
#else
...
#else
      cvt_in1.ref(&input1[i*M*K], tmpa);
      cvt_in2.ref(&input2[i*K*N], tmpb);
#endif
      for(unsigned int j = 0; j < M; j++) {
        for(unsigned int k = 0; k < K; k++) {
