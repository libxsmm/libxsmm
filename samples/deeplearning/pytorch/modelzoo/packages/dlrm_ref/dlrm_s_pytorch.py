...
try:
    import pcl_embedding_bag_dummy # should fail
except ImportError as e:
    print(e)
    pcl_embedding_bag = None

try:
    import pcl_mlp_dummy # should fail
except ImportError as e:
    print(e)
    pcl_mlp = None
...
sync_alltoall = bool(os.environ.get('DLRM_SYNC_ALLTOALL', None))
...
class LRWarmupScheduler(_LRScheduler):
    def __init__(self, optimizer, num_warmup_iters, decay_start_iter, num_decay_iters):
        self.num_warmup_iters = num_warmup_iters
        self.decay_start_iter = decay_start_iter
        self.num_decay_iters = num_decay_iters
        super(LRWarmupScheduler, self).__init__(optimizer)

    def get_lr(self):
        step_count = self._step_count
        if (self.num_warmup_iters > 0) and (step_count < self.num_warmup_iters):
            scale = 1.0 - (self.num_warmup_iters - step_count) / self.num_warmup_iters
            lr = [base_lr * scale for base_lr in self.base_lrs]
        elif self.num_decay_iters != 0 and step_count >= self.decay_start_iter:
            steps_since_decay_start = step_count - self.decay_start_iter
            already_decayed_steps = min(steps_since_decay_start, self.num_decay_iters)
            scale = ((self.num_decay_iters - already_decayed_steps) / self.num_decay_iters) ** 2
            min_lr = 0.0000001
...
class DLRM_Net(nn.Module):
    ...
    def apply_mlp(self, x, layers):
        ...
        align_to = 2
        need_padding = self.use_xsmm and x.dtype == torch.bfloat16 and x.size(0) % align_to != 0
        if need_padding:
            pad_size = align_to - x.size(0) % align_to
            x = torch.nn.functional.pad(input=x, pad=(0,0,0,pad_size), mode='constant', value=0)
            ret = layers(x)
            return(ret[:-pad_size,:])
        else:
            return layers(x)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--arch-embedding-dtype", type=str, default="fp32", choices=['fp32', 'fp16', 'bf16'])
    # j will be replaced with the table number
    parser.add_argument("--arch-mlp-bot", type=str, default="4-3-2")
    parser.add_argument("--arch-mlp-top", type=str, default="4-2-1")
    parser.add_argument("--arch-mlp-dtype", type=str, default="fp32", choices=['fp32', 'fp16', 'bf16'])
    parser.add_argument("--arch-mlp-impl", type=str, default="", choices=['xsmm', 'mkldnn', ''])
    ...
    parser.add_argument("--dump-op-timings", action="store_true", default=False)
    ...
    parser.add_argument("--decay-start-iter", type=int, default=0)
    parser.add_argument("--num-decay-iters", type=int, default=0)
    ...
    # Allow checkpointing with multiple ranks
    if ext_dist.my_size > 1:
        if args.save_model != "":
            args.save_model = "%s.r%d_%d" % (args.save_model, ext_dist.my_rank, ext_dist.my_size)
        if args.load_model != "":
            args.load_model = "%s.r%d_%d" % (args.load_model, ext_dist.my_rank, ext_dist.my_size)
    ...
    if (args.mini_batch_size % ext_dist.my_size !=0 or args.test_mini_batch_size % ext_dist.my_size != 0) and ext_dist.allgatherv_supported == False:
    ...
    dlrm = DLRM_Net(
        ...
        emb_dtype=args.arch_embedding_dtype,
        mlp_dtype=args.arch_mlp_dtype,
        mlp_impl=args.arch_mlp_impl,
    )
    # test prints
    if args.debug_mode:
    ...
    if not args.inference_only:
        ...
        if split_sgd.is_available():
            SGD = split_sgd.SplitSGD
        else:
            SGD = torch.optim.SGD

        if ext_dist.my_size == 1:
            optimizer = SGD(dlrm.parameters(), lr=args.learning_rate)
        else:
            optimizer = SGD([
        ...
        scheduler = LRWarmupScheduler(optimizer, args.num_warmup_iters, args.decay_start_iter, args.num_decay_iters)
    ...
    my_end_time = time_wrap(use_gpu)

    with torch.autograd.profiler.profile(args.enable_profiling, use_cuda=use_gpu, record_shapes=record_shapes, **prof_arg_dict) as prof:
        while k < args.nepochs:
            ...
            for j, (X, lS_o, lS_i, T) in enumerate(train_ld):
                ...
                if args.mlperf_logging:
                    ...
                    my_data_time = current_time - my_end_time
                    if previous_iteration_time:
                        iteration_time = current_time - previous_iteration_time
                    else:
                        iteration_time = 0
                    previous_iteration_time = current_time
                    #print("Data loader time = %10.3f ms, compute time = %10.3f ms, total time = %10.3f ms" % (my_data_time * 1000, (iteration_time-my_data_time) *1000, iteration_time *1000))
                ...
                if not ext_dist.allgatherv_supported and ext_dist.my_size > 1 and X.size(0) % ext_dist.my_size != 0:
                ...
                if not args.inference_only:
                    ...
                    # optimizer
                    optimizer.step()
                    #print("lr used: %s, base_lrs: %s" % (scheduler.get_last_lr(), scheduler.base_lrs))
                    scheduler.step()
                ...
                if args.mlperf_logging:
                    ...
                    if prof and prof_support_suspend_resume and j >= prof_start_iter and j < prof_end_iter: prof.suspend()
                    total_time += t2 - t1
                total_accu += A
                total_loss += L * mbs
                total_iter += 1
                ...
                if should_test and not args.inference_only:
                    ...
                    for i, (X_test, lS_o_test, lS_i_test, T_test) in enumerate(test_ld):
                        ...
                        # Skip the batch if batch size not multiple of total ranks
                        if not ext_dist.allgatherv_supported and ext_dist.my_size > 1 and X_test.size(0) % ext_dist.my_size != 0:
    ...
    if args.enable_profiling:
        ...
        try:
            ...
            pass
        if args.dump_op_timings:
            ext_dist.print_op_timings(prof, use_gpu, file_prefix)
