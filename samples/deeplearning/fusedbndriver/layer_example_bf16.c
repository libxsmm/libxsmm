...
int main(int argc, char* argv[])
{
  ...
  libxsmm_bfloat16 *naive_input_pad_bf16, *naive_output_pad_bf16, *naive_input_add_pad_bf16, *naive_delinput_add_pad_bf16, *naive_delinput_pad_bf16, *naive_deloutput_pad_bf16;
  libxsmm_bfloat16 *naive_libxsmm_output, *naive_libxsmm_delinput, *naive_libxsmm_delinput_add;
  float *naive_libxsmm_output_f32, *naive_libxsmm_delinput_f32, *naive_libxsmm_delinput_add_f32;
  float *naive_beta, *naive_gamma, *naive_delbeta, *naive_delgamma, *naive_expectval, *naive_rcpstddev, *naive_variance;
  libxsmm_bfloat16 *input_libxsmm, *output_libxsmm, *input_add_libxsmm, *delinput_libxsmm, *deloutput_libxsmm, *delinput_add_libxsmm;
  ...
  printf("SIZE Input  (MB): %10.2f MiB\n", (double)(nImg*nFm*ifhp*ifwp*sizeof(libxsmm_bfloat16))/(1024.0*1024.0) );
  printf("SIZE Output (MB): %10.2f MiB\n", (double)(nImg*nFm*ofhp*ofwp*sizeof(libxsmm_bfloat16))/(1024.0*1024.0) );
  printf("SIZE Input   (1): %10.2f MiB\n", (double)(1*nFm*ifhp*ifwp*  sizeof(libxsmm_bfloat16))/(1024.0*1024.0) );
  printf("SIZE Output  (1): %10.2f MiB\n", (double)(1*nFm*ofhp*ofwp*  sizeof(libxsmm_bfloat16))/(1024.0*1024.0) );
  ...
  naive_input_add_pad_bf16       = (libxsmm_bfloat16*)libxsmm_aligned_malloc( nImg*nFm*ifhp*ifwp*sizeof(libxsmm_bfloat16), 2097152);
  naive_delinput_pad_bf16        = (libxsmm_bfloat16*)libxsmm_aligned_malloc( nImg*nFm*ifhp*ifwp*sizeof(libxsmm_bfloat16), 2097152);
  naive_delinput_add_pad_bf16    = (libxsmm_bfloat16*)libxsmm_aligned_malloc( nImg*nFm*ifhp*ifwp*sizeof(libxsmm_bfloat16), 2097152);
  ...
  naive_libxsmm_delinput_add     = (libxsmm_bfloat16*)libxsmm_aligned_malloc( nImg*nFm*ifhp*ifwp*sizeof(libxsmm_bfloat16), 2097152);
  ...
  input_add_libxsmm              = (libxsmm_bfloat16*)libxsmm_aligned_malloc( nImg*nFm*ifhp*ifwp*sizeof(libxsmm_bfloat16), 2097152);
  delinput_add_libxsmm           = (libxsmm_bfloat16*)libxsmm_aligned_malloc( nImg*nFm*ifhp*ifwp*sizeof(libxsmm_bfloat16), 2097152);
 ...
 /* initialize data */
  ...
  if (format == 'A' || format == 'L') {
    ...
    fusedbatchnorm_desc.datatype_in = LIBXSMM_DNN_DATATYPE_BF16;
    fusedbatchnorm_desc.datatype_out = LIBXSMM_DNN_DATATYPE_BF16;
    ...
    CHKERR_LIBXSMM_DNN( libxsmm_dnn_copyin_tensor( libxsmm_input_add,    (void*)naive_input_add_pad_bf16,    LIBXSMM_DNN_TENSOR_FORMAT_NCHW ) );
    ...
    CHKERR_LIBXSMM_DNN( libxsmm_dnn_copyin_tensor( libxsmm_delinput_add, (void*)naive_delinput_add_pad_bf16, LIBXSMM_DNN_TENSOR_FORMAT_NCHW ) );
...
#     pragma omp parallel
      ...
      for ( i = 0; i < nImg*nFm*ofhp*ofwp; ++i ) {
        if ( (naive_output_pad[i] == 0.0f && naive_libxsmm_output_f32[i] != 0.0f) ||
             (naive_output_pad[i] != 0.0f && naive_libxsmm_output_f32[i] == 0.0f)    ) {
          relu_no_match++;
        }
      }
...
#     pragma omp parallel
      ...
      libxsmm_convert_bf16_f32( naive_libxsmm_delinput_add, naive_libxsmm_delinput_add_f32, nImg*nFm*ifhp*ifwp );
      copy_internal_nchw( naive_delinput_pad, naive_delinput, nImg, nFm, ifh, ifw, pad_h_in, pad_w_in);
      copy_internal_nchw( naive_delinput_add_pad, naive_delinput_add, nImg, nFm, ifh, ifw, pad_h_in, pad_w_in);

      /* compare */
      printf("delinput_add:\n");
      libxsmm_matdiff(&norms_bwd, LIBXSMM_DATATYPE_F32, nImg*nFm*ifhp*ifwp, 1, naive_delinput_add_pad, naive_libxsmm_delinput_add_f32, 0, 0);
      printf("L1 reference  : %.25g\n", norms_bwd.l1_ref);
      printf("L1 test       : %.25g\n", norms_bwd.l1_tst);
      printf("L2 abs.error  : %.24f\n", norms_bwd.l2_abs);
      printf("L2 rel.error  : %.24f\n", norms_bwd.l2_rel);
...
#     pragma omp parallel  private(i)
  ...
  libxsmm_free(naive_input_add_pad_bf16);
  libxsmm_free(naive_output_pad_bf16);
  libxsmm_free(naive_delinput_pad_bf16);
  libxsmm_free(naive_delinput_add_pad_bf16);
  ...
  libxsmm_free(naive_libxsmm_delinput_add_f32);
