...
def init_distributed(rank = -1, size = -1, backend=''):
    ...
    if size > 1:
        ...
        if hasattr(dist, 'all_to_all_single'):
            try:
                dist.all_to_all_single(torch.ones([my_size]), torch.ones([my_size]))
...
def print_op_timings(prof, use_gpu, prefix):
    ...
    if my_size > 1:
        ...
        dist.all_reduce(start_time, dist.ReduceOp.MIN)
        start_time = np.trunc(start_time.item())
    # print("Profiling Min Start = %s" % start_time)
    with open("%s.OpTimings.txt" % prefix, "w") as f:
        for i, fe in enumerate(sorted_fe):
            ...
            print("%-3d %-6d %6d %12.4f %12.4f %12.4f %2s %s   %-40s    %s" % (my_rank, i, fe.id, (fe.cpu_interval.start - start_time)/1000.0, (fe.cpu_interval.end - start_time)/1000.0, fe.cpu_interval.elapsed_us()/1000.0, fe.thread, cstr, fe_name.replace(' ', '_'), fe.input_shapes), file=f)
