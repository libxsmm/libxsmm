...
#   pragma omp parallel private(i,j)
#endif
    {
      ...
      for (j = 0; j < iters; ++j) {
        if ( prec_bf16 > 0 ) {
          for ( i = 0; i < num_layers; ++i) {
            my_fc_fwd_exec_bf16( my_fc_fwd[i], fil_libxsmm_bf16[i], act_libxsmm_bf16[i], act_libxsmm_bf16[i+1],
                            bias_libxsmm_bf16[i], relumask_libxsmm[i], 0, tid, scratch );
          }
#ifdef USE_SOFTMAX
          my_smax_fwd_exec_bf16( my_smax_fwd, act_libxsmm_bf16[num_layers], act_libxsmm_bf16[num_layers+1], label_libxsmm, &loss, 0, tid, scratch );
#endif
        } else {
          for ( i = 0; i < num_layers; ++i) {
...
#   pragma omp parallel private(i,j)
#endif
    {
      ...
      for (j = 0; j < iters; ++j) {
        if ( prec_bf16 > 0 ) {
#ifdef USE_SOFTMAX
          my_smax_bwd_exec_bf16( my_smax_bwd, delact_libxsmm_bf16[num_layers], act_libxsmm_bf16[num_layers+1], label_libxsmm,
                                 0, tid, scratch );
#endif
          ...
          my_fc_bwd_exec_bf16( my_fc_bwd[0], fil_libxsmm_bf16[0], delact_libxsmm_bf16[0], delact_libxsmm_bf16[0+1], delfil_libxsmm_bf16[0],
                             act_libxsmm_bf16[0], delbias_libxsmm_bf16[0], relumask_libxsmm[0], MY_FC_PASS_BWD_W, 0, tid, scratch );
...
#ifdef USE_SOFTMAX
          my_smax_fwd_exec_bf16( my_smax_fwd, act_libxsmm_bf16[num_layers], act_libxsmm_bf16[num_layers+1], label_libxsmm, &loss,
                                 0, tid, scratch );
          my_smax_bwd_exec_bf16( my_smax_bwd, delact_libxsmm_bf16[num_layers], act_libxsmm_bf16[num_layers+1], label_libxsmm,
                                 0, tid, scratch );
#endif
