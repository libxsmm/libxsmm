    ...
    for (epoch_id = 0; epoch_id < n_epochs; epoch_id++) {
      for (batch_id = 0; batch_id < n_batches; batch_id++) {
        for ( i = 0; i < num_layers; ++i) {
          ...
          my_fc_fwd_exec_bf16( my_fc_fwd[i], fil_libxsmm[i], input_act_ptr, act_libxsmm[i+1],
                               bias_libxsmm[i], relumask_libxsmm[i], 0, tid, scratch );
        }
        my_smax_fwd_exec_bf16( my_smax_fwd, act_libxsmm[num_layers], act_libxsmm[num_layers+1], train_label + batch_id * MB, &loss,
                               0, tid, scratch );
        ...
        my_smax_bwd_exec_bf16( my_smax_bwd, delact_libxsmm[num_layers], act_libxsmm[num_layers+1], train_label + batch_id * MB,
                               0, tid, scratch );
...
#   pragma omp parallel private(i,j,batch_id)
#endif
    {
      ...
      for (batch_id = 0; batch_id < n_batches; batch_id++) {
        ...
        my_smax_fwd_exec_bf16( my_smax_fwd, act_libxsmm[num_layers], act_libxsmm[num_layers+1], test_label + batch_id * MB, &loss,
                               0, tid, scratch );
