...
namespace edge {
  namespace reproducers {
    int neigh( unsigned int const i_nSteps, unsigned int const i_nElements );
  }
}
...
int main(int i_argc, char *i_argv[]) {
  ...
  edge::reproducers::neigh( l_nSteps, l_nElements );

  return 0;
}


int edge::reproducers::neigh( unsigned int const i_nSteps, unsigned int const i_nElements ) {
  ...
  setupPseudoMesh( edge::reproducers::C_MODE_NEIGH, l_nElements,
                   nullptr,
