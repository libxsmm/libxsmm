...
int main(int i_argc, char *i_argv[]) {
  ...
  if ( i_argc == 3 ) {
    ...
    std::cout << "Usage: ./combine_test {NUM_STEPS} {NUM_ELEMENTS} [-h|--help]\n" << std::endl;
    std::exit(1);
  }
  ...
  for ( unsigned int l_step = 0; l_step < l_nSteps; l_step++ ) {
    ...
    // Neighboring
    for ( unsigned int l_el = 0; l_el < l_nElements; l_el++ ) {
      // Surface Integration (neigh)
      for ( unsigned short l_fa = 0; l_fa < C_ENT[T_SDISC.ELEMENT].N_FACES; l_fa++ ) {
        unsigned int l_neEl = l_elFaEl[l_el][l_fa];
        unsigned short l_neFa = l_fIdElFaEl[l_el][l_fa];
  ...
  std::string l_dumpFileName1 = "./dense_combine_o"+std::to_string(ORDER)+"_"
                                "f"+std::to_string(PP_PRECISION)+"_"
                                "el"+std::to_string(l_nElements)+"_"
                                "stp"+std::to_string(l_nSteps)+"_dofs.log";
  std::string l_dumpFileName2 = "./dense_combine_o"+std::to_string(ORDER)+"_"
                                "f"+std::to_string(PP_PRECISION)+"_"
                                "el"+std::to_string(l_nElements)+"_"
