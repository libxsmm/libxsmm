...
if [[ ${LIBXSMM_TARGET} == "clx" || ${LIBXSMM_TARGET} == "avx512_vl256_clx" ]]; then
    PREC_LIST="F32 BF16 F16 BF8 HF8 F64";
fi
...
do
  ...
  do
    ./eltwise_unary_reduce ${M} ${N_ADJ} 100 ${REDUCE_X} ${REDUCE_X2} ${REDUCE_ROWS} ${REDUCE_OP} ${PREC} ${N_IDX} 0 0 0 1
  done
done
