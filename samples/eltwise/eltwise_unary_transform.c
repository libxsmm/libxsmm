...
int test_vnni_to_vnniT_08bit( libxsmm_blasint M, libxsmm_blasint N, libxsmm_blasint ldi, libxsmm_blasint ldo ) {
  ...
  if ( s == 0 ) {
    printf("SUCCESS unary VNNI transpose 08bit\n");
  } else {
    printf("FAILURE unary VNNI transpose 08bit\n");
    ret = EXIT_FAILURE;
  }
...
int test_norm_to_vnni_08bit( libxsmm_blasint M, libxsmm_blasint N, libxsmm_blasint ldi, libxsmm_blasint ldo ) {
  unsigned char *in;
  ...
  if ( s == 0 ) {
    printf("SUCCESS unary VNNI transform 08bit\n");
  } else {
    printf("FAILURE unary VNNI transform 08bit\n");
    ret = EXIT_FAILURE;
  }
...
int main( int argc, char* argv[] ) {
  ...
  if ( op == 'T' && dtype == 8 ) {
    ...
    printf("Testing 08bit VNNI to VNNI Transpose - M=%i, N=%i, LDI=%i, LDO=%i\n", M, N, ldi, ldo);
    ret = test_vnni_to_vnniT_08bit( M, N, ldi, ldo );
    ...
    printf("Testing 08bit NORM to VNNI Reformat - M=%i, N=%i, LDI=%i, LDO=%i\n", M, N, ldi, ldo);
    ret = test_norm_to_vnni_08bit( M, N, ldi, ldo );
