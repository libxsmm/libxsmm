...
void test_relu_f32_f32_fwd( libxsmm_blasint bitm, libxsmm_blasint M, libxsmm_blasint N, libxsmm_blasint ldi, libxsmm_blasint ldo ) {
  ...
  unsigned int i, j;
  unsigned int s;
  libxsmm_meltw_relu_param relu_param;
  libxsmm_meltw_relu_flags relu_flags;
  ...
  /* use jited tranpose */
  relu_param.in_ptr  = (void*)in;
  relu_param.out_ptr = (void*)out;
  relu_param.mask_ptr = (bitm == 0) ? NULL : (void*)mask;
  relu_flags = (bitm == 0) ? LIBXSMM_MELTW_FLAG_RELU_FWD : LIBXSMM_MELTW_FLAG_RELU_FWD_BITMASK;
  libxsmm_meltwfunction_relu relu_kernel = libxsmm_dispatch_meltw_relu(M, N, &ldi, &ldo, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32, relu_flags, 0);
  if ( relu_kernel == NULL ) {
...
void test_relu_bf16_bf16_fwd( libxsmm_blasint bitm, libxsmm_blasint M, libxsmm_blasint N, libxsmm_blasint ldi, libxsmm_blasint ldo ) {
  ...
  relu_param.mask_ptr = (bitm == 0) ? NULL : (void*)mask;
  relu_flags = (bitm == 0) ? LIBXSMM_MELTW_FLAG_RELU_FWD : LIBXSMM_MELTW_FLAG_RELU_FWD_BITMASK;
  libxsmm_meltwfunction_relu relu_kernel = libxsmm_dispatch_meltw_relu(M, N, &ldi, &ldo, LIBXSMM_DATATYPE_BF16, LIBXSMM_DATATYPE_BF16, relu_flags, 0);
  if ( relu_kernel == NULL ) {
    fprintf( stderr, "JIT for RELU TPP. Bailing...!\n");
    exit(-1);
  }
...
void test_relu_f32_bf16_fwd( libxsmm_blasint bitm, libxsmm_blasint M, libxsmm_blasint N, libxsmm_blasint ldi, libxsmm_blasint ldo ) {
  ...
  relu_param.mask_ptr = (bitm == 0) ? NULL : (void*)mask;
  relu_flags = (bitm == 0) ? LIBXSMM_MELTW_FLAG_RELU_FWD : LIBXSMM_MELTW_FLAG_RELU_FWD_BITMASK;
  libxsmm_meltwfunction_relu relu_kernel = libxsmm_dispatch_meltw_relu(M, N, &ldi, &ldo, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_BF16, relu_flags, 0);
  if ( relu_kernel == NULL ) {
    fprintf( stderr, "JIT for RELU TPP. Bailing...!\n");
    exit(-1);
  }
...
void test_relu_bf16_f32_fwd( libxsmm_blasint bitm, libxsmm_blasint M, libxsmm_blasint N, libxsmm_blasint ldi, libxsmm_blasint ldo ) {
  ...
  relu_param.mask_ptr = (bitm == 0) ? NULL : (void*)mask;
  relu_flags = (bitm == 0) ? LIBXSMM_MELTW_FLAG_RELU_FWD : LIBXSMM_MELTW_FLAG_RELU_FWD_BITMASK;
  libxsmm_meltwfunction_relu relu_kernel = libxsmm_dispatch_meltw_relu(M, N, &ldi, &ldo, LIBXSMM_DATATYPE_BF16, LIBXSMM_DATATYPE_F32, relu_flags, 0);
  if ( relu_kernel == NULL ) {
    fprintf( stderr, "JIT for RELU TPP. Bailing...!\n");
    exit(-1);
  }
...
void test_relu_f32_f32_bwd( libxsmm_blasint bitm, libxsmm_blasint M, libxsmm_blasint N, libxsmm_blasint ldi, libxsmm_blasint ldo ) {
  ...
  relu_param.in_ptr  = (void*)in;
  relu_param.out_ptr = (void*)out;
  relu_param.mask_ptr = (void*)mask;
  relu_flags = (bitm == 0) ? LIBXSMM_MELTW_FLAG_RELU_BWD : LIBXSMM_MELTW_FLAG_RELU_BWD_BITMASK;
  libxsmm_meltwfunction_relu relu_kernel = libxsmm_dispatch_meltw_relu(M, N, &ldi, &ldo, LIBXSMM_DATATYPE_F32, LIBXSMM_DATATYPE_F32, relu_flags, 0);
  if ( relu_kernel == NULL ) {
    fprintf( stderr, "JIT for RELU TPP. Bailing...!\n");
    exit(-1);
