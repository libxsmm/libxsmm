...
void test_vnni_to_vnniT_16bit() {
  unsigned short *in, *in_vnni;
  unsigned short *out, *out_gold, *out_vnni;
  unsigned int i, j, j2;
  unsigned int s;

  in       = (unsigned short*)_mm_malloc( sizeof(unsigned short)*64*64, 64);
  in_vnni  = (unsigned short*)_mm_malloc( sizeof(unsigned short)*64*64, 64);
  out      = (unsigned short*)_mm_malloc( sizeof(unsigned short)*64*64, 64);
  out_gold = (unsigned short*)_mm_malloc( sizeof(unsigned short)*64*64, 64);
  out_vnni = (unsigned short*)_mm_malloc( sizeof(unsigned short)*64*64, 64);

  /* init in */
  for ( i = 0; i < 64; ++i ) {
    for ( j = 0; j < 64; ++j ) {
      in[(i*64)+j] = (unsigned short)(((i*64)+j)%112);
    }
  }
  /* to vnni */
  for ( j = 0; j < 64/2; ++j ) {
    for ( i = 0; i < 64 ; ++i ) {
      for( j2 = 0; j2 < 2; ++j2 ) {
        in_vnni[(j*64*2)+(i*2)+j2] = in[(((j*2)+j2)*64)+i];
      }
    }
  }
  ...
  for ( j = 0; j < 64/2; ++j ) {
    for ( i = 0; i < 64 ; ++i ) {
      for( j2 = 0; j2 < 2; ++j2 ) {
        out_vnni[(j*64*2)+(i*2)+j2] = out_gold[(((j*2)+j2)*64)+i];
      }
    }
  }

  /* use our tranpose */
  vnni_transpose_16bit_64by64( in_vnni, out );
...
int main( int argc, char* argv[] ) {
  ...
  if ( argc != 5 ) {
    printf(" Error! Usage: %s [T/V/VT] [8/4/2/1] [M] [N] \n", argv[0] );
    exit(-1);
  }
  ...
  if ( op == 'T' && dtype == 2 ) {
    printf("Testing 16bit Transpose\n");
    test_normal_to_normalT_16bit( M, N );
  } else {
    printf(" Not implemented case! Usage: %s [T/V/VT] [8/4/2/1] [M] [N] \n", argv[0] );
    exit(-1);
  }
}
