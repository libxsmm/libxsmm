  ...
  for ( j = 0; j < N; ++j ) {
    for ( i = 0; i < M; ++i ) {
      if ( type == 0 ) {
        ...
        libxsmm_bfloat16_f32 bf16_hp_out;
        bf16_hp.i[0] = 0;
...
void apply_colbias_add(const gemm_def *i_gemm_def, void *l_c_gold, void *l_colbias) {
  ...
  if (i_gemm_def->out_type == LIBXSMM_DATATYPE_F32) {
    ...
    for (j = 0; j < n; j++) {
      for (i = 0; i < m; i++) {
        libxsmm_bfloat16_f32 tmp_c;
        libxsmm_bfloat16_f32 tmp_colb;
        float res = 0.0;
        tmp_c.i[0] = 0;
    ...
    for (l_r = 0; l_r < i_gemm_def->br_count; l_r++) {
      for (l_j = 0; l_j < n; l_j++) {
        for (l_i = 0; l_i < m; l_i++) {
          ...
          for (l_s = 0; l_s < (k / l_k_block); l_s++) {
            for (l_k2 = 0; l_k2 < l_k_block; l_k2++) {
              libxsmm_bfloat16_f32 tmp_a_f;
              libxsmm_bfloat16_f32 tmp_b_f;
              tmp_a_f.i[0] = 0;
