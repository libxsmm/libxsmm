        ...
        if ( dtype == LIBXSMM_DATATYPE_F64 ) {
          ...
          if (i_gemm_def->is_Ai4Bf16_gemm > 0) {
            if ( pos_val_only != 0 ) {
              ...
              sc_data[(l_r * ld * n) + (l_j * ld) + l_i] = ((char) (get_random_posneg_p5_num() * 10.0))%16;
            }
          } else {
            if ( pos_val_only != 0 ) {
...
void ref_matmul( const gemm_def* i_gemm_def, const void* a, const void* b, void* c ) {
  ...
  if ( (i_gemm_def->a_type    == LIBXSMM_DATATYPE_F64) &&
    ...
    libxsmm_float16* f16_c = (libxsmm_float16*)c;
    const libxsmm_float16* f16_b = (const libxsmm_float16*)b;
    libxsmm_float16 cur_a, cur_b, c_tmp;
    float up_c;
    int l_k_block = 1;
    const char* env_arch = getenv("LIBXSMM_TARGET");
    const int is_env_SPR = (
...
double jit_matmul( const gemm_def*    i_gemm_def,
...
#endif
  {
    const void *const a = (( i_gemm_def->a_type == LIBXSMM_DATATYPE_I8 && i_gemm_def->b_type == LIBXSMM_DATATYPE_F16 && (i_gemm_def->c_type == LIBXSMM_DATATYPE_F16 || i_gemm_def->c_type == LIBXSMM_DATATYPE_F32) ) ? (const void*)i_gemm_def->scf_f16 :
      (( i_gemm_def->a_type == LIBXSMM_DATATYPE_I8 && i_gemm_def->b_type == LIBXSMM_DATATYPE_BF16 && (i_gemm_def->c_type == LIBXSMM_DATATYPE_BF16 || i_gemm_def->c_type == LIBXSMM_DATATYPE_F32) ) ? (const void*)i_gemm_def->scf_f32 : NULL));
    const void *const c = (( i_gemm_def->a_type == LIBXSMM_DATATYPE_I8 && i_gemm_def->b_type == LIBXSMM_DATATYPE_I8 && i_gemm_def->c_type == LIBXSMM_DATATYPE_F32 ) ? &(i_gemm_def->scf) : NULL);
    LIBXSMM_VALUE_ASSIGN(gemm_param.a.tertiary, a);
    LIBXSMM_VALUE_ASSIGN(gemm_param.c.tertiary, c);
  ...
  if (i_gemm_def->br_type == 0) {
    ...
    if ( l_info.prefetch != LIBXSMM_GEMM_PREFETCH_NONE ) {
      LIBXSMM_VALUE_ASSIGN(gemm_param.a.quaternary, i_a);
      LIBXSMM_VALUE_ASSIGN(gemm_param.b.quaternary, i_b);
      gemm_param.c.quaternary = o_c_perf;
    }
